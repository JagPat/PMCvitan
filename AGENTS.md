...
- A current-head finding makes `codex-current-head` fail and returns the PR to
  ...
  PR did not declare.
- Corrections are watched, not assumed. The hourly handoff job holds one lease
  per pull request, exact head and owner, and publishes at most one notification
  when that head has not moved within the bounded interval. Only a new head
  clears the lease — a reaction, a reply, or a subscription is not progress. An
  owner GitHub cannot start (today, anything other than `claude`) is reported as
  `correction_stalled` with the owner, head, findings, and required resume
  action, rather than as work in progress.
