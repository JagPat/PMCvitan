...
- A notice never claims more than it knows. An owner GitHub cannot start (today,
  ...
  `correction_stalled` and names the marker that fixes it. Detecting that an
  asked owner never started — a lease keyed to pull request, exact head and
  owner — is a separate follow-up unit and is not implemented yet, so a stalled
  correction is still noticed by a human.
