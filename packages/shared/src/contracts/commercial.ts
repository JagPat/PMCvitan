...
export const COMMERCIAL_COMMANDS = [
  ...
  // Phase 5 Task 5 (§E/§F/§H) — the §E verdict makes `verified` safe, a certificate makes
  // `certified` safe, and the deduction pair are their own attributable acts (§I). `verify` is a
  // COMMAND rather than a read because the verdict has a consequence: a matched claim moves to
  // `verified`, an exception moves it to `disputed` with the exception naming itself.
...
export const COMMERCIAL_QUERIES = [
  ...
  'commercial.bills',
  'commercial.bill',
  // Phase 5 Task 5 — the §E triple as a READ, so a reviewer can see the verdict without moving
  // the claim, and the certificate register with its deductions and net payable.
...
/**
 * Phase 5 Task 5 (§E) — the exception kinds a three-way verification can return. Each names its
...
/** §H — one withholding against a certificate, with its releases folded. */
export interface DeductionDto {
  id: string;
  type: 'retention' | 'penalty' | 'other';
  amount: string;
  reason: string | null;
  /** Σ releases against this row — the retained balance is a FOLD, never a stored column */
  released: string;
  /** `amount − released` */
  unreleased: string;
  recordedAt: string;
...
/** §E/§G — the certificate and the money that hangs off it. */
export interface CertificateDto {
  id: string;
  billId: string;
  versionId: string;
  certifiedAmount: string;
  /** §G bound 4 — `CERTIFIED − unreleased deductions`, the ceiling any approval is measured against */
  netPayable: string;
  ...
  supersededAt: string | null;
  supersedeReason: string | null;
  /** §I — the attributable override, when one was needed to certify at all */
  sodException: { rule: string; actorId: string; approverId: string; reason: string } | null;
}
