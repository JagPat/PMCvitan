...
test('ensure_web_env matches API demo-auth on the default local URL', () => {
  const webEnv = join(root, 'apps/web/.env');
  const backup = `${webEnv}.ci-backup`;
  const hadBackup = execSync(`test -f ${JSON.stringify(webEnv)} && echo yes || echo no`, {
    cwd: root,
  ...
  try {
    ...
    bashEval(
      `cd ${JSON.stringify(root)} && source scripts/cloud-agent-env.sh && `
      + `export DATABASE_URL="$DEFAULT_DATABASE_URL" && ensure_web_env`,
    );
    const text = execSync(`cat ${JSON.stringify(webEnv)}`, { cwd: root, encoding: 'utf8' });
    assert.match(text, /VITE_ALLOW_DEV_AUTH='true'/);
  } finally {
    if (hadBackup) {
      execSync(`mv ${JSON.stringify(backup)} ${JSON.stringify(webEnv)}`, { cwd: root });
    } else {
...
test('seed.ts serializes on a single-backend advisory lock and does not use AuditLog as a sentinel', () => {
  const text = execSync('cat apps/api/prisma/seed.ts', { cwd: root, encoding: 'utf8' });
  const start = execSync('cat scripts/cloud-agent-start.sh', { cwd: root, encoding: 'utf8' });
  const lock = text.indexOf('pg_advisory_lock');
  const complete = text.indexOf('fixtureComplete');
  const body = text.indexOf('async function seedBody');
  const unlock = text.indexOf('pg_advisory_unlock');
  assert.ok(lock >= 0 && complete >= 0 && body >= 0 && unlock >= 0, 'missing lock/complete/body/unlock');
  assert.ok(text.includes("hashtext('${LOCK_KEY}')") || text.includes("hashtext('vitan-pmc-destructive-seed')"));
  assert.ok(lock < complete, 'completeness must be observed after the lock');
  assert.ok(complete < body, 'skip/rebuild decision must precede the destructive body');
  assert.match(text, /connection_limit=1/);
  assert.match(text, /pg_backend_pid/);
  assert.match(text, /STARTER_TEMPLATE\.name/);
  assert.doesNotMatch(text, /cloud-agent-seed-complete/);
  assert.doesNotMatch(start, /AuditLog/);
  assert.match(start, /pnpm --filter api seed/);
});
