...
test('C1: a recorded claim is still checked against current main', () => {
  // `main` moves while CI runs and the review waits, so being built on it is not
  // settled once at admission — the last assessment before merge is the one that
  // matters. The recorded-claim path returned before this check, so a claimant
  // that fell behind during the wait merged anyway.
  const source = pr(354);
  const claimant = pr(360, { state: 'open', replaces: 354 });
  const recorded = [exhausted(source, [360])];

  const behind = lineage(claimant, recorded, [source], false);
  assert.equal(behind.allowed, false);
  assert.match(behind.detail, /built on current `main`; merge `main` into this branch/u);

  const current = lineage(claimant, recorded, [source], true);
  assert.equal(current.allowed, true, current.detail ?? '');
  assert.equal(current.claimFor, null, 'still nothing more to record');
});

test('C2: a unit holding two claims discharges neither', () => {
  // Two runs can record claims for one unit against DIFFERENT sources: one
  // re-reads it declaring #350, the declaration changes to #354, and a second
  // head-specific run reads no claim yet and records #354. Reading only the
  // first claim would admit the unit on whichever source was enumerated first,
  // and merging it would appear to settle both.
  const first = pr(350);
  const second = pr(354);
  const overloaded = pr(360, { state: 'open', replaces: 354 });
  const both = [exhausted(first, [360]), exhausted(second, [360])];

  const result = lineage(overloaded, both, [first, second]);
  assert.equal(result.allowed, false);
  assert.match(result.detail, /holds claims on #350 and #354/u);
  assert.match(result.detail, /one unit carries one obligation/u);

  // And fresh work stays blocked: neither obligation is settled by that merge.
  const fresh = pr(400, { state: 'open' });
  const merged = { ...overloaded, state: 'closed', merged_at: at(600) };
  const blocked = lineage(fresh, both, [first, second, merged]);
  assert.equal(blocked.allowed, false);
  assert.match(blocked.detail, /still requires a replacement/u);
});

test('C3: claims recorded in the same second keep the timeline order', () => {
  // GitHub timestamps label events to the second, so two claims recorded in the
  // same second are ordered only by the timeline. Breaking the tie on the
  // claimant's NUMBER instead would reject whichever was actually first
  // whenever the higher-numbered unit got there.
  const source = pr(354);
  const higher = pr(361, { state: 'open', replaces: 354 });
  const lower = pr(360, { state: 'open', replaces: 354 });
  // #361's claim is recorded FIRST, in the same second as #360's.
  const raced = exhausted(source, [
    { claimant: 361, recordedAt: at(500) },
    { claimant: 360, recordedAt: at(500) },
  ]);
  const all = [source, higher, lower];

  const winner = lineage(higher, [raced], all);
  assert.equal(winner.allowed, true, winner.detail ?? '');

  const loser = lineage(lower, [raced], all);
  assert.equal(loser.allowed, false);
