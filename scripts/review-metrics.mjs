...
export function aggregate(snapshot) {
  ...
  for (const pr of merged) {
    ...
    // a reaction binds no SHA; a review does. Every signal is read as of the merge: a comment,
    // review or reaction that landed after `merged_at` was not evidence available to it
    const cleanReactions = (snapshot.reactions[pr.number] ?? []).filter((r) => isCodex(r) && r.content === '+1' && before(r.created_at)).length;
    if (comments.length === 0 && reviews.length === 0 && cleanReactions > 0) missingEvidence += 1;
...
export function render(report) {
  ...
  return [
    ...
    '- **Missing evidence**: a merged PR whose only Codex signal as of `merged_at` is a thumbs-up reaction, which binds no SHA; comments, reviews and reactions that landed after the merge are not evidence for it; nothing is guessed.',
