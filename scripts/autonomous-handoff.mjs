...
export async function handOffCorrectionLease(
  ...
  // A failure the GATE recovers from is DISPATCHED, not reported.
  // `auto-merge.yml` recovers only through `workflow_dispatch` and the timeout
  // path publishes its failure and throws, so no later CI event arrives —
  // excluding these from the lease left the pull request draft until a human
  // noticed. This workflow already holds `actions: write` and already dispatches
  // itself, so the watchdog asks for the recovery rather than describing it.
  // `gateRecoveryStatus` holds the ask while the gate's own accepted request is
  // still pending.
