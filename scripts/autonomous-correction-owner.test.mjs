...
test('owner resolution is applied on top of git-faithful parsing', () => {
  ...
  // admitted owner is `invalid` (not routed). `codex` is now an admitted CANDIDATE owner (merge-admission
  // unit), so its trailer resolves `declared`; merge-eligibility is enforced separately by the gate.
