...
if seed_permitted; then
  # Completeness is decided INSIDE seed.ts under the advisory lock. Do not
  # pre-query a sentinel from this shell — that observation races with seed.
  echo "[cloud-agent-start] Ensuring seed (skip-or-rebuild is serialized in prisma/seed.ts)"
