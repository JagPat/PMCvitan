...
async function acceptedDeferral(client, pullRequest, expectedHead, evidence) {
  try {
    ...
    const convergence = assessConvergence({
      ...
      changedFiles: commit.files ?? [],
      pullRequestFiles,
      // The SAME evidence view the cap was measured against.
      //
      // `assessConvergence` otherwise counts finding heads from its own live read,
      // and a partial one makes a docs-only unit look below the deferral threshold
      // — so `deferralRequired` comes back false, this returns false, and the unit
      // is forced into replacement despite holding a valid handoff. That is the
      // exact partial-read condition the durable floor exists to survive, so the
      // deferral question has to be asked against the floor too, not beside it.
...
export async function enforceRestructure(
  client,
  pullRequest,
  expectedHead,
  // The caller may already KNOW this head bears findings — it just classified
  // them. Passing that in beats re-reading the API to rediscover it, because the
  // head a partial read loses is precisely the one that crosses the cap.
  ...
  let result = assessRestructure(inputs);
  ...
  if (result.state === 'restructure_required'
    ...
    result = assessRestructure({ ...inputs, deferralInForce: true });
  }
