...
const snapshot = (extra = {}) => ({
  ...
  pulls: [
    ...
    { number: 7, state: 'closed', merged_at: at(11), created_at: at(2) }, // its only reaction landed AFTER the merge: not evidence, not "missing evidence" either
  ],
  ...
  reactions: { 1: [], 3: [{ user: CODEX, content: '+1', created_at: at(6) }], 6: [{ user: CODEX, content: '+1', created_at: at(9) }], 7: [{ user: CODEX, content: '+1', created_at: at(12) }] },
  jobs: { 1: [job(1, at(8, 12), at(8, 13)), job(1, at(8, 12), at(8, 13)), job(2, at(8, 14), at(8, 15))], 3: [], 6: [], 7: [] },
  ...extra,
});
...
test('week bounds are UTC and end-exclusive; the merged cohort respects both boundaries and open PRs are backlog', () => {
  ...
  assert.throws(() => weekBounds('2026-02-30'), /YYYY-MM-DD/u, 'an impossible date is refused, not normalised to March');
  const report = aggregate(snapshot());
  assert.deepEqual(report.pulls.map((p) => p.number), [1, 3, 6, 7]);
...
test('clean-only evidence is reported as missing evidence, zero merges divide nothing, and missing job data is counted', () => {
  const report = aggregate(snapshot());
  assert.equal(report.missingEvidence, 1, 'PR 3 only: PR 6 has a SHA-bound review, PR 7 has nothing as of its merge');
  // the same reaction BEFORE the merge is the missing-evidence shape
  assert.equal(aggregate(snapshot({ reactions: { 3: [], 6: [], 7: [{ user: CODEX, content: '+1', created_at: at(10, 23) }] } })).missingEvidence, 1);
  assert.match(render(report), /reactions that landed after the merge are not evidence/u);
  assert.equal(report.medianFirstFindingToMergeHours, 24);
  assert.equal(report.ciHoursPerMerge, 2 / 4);
  assert.equal(aggregate(snapshot({ jobs: { 1: undefined, 3: [], 6: [], 7: [] } })).jobsMissing, 1);
...
test('collection fails visibly on a rate limit or an incomplete page instead of writing a partial snapshot', async () => {
  ...
  const calls = [];
  // a dormant open PR (last updated before the week) is backlog: it comes from the OPEN listing, which has no week cutoff
  ...
  const pages = [];
  const early = async (url) => { pages.push(url); return { ok: true, status: 200, json: async () => (url.includes('state=closed') ? full : []) }; };
  await collect({ week: '2026-09-07', repository: 'o/r', token: 't', fetchImpl: early });
  assert.equal(pages.filter((url) => url.includes('state=closed')).length, 1, 'the merged listing stops at the week; the open listing is separate');
});
