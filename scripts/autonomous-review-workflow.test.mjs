...
function automatedMergeEvidence(pullRequest, owner = 'claude') {
  return {
    ...
    // (readable, non-codex, body-consistent) head bound to the reviewed SHA so promotion is not held.
...
test('a buried clean verdict cannot promote a draft without a fresh polled review', async () => {
  ...
  const client = {
    ...
    async commit(sha) {
      ...
      // promotion/final-admission ownership hold passes and the recovered-clean verdict merges as intended.
...
test('a base retargeted INSIDE the setDraft window is refused on the post-mutation object', async () => {
  ...
  const client = {
    ...
    // Eligible claude head so the promotion guard passes and the retarget is the only refusal.
    async commit(sha) { return { sha, commit: { message: 'chore: unit\n\nCorrection-Owner: claude\n' } }; },
    async setDraft(current, draft) {
      setDraftCalls += 1;
      return {
