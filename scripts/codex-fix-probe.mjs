 ...
 * ordinary `GITHUB_TOKEN` for all reads and complete final authorization, then uses the dedicated
 * `CODEX_WAKE_GITHUB_TOKEN` only for the final PR-comment POST. A missing or reused credential fails
 * closed. It writes nothing but one PR comment: no AI credential, no required-status write, no branch update, no merge. Two safety
...
export function codexFixComment({ pullRequestNumber, headSha, sourceBranch, findingRef, marker, originRunUrl, runAttempt }) {
  return [
    marker,
    '@codex fix',
    '',
    ...
    '',
    `This automated GitHub Actions request was posted through the repository owner's consenting account from ${originRunUrl} (attempt ${runAttempt}).`,
    'It is dispatch evidence only — no review clearance is implied, and the shadow review remains non-authoritative.',
  ].join('\n');
}
 ...
 */
export async function runDispatch({ env, loadEvent, readApi, writeApi, writeOutput = () => {}, onBeforePost = async () => {} }) {
  ...
  const livePull = await readApi.getPull(repository, pullRequestNumber);
  const findingComment = await readApi.getReviewComment(repository, findingCommentId);
  const threadUnresolved = await readApi.threadUnresolved(repository, pullRequestNumber, findingCommentId);
  const findingRef = findingComment?.html_url ?? `comment-${findingCommentId}`;
  const existingComments = await readApi.listComments(repository, pullRequestNumber);
  ...
  const freshPull = await readApi.getPull(repository, pullRequestNumber);
  const freshComment = await readApi.getReviewComment(repository, findingCommentId);
  const freshThreadUnresolved = await readApi.threadUnresolved(repository, pullRequestNumber, findingCommentId);
  const freshFindingRef = freshComment?.html_url ?? `comment-${findingCommentId}`;
  const freshComments = await readApi.listComments(repository, pullRequestNumber);
  ...
  const created = await writeApi.postComment(repository, pullRequestNumber, codexFixComment({
    pullRequestNumber,
    headSha,
    sourceBranch: reauthorized.sourceBranch,
    findingRef: freshFindingRef,
    marker: reauthorized.marker,
    originRunUrl: `${env.GITHUB_SERVER_URL}/${repository}/actions/runs/${env.GITHUB_RUN_ID}`,
    runAttempt: env.GITHUB_RUN_ATTEMPT,
  }));
  const evidence = { commentId: created.id, commentAuthor: created.user?.login, commentUrl: created.html_url };
  if (!Number.isInteger(evidence.commentId) || !evidence.commentAuthor || !evidence.commentUrl) {
    throw new Error('posted_comment_evidence_missing');
  }
  writeOutput(`comment_id=${evidence.commentId}\ncomment_author=${evidence.commentAuthor}\ncomment_url=${evidence.commentUrl}\n`);
  return evidence;
}

export function requireProbeTokens(env) {
  if (!env.GITHUB_TOKEN) throw new Error('GITHUB_TOKEN is required');
  if (!env.CODEX_WAKE_GITHUB_TOKEN) {
    throw new Error('CODEX_WAKE_GITHUB_TOKEN is required; fallback is forbidden');
  }
  if (env.GITHUB_TOKEN === env.CODEX_WAKE_GITHUB_TOKEN) {
    throw new Error('read and comment credentials must be distinct');
  }
  return { readToken: env.GITHUB_TOKEN, writeToken: env.CODEX_WAKE_GITHUB_TOKEN };
}

async function main() {
  const [mode] = process.argv.slice(2);
  if (mode !== 'dispatch') throw new Error('Expected dispatch mode');
  const { readToken, writeToken } = requireProbeTokens(process.env);
  const evidence = await runDispatch({
    env: process.env,
    loadEvent: () => JSON.parse(readFileSync(process.env.GITHUB_EVENT_PATH, 'utf8')),
    readApi: productionApi(readToken),
    writeApi: productionApi(writeToken),
