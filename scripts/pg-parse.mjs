...
// `relationsIn` USED TO LIVE HERE, AND IT IS DEFERRED WITH THE RULES IT SERVES.
//
// It answered "which relations does this query name" by lowercasing `RangeVar.relname` and dropping
// `schemaname` — so `pg_trigger`, `public.pg_trigger` and `"PG_TRIGGER"` all came back as
// `pg_trigger`, though PostgreSQL treats them as three different objects (it preserves the case of a
// quoted identifier, and a schema qualifies the name). A future invariant asking whether a guard
// reads `pg_trigger` would then accept evidence gathered from a same-named USER relation: a check
// narrower than the object it judges, which is the defect this line of work exists to detect.
//
// Getting it right needs a decision this unit has no standing to make — what counts as "the catalog
// relation", how the search path bears on it, what a rule may conclude from an unqualified name — so
// it belongs to the rule that needs it, together with the rest of the deferred rule support. The
// binding's own consumers ask `nodesOfType(tree, 'RangeVar')` and read the identity fields whole.
