...
test('dedicated comment credential is mandatory and never falls back to the read token', () => {
  assert.throws(() => requireProbeTokens({ GITHUB_TOKEN: 'ordinary' }), /fallback is forbidden/u);
  assert.throws(() => requireProbeTokens({ GITHUB_TOKEN: 'same', CODEX_WAKE_GITHUB_TOKEN: 'same' }), /must be distinct/u);
  assert.deepEqual(requireProbeTokens({ GITHUB_TOKEN: 'ordinary', CODEX_WAKE_GITHUB_TOKEN: 'dedicated' }), { readToken: 'ordinary', writeToken: 'dedicated' });
});
...
test('the posted request is a single @codex fix naming head/branch/finding, requiring a stale-head recheck, preserving other findings', () => {
  ...
  const body = codexFixComment({ pullRequestNumber: 597, headSha, sourceBranch: 'codex/feature', findingRef, marker, originRunUrl: 'https://github.com/JagPat/PMCvitan/actions/runs/42', runAttempt: '2' });
  ...
  assert.match(body, /automated GitHub Actions request/u);
  assert.match(body, /actions\/runs\/42.*attempt 2/u);
  assert.doesNotMatch(body, /github-actions\[bot\]|Generated by Claude/u);
});
...
test('runDispatch refuses an untrusted ref and a non-dispatch event before any API call', async () => {
  ...
  const baseEnv = { GITHUB_EVENT_NAME: 'workflow_dispatch', GITHUB_REF: 'refs/heads/main', GITHUB_SERVER_URL: 'https://github.com', GITHUB_RUN_ID: '42', GITHUB_RUN_ATTEMPT: '1', PROBE_PR_NUMBER: '597', PROBE_HEAD_SHA: headSha, PROBE_FINDING_COMMENT_ID: '4023550608', PROBE_AUTHORIZATION: expectedAuthorization({ pullRequest: 597, headSha }) };
  await assert.rejects(runDispatch({ env: { ...baseEnv, GITHUB_REF: 'refs/heads/codex/feature' }, loadEvent: () => ({ repository: { full_name: repository } }), readApi: api, writeApi: api }), /untrusted_ref/u);
  await assert.rejects(runDispatch({ env: { ...baseEnv, GITHUB_EVENT_NAME: 'push' }, loadEvent: () => ({ repository: { full_name: repository } }), readApi: api, writeApi: api }), /not_workflow_dispatch/u);
});
...
test('overlapping dispatches post once: the second dedups at the pre-post recheck (barrier on the real seam)', async () => {
  ...
  const makeApi = () => ({
    ...
    postComment: async (_repo, _n, body) => {
      const created = { id: comments.length + 1, user: { login: 'JagPat' }, html_url: `https://github.com/JagPat/PMCvitan/pull/597#issuecomment-${comments.length + 1}`, body };
      comments.push(created);
      return created;
    },
  });
  const env = { GITHUB_EVENT_NAME: 'workflow_dispatch', GITHUB_REF: 'refs/heads/main', GITHUB_SERVER_URL: 'https://github.com', GITHUB_RUN_ID: '42', GITHUB_RUN_ATTEMPT: '1', PROBE_PR_NUMBER: '597', PROBE_HEAD_SHA: headSha, PROBE_FINDING_COMMENT_ID: String(commentId), PROBE_AUTHORIZATION: expectedAuthorization({ pullRequest: 597, headSha }) };
  ...
  const bRun = runDispatch({ env, loadEvent, readApi: makeApi(), writeApi: makeApi(), onBeforePost: async () => { bAtBarrier(); await barrier; } });

  await bReached;                                     // B has authorized (saw no marker) and is paused
  const aEvidence = await runDispatch({ env, loadEvent, readApi: makeApi(), writeApi: makeApi() }); // A authorizes (empty) and posts
  releaseB();                                         // release B: its recheck now sees A's comment

  assert.equal(aEvidence.commentAuthor, 'JagPat');
  assert.match(aEvidence.commentUrl, /issuecomment-/u);
...
test('the pre-post recheck reruns the full authorizer: a mid-window resolve, retarget or withdrawal aborts with no post', async () => {
  ...
  for (const [label, mutate] of [
    ...
    const api = {
      ...
      postComment: async (_repo, _n, body) => { const created = { id: comments.length + 1, user: { login: 'JagPat' }, html_url: `https://github.com/JagPat/PMCvitan/pull/597#issuecomment-${comments.length + 1}`, body }; comments.push(created); return created; },
    };
    await assert.rejects(
      runDispatch({ env, loadEvent, readApi: api, writeApi: api, onBeforePost: async () => { mutate(state); } }),
      /stale_before_post/u,
      label,
    );
...
test('the probe workflow declares a non-cancelling per-PR concurrency group, the main-ref guard and minimal permissions', () => {
  ...
  assert.ok(workflow.includes('CODEX_WAKE_GITHUB_TOKEN: ${{ secrets.CODEX_WAKE_GITHUB_TOKEN }}'));
