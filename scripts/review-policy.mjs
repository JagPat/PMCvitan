...
// declare `codex` and be tracked as an in-flight unit. It is NOT awakenable from GitHub
// (the implementation task and reviewer share one bot identity, and a candidate stays held
// pending independent reviewer activation), so admission here is not wake authority — and it
// is not merge authority either: the pre-publication ownership hold refuses to promote a
// Codex-owned head. See AWAKENABLE_FROM_GITHUB, headOwnerEligibility, and docs/POLICY.md.
...
// A retryable INFRASTRUCTURE failure, not an ownership fault: the exact HEAD commit could not
// be read (or the git-faithful trailer primitive could not run), so the trailer's validity is
// unknown and no correction is owed. It joins the retryable set below so the watchdog
// re-dispatches the gate (which re-reads the commit) rather than stranding a pending status.
export const OWNERSHIP_READ_RETRY = 'validation: head commit ownership temporarily unreadable — retrying';
// A READABLE ownership fault: the exact head's Correction-Owner trailer is missing, invalid,
// names the validation-only `codex` candidate, or disagrees with the PR body marker. The
// pre-publication hold writes it as the leading text of a `scope:` failure detail so an
// ineligible head is never promoted to ready, armed for auto-merge, or given a green required
// status. It is NOT retryable — a new head with a single agreeing trailer (or a body edit,
// when the trailer is already valid) is owed, not a re-dispatch.
...
// The two remedies for a readable ownership fault, produced by the hold and read back by the
// watchdog (which sees only the persisted detail). A VALID trailer with only a missing/mismatched
// BODY marker is body-edit recoverable; a missing/invalid/disagreeing TRAILER needs a new head.
// Both lead with the signature above, and the `body` variant carries the fixed `is valid` phrase
// inside GitHub's 140-char truncation so the watchdog tells them apart without a second read.
