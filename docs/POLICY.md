...
`<!-- correction-owner: codex -->`. Branch names never authorize ownership (a
`claude/**` branch may declare another owner; a `codex/**` branch is ordinary). The
marker selects an agent type, not a session: coordinate one producer per branch. Only
...
provenance, permissions and acceptance of watchdog mentions must be verified first.
...
no session is running. Codex is admitted as a candidate but is not awakenable, so
a Codex-owned correction — and any other non-awakenable or unresolvable owner
declaration — reports `correction_stalled` with the exact corrective action.
