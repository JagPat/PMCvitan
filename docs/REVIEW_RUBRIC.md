...
| migration immutability / replay | Are deployed bytes unchanged (POLICY: deployed migrations are immutable)? Does the new migration apply twice? | bytes: the `review-scope` job runs the BASE commit's `scripts/migration-manifest.mjs verify` (a protected file's changed bytes, a removed, redefined or phantom entry, an unrecorded new migration each fail); the reviewer reads `git diff --name-only <base>...HEAD -- apps/api/prisma/migrations/` and expects only NEW directories; replay only: `rerunTwice` |
