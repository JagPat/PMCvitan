...
| lock order / concurrency | Is the guard's lock taken before its read? Show the interleaving. | `lockOrderProbe` (a real lock wait, never a sleep; a competing writer raced into the guard's read-to-mutation window; a holder, contender, inspection, release or abort that throws fails the probe; the suite's `abort` frees the row whatever the release did) |
