...
| **3** | ✅ **ProjectTemplate** presets (`GET/POST/DELETE /orgs/:id/templates`) + **save-project-as-template**. A preset is a named module selection ("G+2 Residence"); picking one at Create Project **expands through the same module instantiation** as an à-la-carte pick (and unions with explicit picks + `structureFrom`). "Save as template" (in the Locations editor) captures the project's **full structure as ONE new module** wrapped in a preset — a single coherent module keeps activity/checklist place references intact; richer multi-module presets are hand-composed via explicit `items`. UI: the New-project **Start from** select groups **Templates** and **Copy structure from a project**; the preset note lists its modules. "Save selection as a module" (zone bookmark) shipped in Slice 2. | the reusable library | medium |
| **4** | ✅ **Seed Vitan's starter library + align the API seed with the demo spine.** A single source (`apps/api/src/domain/seed-data.ts`) now feeds both prisma scripts: the full demo location tree (incl. the draft Basement branch), decisions/activities/inspections/materials **with their `nodeId` attachments**, and the **starter library** — 7 modules (Ground/Upper/Terrace zones, Kitchen + Bedroom spaces, Standard Residential Phases, Standard QA) wrapped in a **"G+2 Residence"** preset, created org-scoped and **only when the org has no modules** (live-safe). `seed.ts` was rewritten around it (complete wipe list; drafts get `authorId`+null `publishedAt` — also fixing a latent bug where seeded decisions were never published); `ensure-accounts.ts` backfills the spine + attachments (create-only) and the library on prod. A drift test (`seed-data.test.ts`) pins the tree against the shared demo seed and validates every starter payload against the module contract | never-blank projects; closes the data-flow audit's #1 finding | small |
