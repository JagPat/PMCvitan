...
| 6B-i | §F derivation over the three folds, wired into every EXISTING fold-mover | **PROPOSED SPLIT, scoped before implementation.** `deriveBillStatus` — the §F first-match truth table over `NET_PAYABLE`/`APPROVED`/`PAID` — plus the re-derivation call at every writer that can move any of the three: `payment.approve`, `payment.record`, `deduction.record`, `deductions.release`, `certificate.certify`, `certificate.supersede`. **All three folds already exist** (`NET_PAYABLE` from 5C, `APPROVED`/`PAID` from 6A), so this needs no new fact and no new table, and it is strictly STRICTER than the finished rule: with no reversal yet, no status can move backwards. Estimated ~700–900 lines |
| 6B-ii | Payment reversals | **PROPOSED SPLIT.** The append-only `PaymentReversal` fact (strictly positive, provenance-bound, append-only trigger), the bound `Σ reversals ≤ Σ payments`, `payments.reverse` under the bill lock RE-USING 6B-i's derivation as a CAS transition, and the concurrency the plan names (concurrent reversals over-release `PAID` by the same shape two concurrent payments break bound 5). Estimated ~800–1,000 lines |
