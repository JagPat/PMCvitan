...
| 5A | §E three-way verification + the `verified` arrow | in_progress — draft PR #276. Ships the derived triple (ordered · accepted/measured · billed), the pro-rata tax/freight cap, the six exception kinds, and the ONE arrow the verdict makes safe. No certificate, no deductions, no SoD — each needs evidence this increment deliberately does not invent |
| 5B | Certification + frozen consumption sets + §G bound 3 + §I SoD + the certificate-refusal arms | not_started |
