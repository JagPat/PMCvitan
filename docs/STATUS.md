...
| 3 | `Measurement` (§D) + the `revertSignOff` withdrawal guard | in_review — draft PR #272 from `main` `96b0713`; §D carried forward verbatim, the measurement fact + both withdrawal guards + the COMMITTED labour consumption term in, probes 19/19. THREE Codex rounds returned ten findings, all fixed forward reproduce-first; convergence audit `docs/reviews/pr-272-convergence.md` names three roots. **STOP** = a narrow review AFTER this task, before any bill can consume a measurement |
