...
at the same limit and was claimed by #412; #412 then closed at the same limit and is
claimed by the open PR, which carries `Replaces: #412`. Neither #410 nor #411 is
...
#410, #411 and #412 — the last three added when each closed at the two-finding-head
limit. #410 was claimed and settled by #411's admission and #411 by #412's; #412 is
claimed by the open PR, which carries `Replaces: #412`. (#381 carries the label but is settled by merged #382, #394 by merged #395,
...
**ROUND 2 ON #412 RETURNED TWO MORE P1s AND A P2, THE UNIT HIT THE TWO-FINDING-HEAD
LIMIT, AND THE OPEN PR REPLACES IT — NOT AS A MECHANICAL CARRY-OVER, BUT AS THE
STRUCTURAL CHANGE THE SIXTH ROUND FINALLY MADE UNAVOIDABLE.** The decisive fact is
WHERE the second P1 landed: inside the deploy-time verifier #412 had just added to
close round 1, checking CHECK constraints by `conname` and `convalidated` only — the
exact by-name defect the migration itself had been fixed to avoid two rounds earlier.

Six rounds had gone by name → by definition → by function body → by column contract →
by persistence and volatility → by enforcement state → and then by name again, in a
second place. That is not carelessness about which attribute to check. **Every check
in the unit was written as a LIST, and a list is always potentially shorter than the
set of things that can go wrong.** Extending it once more buys round seven.

So the replacement stops choosing attributes. Section 9 reads EVERY object of every
kind attached to the table out of the catalog and compares the full actual set against
the full expected set with a FULL OUTER JOIN, refusing on a difference in EITHER
direction — MISSING, UNEXPECTED, or same-name-different-definition. Fifty-five expected
rows: 11 columns, 10 constraints, 4 indexes, 5 triggers, 20 internal foreign-key
enforcement rows, 5 functions. The UNEXPECTED leg has no equivalent in #412 at all — an
extra index, CHECK, trigger or key attached to this table was invisible to every check
in the file.

**Section 1's second copy of that comparison is gone (606 → 82 lines).** It used to
enumerate the columns, the ten constraints, the five key targets, the twenty internal
triggers, the three indexes, the five functions and the five triggers so a foreign
table was refused before section 2 ran — and that duplicate is precisely how the
by-name defect survived in one of the two. Section 1 now decides only what must be
decided before the table is read (its relation kind), the populated-and-incomplete
refusal, and the row verdict it has always deferred; the comparison happens once, and
the write barrier is lifted only after it passes. Two exceptions are named IN the file
with their reasons: the install barrier, whose expected state differs between the two
callers, and the function bodies, compared against the single `$body$` literals rather
than a second copy of them.

**Roughly 1,150 lines of prose arguing which catalog attributes were CHECKED, COVERED
BY, or EXCLUDED are gone**, because set-equality stops excluding anything. What
survives as operator-facing text moved to `docs/RUNBOOK.md` §B1. `migration.sql` is
2,236 → 1,594 lines. The unit is 6,029 → 5,699 changed lines; the smaller net is
honest — the migration shed 642 lines while the suite GAINED four probes (P35–P38)
that #412 did not have, including the one that makes the single-inventory claim
checkable: it builds its own database, migrates it by the ordinary deploy path, and
asserts the inventory and the installed catalog differ in NO direction.
