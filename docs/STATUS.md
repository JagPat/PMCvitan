...
| 5B | Certification + frozen consumption sets + §G bound 3 + §I SoD + the certificate-refusal arms | not_started — NEXT. Note for its fixtures: the merged receipt seal (#277) means any direct `CommandExecution` write must reserve-then-complete IN ONE TRANSACTION with a non-blank `resultRef` |
