...
| 3 | `Measurement` (§D) + the `revertSignOff` withdrawal guard | in_progress — branch `claude/phase5-task3` from `main` `96b0713`; §D carried forward verbatim, the measurement fact + both withdrawal guards + the COMMITTED labour consumption term in, probes 9/9. **STOP** = a narrow review AFTER this task, before any bill can consume a measurement |
