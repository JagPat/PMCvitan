...
| 5C | §H deduction ledger + releases + the `NET_PAYABLE` floor | in_review (draft PR #284) — the last increment of Task 5. **SPLIT at round 5 on JagPat's explicit instruction:** five finding-bearing heads (the orchestrator's stated limit) with three of round 5's four findings landing on seals rounds 3 and 4 had just added, and two of those four being the same half-a-rule shape the convergence audit had named one head earlier — both inside the re-statement machinery. **Re-statement moves to its own follow-up review unit.** This PR honours §H's rule (a retained balance never vanishes without an attributable release) by REFUSING to supersede a certificate that still holds money — service refusal naming the row and balance, plus `phase5_t5c_supersede_needs_release` at COMMIT — which is STRICTLY STRICTER than re-statement and so satisfies the same split criterion that made 5B safe. Round 5's finding 1 DISSOLVES under it (the withheld fold is necessarily zero at supersession, so `claim` is the accurate mover); finding 3 is FIXED here (`resultRef` IS the row, both tables). Cost stated plainly: until the follow-up lands, correcting a certificate with money held requires releasing it first. §H travels VERBATIM from `claude/phase5-planning` `a4d469b`. It supplies the first REAL subtraction into the term unit C shipped: `certified-payable` is already defined as `NET_PAYABLE − APPROVED`, so deductions must subtract INTO that fold rather than add a parallel bucket. **The plan's post-Task-5 STOP applies when this merges**, with payment authority fully in place |
