...
**THE DOCS-ONLY 4c PLAN UNIT IS PR #479** (branch `claude/decision-workflow-4c-plan-r8`,
`Replaces: #478` — the seventh replacement) — the first review unit of
...
: state`), so `open_pr: 479` survives its own merge (falling back to `task: 4`) AND keeps
`detectStatusDrift` quiet while #479 is open. Leaving it `none` bought nothing and cost a
drift-shepherd cycle.

LINEAGE vs LEDGER — two different things, both true (review round 8). The WORK lineage is
#470 → #471 → #473 → #474 → #476 → #477 → #478 → #479: each closed at the two-finding-bearing-head
limit and the next carries the same unit forward. The LEDGER claim is a separate, executable fact: the
scope gate holds an obligation only while its PR carries the `review-replacement-required`
label. #478 IS labelled, so #479 declares `Replaces: #478` — its immediate exhausted
