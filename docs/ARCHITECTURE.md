...
- **A retried or duplicated command executes its effect exactly once and replays the same result.** A consequential command runs under an idempotency key through the kernel `executeCommand(prisma, …)` (`apps/api/src/platform/commands.ts`): it reserves a `CommandExecution` receipt, runs the canonical mutation + its audit + `DomainEvent`, and flips the receipt to `succeeded` — **all in ONE transaction**. A concurrent duplicate blocks on the receipt's scope-specific partial-unique lock; when the winner commits, the loser reads the committed result and replays it. The receipt is **actor-scoped**: the lookup keys on `scopeKind` + the matching scope id(s) + `actorId` + `commandType` + `idempotencyKey`, so a replay can never return one actor's result to another. Same key + same request replays; **same key + different request → 409**; a mutation that throws rolls the reservation back with it, so the key stays retryable. Scope-specific **partial unique indexes** (`WHERE scopeKind='project'` / `='org'`) mean an org-scoped key never constrains a project-scoped row; the tenant `organizationId` on a project-scoped receipt is DERIVED from the project (a composite `(organizationId, projectId) → Project(orgId, id)` FK, exactly like the event envelope). Rollout is additive: an absent key runs today's compare-and-set path unchanged (no regression) until missing-key enforcement is switched on behind a capability/version gate (`COMMAND_KEY_ENFORCED`, a Phase-2 completion criterion after old clients drain). The decision pillar is the first wired module (the web sends a stable key per command, carried on its offline outbox op so an online send and a later replay share one key); the rest of the command inventory migrates across Tasks 8–10. This is Phase 2 Task 5.
- **Upgrade evidence.** `apps/api/scripts/upgrade-proof.sh` (CI job `upgrade-proof`) rebuilds a pre-Phase-1 database from the real ledger, plants legacy shapes (pending change requests, revisions without projectId, counter-only items, zero-item closings, stored gate flags), applies all Phase 1 migrations the way Prisma does and asserts every legacy meaning survived — including that the additive Phase-2 event store and command ledger add their tables/constraints while writing no rows. Each migration is diagnostic-first and ABORTS on ambiguous legacy data rather than guessing.
