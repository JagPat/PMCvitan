...
**The first one identifies the DATABASE; the other two identify the DATASET, and that distinction
is the point.** A project id and a project count travel with the data: a clone or a
...
misconfiguration the guarantee is supposed to catch. `system_identifier` is generated by `initdb`,
lives in the control file rather than in any table, and `pg_dump` does not carry it, so a logical
restore into another cluster has a different one and is refused.

**The honest limit, so you are not surprised by it:** a *block-level* copy — a snapshot restore, a
filesystem clone, a physical replica — reproduces the control file too, so it carries the same
identifier and this check cannot distinguish it. Nothing readable from inside the database can.
What the check closes is the copy that ordinary tooling makes.
...
| `system-identity-mismatch` | the connected cluster is a **different** one — a clone, a restore, or the wrong `DATABASE_URL` | check `DATABASE_URL` first; only change the configured identifier when the deployment has genuinely moved clusters |
