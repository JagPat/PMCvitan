...
| `carries a constraint named "ActivityDependency_install_incomplete_check" that is NOT the install barrier this migration writes` | A same-named constraint that judges less than the real barrier — a hollow `CHECK (TRUE)`, or a `NOT VALID` one. That name is the only thing keeping a writer out of a half-installed graph, so a forgery means the table was open while this file believed it shut. Drop the incomplete table, or restore the barrier verbatim (`CHECK ("id" !~ '^')`), then re-run. |
| `present and validated but NOT ENFORCED: an internal trigger implementing them is disabled` | `ALTER TABLE … DISABLE TRIGGER ALL` — what a failed restore or a bulk load leaves behind — switches off the internal triggers that implement a foreign key without touching the catalog row, so the deparse and `convalidated` both still read correct. The migration will not re-enable them, because re-enabling validates nothing that arrived while they were off. `ALTER TABLE "ActivityDependency" ENABLE TRIGGER ALL`, reconcile any rows written meanwhile, then re-run. |
...
4. **Re-run the deploy** — `sh apps/api/scripts/migrate.sh` — and verify the guards are in force,
   not merely recorded. Expect `4|1|5|0|0`: four validated CHECKs, the partial unique index, five
   armed triggers, NO install barrier (a barrier still present means the install did not finish and
   the table is still unwritable), and NO unenforced containment key — the last column counts
   foreign keys whose internal enforcement triggers are disabled, which a validated-and-deparsed
   check cannot see.
   ...
   SELECT (SELECT COUNT(*) FROM pg_constraint
       ...
       || '|' || (SELECT COUNT(DISTINCT k.conname) FROM pg_constraint k
                    JOIN pg_trigger g ON g.tgconstraint = k.oid
                   WHERE k.conrelid = '"ActivityDependency"'::regclass
...
over a populated COMPLETE one — and CI runs that script in the required `api` job.
