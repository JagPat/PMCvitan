...
**If it fails:** the log names each trigger and what is wrong with it. Reinstall the seals by
re-running that migration's SQL against the database —
...
missing, treat that marker as unproven: it may have been forged or may predate the damage. Check
`OutboxOperatorAction` for the `projection.rebuild` invocation and `projection.rebuild.result` rows
that a genuine repair writes alongside it; if they are absent, the marker did not come from a
repair.
