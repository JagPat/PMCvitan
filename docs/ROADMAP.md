...
| **7c-auth channels (zero-DLT)** | Backend adds pluggable sign-in channels so the app can go fully live without DLT: **Telegram Gateway** phone-OTP (`SmsService` priority MSG91 → Telegram → stub — transparent to the existing phone flow), **email OTP** (`EmailService`, SMTP or stub, `/auth/email/*`), and **Google sign-in** (`GoogleAuthService`, `/auth/google`). All resolve via `signInOrProvision`. Frontend UI for email-OTP + Google is the next slice | ✅ Backend done |
