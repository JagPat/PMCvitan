...
| **7c-auth** | Real auth backend: email+password accounts (bcrypt), phone OTP (MSG91 v5, dev-stubbed with no provider), worker device tokens. `/auth/login`, `/auth/otp/request`, `/auth/otp/verify`, `/auth/worker/token`. Passwordless dev auth kept for the demo persona switch, gated by `ALLOW_DEV_AUTH`. Demo accounts seeded; RBAC hardened (only PMC/client see pending). Frontend OTP/login wiring is a follow-up slice | ✅ Backend done |
