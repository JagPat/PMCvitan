...
Four further P1 findings from the round-8 review of the recorded criteria are carried the
same way, by the same owner decision:

9. **Unit 2:** the equality rollout must survive OLD roster writers — an old instance's
   `formCrew` with an already-bound in-charge, or `addCrewMember` attaching an unbound
   worker to a bound crew, must not be rejected by the null-strict trigger: delay
   binding/enforcement until old roster writers are retired, or give those writers a
   database-compatible derivation path.
10. **Unit 2:** the backfill binds the ENTIRE connected active crew/worker component
    (in-charge edges included) — one worker may hold active memberships in multiple crews
    today, so binding one crew while a shared worker's other crew stays null aborts at the
    worker-side equality check; multi-crew memberships are resolved or bound together.
11. **Unit 1:** the structural source seal covers the labour source — the deployed
    `phase6_project_party_sourced()` constraint trigger (`20270801000000`) counts only
    `ProjectPartyCompanySource`/`ProjectPartyVendorSource`, so removing the last company
    source while a labour source remains would abort at the trigger even after the SERVICE
    count is extended: the same migration that adds the labour-source table extends that
    function and installs the corresponding constraint trigger on the new table.
12. **Unit 4:** `status='active'` is not the complete liveness test — requirement revision
    or cancellation does not release an allocation, and `recordWork` separately re-derives
