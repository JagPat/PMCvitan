...
**O2, staged as six units, in this order, and none starts as a side effect of another.**
The migration, the binding commands, and the enforcement are THREE separate units by the
repository's own mandatory seam — the attribution shape is purely additive and nullable, so
the old release serves against it untouched, and the nullable schema is itself the viable
forward-compatible boundary: the migration can land first and the commands follow before
enforcement, so no inseparable-unit justification exists and none is claimed:
...
0. **Fail closed now** (service only, no schema): the three writes are OPEN at the API
   ...
   construction, lifted per-command by the unit that makes the call safe (unit 3 for worked
   minutes and output, unit 5 for attendance — with unit 5's rollout sequencing below).
...
1. **The attribution shape** (additive migration ALONE, no service change): the
   ...
   party-snapshot columns on the three evidence tables — anchored on `ProjectParty` with a
   NEW labour justification source and its release protocol (§2 — removing a contractor
   company that still owns workers is a defined refusal-or-reassignment, decided through
   the participant channel, never an FK accident and never Orgs reading Labour tables).
   Nullable, additive, diagnostic-first, writing no rows. Two DB seals belong to this SAME
   migration or the shape is not what it claims: **the append-only triggers extend over the
   new columns** — `LabourAttendance`, `LabourWorkFact` and `ActivityWorkOutput` freeze
   their evidence by ENUMERATED column comparison (`phase4_t3_attendance_append_only` and
   siblings), so a party snapshot left out of that enumeration is silently mutable by any
   alternate writer, exactly the reattribution the snapshot exists to prevent; and **the
   snapshot is populated DB-SIDE at insert** — a BEFORE INSERT trigger derives the party
   snapshot from the worker's binding when the writer did not supply it, because during the
   mixed-version window the OLD release keeps writing pmc/engineer evidence with no
   knowledge of the column, and nullable DDL alone would let those rows commit
   snapshot-less and unrepairable after a roster change.
2. **The binding commands** (service only, schema untouched): the population path —
   ...
   membership↔party, usable as the backfill on existing projects, because references that
   nothing can write leave the later units vacuously green and the capture context empty.
   The invariants: **the binding is FROZEN once evidence relies on it** — the initial bind
   is explicit and attributable, a rebind is a CAS lifecycle (release + new bind, audited)
   refused while it would recast recorded evidence — and **one ownership authority** — the
   WORKER's party is authoritative, and party equality is enforced wherever a crew names a
   worker: membership create/change AND `Crew.inchargeWorkerId` at `formCrew` and any
   future in-charge change (the in-charge relation is separate from `CrewMembership` and
   today validates only project containment, `labour.service.ts:229` — an equality check
   that skips it leaves a party-A in-charge inside a party-B crew). Rebind's
   "no evidence relies on this binding" guard and unit 3's record-side guard must hold
   under the SAME locks (below), or each passes while the other commits.
3. **The ownership enforcement** (service only, schema untouched): re-derive "own" INSIDE
   ...
   **the record and rebind paths serialize on the same binding rows** — the record
   transaction takes `FOR UPDATE` on the membership binding and worker binding it derives
   authority from, the same rows `rebind` locks before its no-reliant-evidence guard, so a
   rebind cannot commit between the record's read and its insert (either the record sees
   the new party, or the rebind sees the new evidence and refuses — both orderings proven
   under the deterministic barrier); and **the output reference is slice-bound** —
...
4. **The own-scope read contract** (server): the narrow queries above, policy named for what
...
5. **The capture surface** (web + server contract): the minimal UI over those reads. The
   ...
   unit 5 lands, attendance capture is refused at the server (unit 0) and absent from the
   surface — worked minutes and output (units 1–4) do not wait for it.
...
This proposal starts none of the six. All are product scope beyond evaluate-and-propose;
...
units 1–3 is the multi-contractor forgery §2 describes, and an attendance path without
unit 5's device-authenticated proof is the citation-only evidence §1.4 refuses.
...
- Worker-device (`§H`) BINDING flows — anonymous onboarding and `orgs.workerDevice.bind` are
  untouched. (Unit 5 *uses* an already-bound device as attendance evidence — with the fresh
