...
the worker/crew side** (a nullable same-project party reference, the `Vendor`/`ProjectVendor`
reuse pattern §H already set), with membership↔party closing the loop.
...
**O2, staged as four units, in this order, and none starts as a side effect of another.**
...
1. **The attribution shape** (additive migration only): the project-contained party
   reference on `Worker`/`Crew` (§2) and the membership↔party link — nullable, additive,
   diagnostic-first, writing no rows; the old release is byte-identical in behaviour.
2. **The ownership enforcement** (service only, schema untouched): re-derive "own" INSIDE
   `recordWork`/`recordOutput`/`recordAttendance` transactions from the unit-1 relations, so
   a contractor token is refused on another party's allocation, activity or worker
   regardless of what any UI sends. Reproduce-first: the two-contractors probe (A submits
   B's ids) is RED today and refused after; an in-house (no-party) worker's records stay
...
4. **The capture surface** (web + server contract): the minimal UI over those reads. The
   ...
   accepts device evidence only in that form from a contractor token. `manualReason` stays a
   pmc exception. Until unit 4 lands, attendance capture simply is not offered on the
   surface — worked minutes and output (units 1-3) do not wait for it.
...
This proposal starts none of the four. All are product scope beyond evaluate-and-propose;
the attribution shape is a schema decision deserving its own review unit, and it comes
FIRST — the surface without units 1-2 is the multi-contractor forgery §2 describes, and an
attendance path without unit 4's device-authenticated proof is the citation-only evidence
§1.4 refuses.
