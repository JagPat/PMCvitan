...
single `@codex fix` comment through the dedicated consenting-account credential and records the
actual returned comment id, author, and URL as dispatch evidence. That is the only bot mention added, and it
...
## Validation-only successor stage
...
target tip, candidate head, and tested merge commit. The current target must also be the comparison
