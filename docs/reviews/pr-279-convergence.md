...
Six finding-bearing heads, twenty-eight findings — and, at the end, a decision to SPLIT rather than
push a seventh. Per `CLAUDE.md` this stops being another isolated
...
| `1089c18` | 4 | 4×P2 — **two of them on code round 5 had just added** |
...
## Round 6: the corrections themselves became the surface

Round 6's four findings are: standing read without `FOR UPDATE`; the append-closure read without
`FOR UPDATE`; §G bound 3 as `>` disagreeing with the new §J bucket; and the §I seal not binding the
exception to the certifying command.

**Two of the four are on code round 5 added.** The append-closure was round 5's answer to finding 21;
the §J bucket was round 5's answer to finding 24. Both were correct as far as they went and both
opened new surface, and that is the observation this audit ends on: past a certain size a unit stops
converging, because each correction adds review surface faster than review retires it. The round
counter is not a formality — it is measuring that.

The first two are also, once more, ONE class: *a predicate that reads without serializing.* Round 4
made exactly this correction to the quantity bound. Round 6 made it to the two predicates round 5
wrote. Question 2 of the three — **which WRITERS can break this invariant** — has a sibling that
this PR kept re-learning: **which readers can be overtaken by a writer while they decide.** A check
that reads a row it does not lock has not decided anything; it has guessed, and the guess is right
until it is not.

So the closure round 6 leaves is not another patch. It is the split: §I's override — eleven of the
twenty-eight findings — becomes its own review unit, the §J bucket becomes another, and this unit
keeps the certificate and its evidence. **The refusal ships with the certificate and the override
ships after it**, so no intermediate state is less strict than the finished rule.
