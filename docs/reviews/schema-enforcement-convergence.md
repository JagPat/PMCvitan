...
1. **A schema holding only non-relational objects** — a sequence, an index — now reads
   *applicable* where it previously read *not applicable*. It reports `enforcing: true` with no
   findings, so no deploy behaviour changes; but the boundary moved and this is where it moved to.
...
3. **Clause 1 now reaches cross-schema parents.** A disabled *user* trigger on a referenced table in
   ...
   but it is the change most likely to surprise.
