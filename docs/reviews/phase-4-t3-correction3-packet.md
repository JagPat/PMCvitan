...
## Round 3l — the current-head Codex review of `4ed5eab` (2 findings)

Two findings (P1 + P2), each reproduced RED at `4ed5eab` before the fix.

| # | finding (P) | fix |
|---|---|---|
| 1 | the migration healed `phase4_t3_attendance_append_only` in a LATER statement than the locked DIAGNOSE-AND-SEAL block. The file has no transaction wrapper (Prisma commits per statement), so on a database this migration explicitly accepts for healing — the trigger still bound to the 20270210 body, which never froze `manualReason` — there was a real window after the locked block committed (marker trigger + CHECK installed, `ACCESS EXCLUSIVE` lock released) and before the standalone heal committed, in which a direct writer could UPDATE a live row into the reserved marker with the revocation triple pre-set: the reserved-marker trigger fires only on INSERT, the marker CHECK passes because `revokedAt` is set, the stale body permits the `manualReason` rewrite, and no diagnostic runs again before `migrate.sh` verifies only seals and exits — a forged marker with NO repair evidence on a deploy that reports success (P1) | the canonical body — byte-for-byte `20270220000000`'s text, body lines at column 0 so `md5(prosrc)` is unchanged — is now asserted INSIDE the locked block, immediately after the two `LOCK TABLE` statements and BEFORE the evidence-table early-return, so the healed freeze and the marker seals become visible in ONE commit and the window does not exist; the standalone heal section is REMOVED (a pointer comment records why); and the POST-CONDITIONS block gains a body-md5 assert — the migration refuses to record itself applied unless the bound body is byte-canonical (`R3l-A`: regress to the 20270210 body, apply EXACTLY the statement prefix through the locked block, and the forge UPDATE is refused with the append-only message while the bound body md5 is already canonical — at `4ed5eab` the same prefix left the stale body live and the forge UPDATE committed, `promise resolved "1"`; the remainder then replays clean and `correctionSeals()` recovers. `R3l-A2` pins the post-condition md5 literal to the machine-extracted 20270220 layer body — it must appear at least twice in the file, the prerequisite VALUES row AND the post-condition — and pins the in-block heal's bytes to 20270220's; `t3c-canonical-fn-bodies.generated.ts` regenerated byte-identical, so every service-side seal is unchanged) |
| 2 | `assertRevokerEntitled` accepted ANY active project membership as `revokedById` (`OrgsParticipant.hasProjectStanding`), so a plan could name an active contractor/client/engineer on the attendance project — writing an immutable revocation + evidence record attributing the correction to someone who could not have performed it through the application, whose revoke path is `ROLE_POLICY['attendance.revoke']` = pmc-only (P2) | the participant method is REPLACED by `hasProjectRoleStanding(tx, projectId, userId, roles)` — an ACTIVE membership whose role is in `roles`, or (only when `roles` admits `'pmc'`) owner/admin of the project's org, mirroring `ProjectAccessService.authorize`'s super-admin semantics (that path operates a project AS pmc); bare standing is no longer offered. `assertRevokerEntitled` passes `ROLE_POLICY['attendance.revoke']` — the POLICY stays with its shared owner, the MEMBERSHIP facts stay with orgs — and the refusal message states the rule. RUNBOOK §P4T3C2/§P4T3C3 plan requirements updated to match (`R3l-B`: an active contractor on the very project is refused with `has no attendance-revoke standing` and NOTHING is written — row untouched, no evidence; an org ADMIN with no explicit membership is accepted, so the rule tightened exactly to the app's authority, not past it; at `4ed5eab` the contractor's repair committed, `promise resolved "{…}"`) |

**Honest notes.** (1) The `R3l-A` probe replays the reviewer's window DETERMINISTICALLY, not by
timing: it applies the exact statement prefix Prisma would have committed through the locked block
(split at the FINDING-3 banner) over the healing-eligible body state, then attempts the forge as a
direct writer. (2) The remaining pre-block window is by design: between the prerequisite block and
the locked block the marker CHECK does not yet exist, so a forge there produces a marked row the
locked block's own diagnostic then catches as an ABORT (`F1.marker`, no matching evidence) — the
correction closes the post-seal window, where NOTHING looked again. (3) The four pre-existing
`has no standing on project` test regexes AND the runner proof's Case-5 grep track the new message
text (the first full-battery run caught the shell grep still pinning the old string — 76/77, the
lone failure being exactly that stale pin); the probes' semantics (nonexistent user refused before
any write; cross-tenant user refused; org owner accepted) are unchanged.

**Gates (round 3l):** reproduce-first — `R3l-A` (the forge UPDATE committed in the heal gap),
`R3l-A2` (the canonical md5 appeared only once — no post-condition pin) and `R3l-B` (the
contractor's repair committed) all RED at `4ed5eab`, GREEN after. The focused
`phase4-t3-correction3.test.ts` **62/62**. `pnpm check` EXIT 0 (web 432/432, API **671/671**,
`check:automation` included). Full integration suite on a pristine migrated DB: **69 files / 652
tests**, EXIT 0. `upgrade-proof.sh` PASSED, EXIT 0. `phase4-t3-correction3-production-runner-proof.sh`
PASSED — **77 assertions**, EXIT 0 (its first run was 76/77: the Case-5 grep still pinned the old
refusal string — fixed, see honest note 3). `test:e2e:api:outbox` **25 passed / 6
sender-mode-skipped**, EXIT 0, first run. `test:e2e:api:allmodules` — reported EXACTLY as observed:
four runs on this session's container, each **30/31** with ONE failure ROVING across three
independently-DOCUMENTED timing-flaky browser specs (`daily-log-lost-response` in runs 1 and 3,
`daily-log-module-query` in run 2, the `project-scope` history test in run 4 — each recorded as a
known flake in earlier packets), every one of the 31 tests passing in ≥3 of the 4 runs and both
daily-log specs passing in run 4. The captured failure mechanism is environmental, not functional:
Playwright's page snapshot shows the app still on the For You screen — the nav click was swallowed
during vite-dev-server hydration, so the target screen never opened; no app assertion failed. The
ENTIRE web tree, e2e harness and seed are byte-for-byte identical to `4ed5eab`
(`git diff 4ed5eab -- apps/web scripts/test-api-e2e.sh apps/api/prisma/seed.ts` is empty), the head
on which CI ran this exact suite GREEN hours earlier — and this round's diff contains no web,
daily-log, snapshot or seed surface. After many hours of battery runs the container's hydration is
