...
and two on #391, five and five on #392, three and four on #393, and four and three
on #394. That is twelve units at two heads each, which is where the twenty-four comes
from and why the claim that every unit from #383 through #394 spent its whole
allowance is checkable rather than asserted. The rise from #383 onward was
...
Twenty-four finding-bearing heads across twelve units bought the table and its
consequences — #383 through #394, every one of them at two apiece — which is the
...
this document's own lineage is the demonstration: #383 through #394 each closed at
the round limit and each added one obligation as it closed — twelve units, twelve
...
4. **Concurrency resolves by recorded order, not by locking.** Label writes cannot
   ...
   **Admission needs the same treatment, and stating it only for settlement was the
   next missing half: the declared bundle and its admission are ONE immutable,
   retry-safe record, written before any member can win.** Recorded member by
   member, two evaluators disagree about what was declared: with A declaring
   `{#1,#2}` and B declaring `{#2,#3}`, one evaluator sees A's first member and a
   complete B, and admits B; after A's second member lands another evaluator reads
   the earlier record and authorizes A as well. Releasing the loser afterwards
   cannot retract a queued auto-merge, so both proceed. A partial declaration must
...
5. **And the cutover into it is a design problem in its own right**, whose hazards
   ...
   re-admitted; and **queued auto-merges must be cancelled or explicitly
   grandfathered before the fence moves** — draining runs does not reach them,
   because `enableAutoMerge` returns as soon as GitHub accepts the request and
   GitHub owns the merge from then on, so an old controller can queue a candidate
   and exit, the rollout sees no run to drain, the fence switches, and the
   old-policy candidate merges afterwards. This document records that mechanism
   earlier and the requirement did not carry it. Above all — **every input such a
   bootstrap could read is
...
- **Pending: #377, #378, #379, #383, #384, #385, #386, #387, #388, #389, #390, #391,
  #392, #393 and #394. Settled: #381, by merged #382** — which still carries the label,
