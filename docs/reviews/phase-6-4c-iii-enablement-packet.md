...
**Review lineage:** PR #503 → PR #505 → this unit, each closed unmerged at the review-round
limit. `Replaces: #505`.

## The round-3 review of head `6f8a036` — the transaction boundary, conceded

**The migration now carries its own `BEGIN`/`COMMIT`.** Two earlier heads argued it did not need
one, on the strength of a measurement: `CREATE TABLE x; SELECT 1/0;` under `prisma migrate deploy`
leaves no trace of the first statement, so the runner supplies a transaction. The measurement is
real and reproducible — and it is the wrong KIND of evidence. Prisma DOCUMENTS the opposite
guarantee (migrations are not automatically wrapped), so what was measured is an implementation
detail of one version. An indivisibility that rests on undocumented behaviour is lost silently at
the next upgrade, with no test failing.

What it guards is not cosmetic. Between dropping the reservation and installing the preservation
seal, a generic capability writer can insert a `consultation` row carrying a forged human
`enabledById`; the backfill's `ON CONFLICT DO NOTHING` then PRESERVES that forgery instead of
correcting it, and the new seal freezes it in place. The reviewer was right that the file must
state the boundary rather than inherit it.

Verified both ways under the real runner: an explicit-BEGIN migration applies and is recorded, and
one that raises mid-transaction leaves no object behind. Two other migrations in this repository
already ship an explicit `BEGIN`, so this is the existing convention rather than a new one. A probe
pins the boundary in the SHIPPED file, and the probes that replay the migration inside a
transaction they already opened now strip its transaction-control statements (`splitSqlForReplay`)
while executing every DDL and DML statement verbatim.
...
- Full integration battery — **102 files / 1391 tests, 0 failures, 0 skipped**, on a database
