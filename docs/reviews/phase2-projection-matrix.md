...
The consequence bundle each pillar mutation produces today (the set Tasks 3–6 reproduce via the DomainEvent envelope + per-consumer outbox). **Actor column** distinguishes `resolveActor` (real `actorId`, `common/actor.ts:21`) from a **bare** `actor: user.role` (null `actorId`) — the attribution split **Task 3 has now closed** (see the closure note under the table; the "bare role" rows below are the pre-Task-3 baseline this packet captured). **Every branch is asserted in its own isolated test with a fresh capture window** in `phase2-consequences.test.ts` (**42 tests**, real socket + push capture): the exact canonical write, audit, event, notification, socket payload `{ projectId }`, push body+roles, the no-side-effect facts, and rollback.
...
**Attribution split (the finding — now CLOSED by Task 3):** as captured at the Task-1 baseline, `decisions.*`, `inspections.*`, `drawings.*`, `activities.{complete,override,revokeOverride}` used `resolveActor` (real `actorId`), while `activities.{create,update,remove,start}`, `phases.*`, and all of `daily-log.*` wrote a **bare** `actor: user.role` with null `actorId`; `nodes.*` and `media.*` wrote **no audit row at all**.

**Task 3 closure.** All ~29 audit writes across the six pillar services now route through one canonical writer, `recordAudit(db, entry)` in `apps/api/src/platform/audit.ts` — the former bare-role sites (`activities.{create,update,remove,start}`, `phases.{create,remove}`, `daily-log.{start,addMaterial,submit,flagMismatch}`) gained a resolved `Actor` via `resolveActor`, so every row carries real `actorId`/`actorName`/`actorRole`. `actorKind` (`'human'` from `resolveActor`, `'system'` from `systemActor`) is resolved at the source so Task 4's `DomainEvent` and its audit row agree on who acted; it is **not** yet persisted on `AuditLog` (no schema change beyond Phase 1 — the column lands with the event envelope in Task 4). `platform/audit.test.ts` is a source tripwire that enforces `platform/audit.ts` as the ONLY file writing `auditLog.create`. The "bare role" annotations in the table above are retained as the pre-Task-3 baseline. Two gaps remain, out of Task 3's scope: `nodes.*`/`media.*` still record no audit row (Task 4's event catalog covers them as an event-coverage matter, not an attribution-convention one), and there is still **no audit READ API** — audit remains write-only.
