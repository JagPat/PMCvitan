...
## Round 13 — the five Codex findings on head `a58e949` (1×P1 REFUTED by execution, 4×P2), one batched head

The fourth consecutive in-window verdict. Each finding verified against the seal network and
the live database FIRST: the P1 dissolved under execution, the four P2s are real and fixed
reproduce-first (RED 5/5 at `a58e949`).

| # | finding | fix |
|---|---|---|
| R13-F1 (P1, **REFUTED with evidence**) | claim: the touch trigger's unconditional `COALESCE(OLD."decisionId", NEW."decisionId")` "will raise on every DecisionOption insert" and "touch an unavailable NEW record on deletes" | refuted by EXECUTION on PostgreSQL 16.13: since PostgreSQL 11 the plpgsql `OLD`/`NEW` variables are NULL RECORDS when not applicable (the current plpgsql-trigger docs: "This variable is null in statement-level triggers and for INSERT operations"), and a field of a null record reads as NULL — never an error — so the COALESCE lands on the populated side. The full battery already runs every decision-create through this trigger; probe R13-F1 pins it explicitly (raw option INSERT and DELETE both execute AND record their notes), and the trigger carries the docs citation. A claim that would have failed every option insert in the suite cannot be patched around silently — it is either real (it was not) or refuted on the record |
| R13-F2 (P2) | the round-12 touch note lived in `pg_temp`, which the SAME session can DROP with **no privilege on the app schema at all** — `[edit option, DROP the temp note, withdraw]` committed a withdrawal whose frozen question was edited in the withdrawing transaction | the note moves to the REAL `DecisionOptionTouch` table (owned + read-encapsulated by the decisions module; written ONLY by the DB trigger), keyed by `(decisionId, txid_current())` — the TOP-LEVEL transaction id, so a SAVEPOINT cannot detach the note from its write (they share the same subtransaction fate). Its guard trigger refuses erasing or rewriting a note in the transaction that wrote it (`pg_trigger_depth() = 1` distinguishes a direct statement from the decision's own delete/re-key cascade, which passes — so the sanctioned destructive resets need NO new bypass; notes from committed transactions are inert history swept by the cascade). Blinding the seal now requires `ALTER TABLE ... DISABLE TRIGGER` — the same OWNERSHIP privilege every sanctioned bypass in this seal network already requires, which is exactly what the pg_temp design lacked. The old plan-cache footgun goes with the temp table: a real table has a stable oid, so the trigger is plain static SQL now. Probed: the verbatim a58e949 bypass refuses; the direct note erase refuses; the note rewrite refuses; committed notes stay inert (a clean later withdrawal succeeds); + three upgrade-proof rejections |
| R13-F3 (P2) | the round-12 type freeze left the event's DECISION mutable — an UPDATE keeping `type='approved'` while changing `decisionId` re-points a legacy decision's ONLY approval evidence at some other LIVE decision (the round-10 arm judges only what it lands ON), and the pending row withdraws afterwards | approval evidence stays with its decision: any `decisionId` change on an `approved`/`reapproved` event refuses ("cannot be re-pointed away"), placed AFTER the round-10 target check so a re-point onto a withdrawn decision keeps its established refusal and every existing battery message is unchanged; non-approval events remain re-pointable. Probed (re-point away refuses; the laundered withdrawal stays refused; a benign event still re-points) + an upgrade-proof rejection |
| R13-F4 (P2) | the round-12 replacement generation copied only rows that EXIST in the retired one — the never-applied shape (a withdrawn decision with NO projection row, a case the stale predicate itself names) produced a caught-up replacement that still OMITS the withdrawal, served as complete by `projectionSlice()` | the missing withdrawn rows are seeded from CANONICAL truth in the same migration step, composing the dto exactly as `serializeDecision` writes it (nullable columns ABSENT when NULL — the `?? undefined` mappings; `photoSwatch`/`draft`/`options` always present; options ordered by `order` with `photoUrl` absent when null; the withdrawal keys in the established `to_char` ISO shape). Pinned by the probe's `storedDecisionRows == computeDecisionRows` equality — the operator diagnostic's OWN comparators, covering every key column and the served payload byte-for-byte — plus checkpoint preservation and the post-migration delivery applying |
| R13-F5 (P2) | `DailyLogService.addMaterial` validated the linked decision with `existsInProject`, so a stale/offline client or a direct POST could record a NEW matched delivery against a withdrawn decision the UI treats as terminal | the daily-log material write adopts the activities linkability authority: a `linkableInProject` pre-check (fast-fail UX) + the in-tx recheck under the decisions-owned row share lock (a withdraw committing between the two reads serializes). The refusal is audience-shaped by the visibility rule: pmc gets the honest terminal reason (the activities wording), every other role gets the same `Unknown decision for this project` a nonexistent id produces — a withdrawn decision is pmc-only, so the refusal must not reveal it. Probed over the REAL route (engineer 400 audience-safe, pmc 400 honest, live decision still links 201, zero rows recorded) |

Round-13 gates: probe file 59/59 (RED at `a58e949`: 5/5); `pnpm check` EXIT 0 (web 764/764,
API 791/791); upgrade-proof extended (the round-13 battery block: the dead pg_temp bypass, the
