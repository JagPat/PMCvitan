...
- `phase4-t1-correction3.test.ts` — **ORDERING 1**, **ORDERING 2**, and a **MULTI-SKILL** referential-integrity/no-deadlock check. Each `it` loops **10×** internally and the invariant asserted is that the final database is always referentially valid (zero orphans) with exactly one side committing.
  - **NOTE (corrected by correction 4 — see `phase-4-t1-correction4-packet.md`):** as originally shipped in this correction, the two ordering probes committed session A *before* session B began, so they were SEQUENTIAL referential-integrity checks, not genuine concurrent overlaps, and the multi-skill probe was likewise sequential. Correction 4 rewrites ORDERING 1 and ORDERING 2 as REAL two-session overlaps (a dedicated client holds one session's transaction open, and the conflicting session is confirmed *blocked on the FK lock* via `pg_stat_activity` before the held transaction commits — condition-based, no fixed sleeps), and relabels the MULTI-SKILL probe honestly as a sequential referential-integrity + no-deadlock test. Read this bullet together with the correction-4 packet for the accurate concurrency claims.
