...
| `Activity` | A unit of site work (the spine) | code (ACT-31), name, zone, decisionId, **nodeId** (location), plannedStart/End, actualStart/End (actual end = the CLAIMED work-end day), **status** (`not-started\|in-progress\|awaiting-signoff\|done\|blocked` — Task 5), **doneAt** (the sign-off civil day — written ONLY by closing-inspection approval), **completionRequestedById/Name/At** (the completion CLAIM — composite `(projectId, userId)` FK to Membership) |
| `ActivityGate` | The four readiness gates | activityId, kind (`decision\|material\|team\|inspection`), state (`ok\|wait\|fail\|na`) |
| `Inspection` | A checklist / review | code (INSP-22), title, zone, **nodeId** (location), **activityId** (the REQUIREMENT EDGE — composite FK, Task 4), **closing** (Task 5 — the unambiguous sign-off marker; the legacy `INSP-<id>-close` id-pattern linkage is retired), **reinspectionOfId** (the predecessor a reinspection re-checks; at most ONE direct child — partial unique `Inspection_one_reinspection_child`), **assigneeId** (active engineer/contractor member — Membership FK) + **dueDate**, **submittedById/Name + decidedById/Name** (attribution) |
...
| `CommandExecution` | Command-idempotency receipt ledger (Phase 2 Task 5) | id (uuid PK), **scopeKind** (`project\|org`), **organizationId** (NON-NULL) + **projectId?** (NULL for org scope), actorId, commandType, idempotencyKey, requestHash, **status** (`reserved\|succeeded\|failed`), resultRef?, createdAt/completedAt?. Scope-specific **partial unique indexes** `command_execution_project_key WHERE scopeKind='project'` on `(projectId, actorId, commandType, idempotencyKey)` and `command_execution_org_key WHERE scopeKind='org'` on `(organizationId, …)` — an org key never constrains a project row; **scope CHECK** + **status CHECK**; composite project-scoped tenant FK `(organizationId, projectId) → Project(orgId, id)` (org scope: `organizationId → Org`). Written ONLY through `platform/commands.ts` `executeCommand` inside the mutation transaction (reserve → mutation+audit+event → succeeded receipt) |
| `OutboxDelivery` | Per-consumer transactional outbox (Phase 2 Task 6) | id (uuid PK), **eventId** (FK DomainEvent, cascade), projectId, consumer, **consumerKind** (`ordered\|unordered`), **streamPosition** (BIGINT, copied from the event), **status** (`pending\|leased\|succeeded\|dead` — NO ambiguous `failed`), attempts, nextAttemptAt, leaseOwner?/leaseExpiresAt?, lastError?, payload?. `@@unique([eventId, consumer])`; status/consumerKind CHECKs. One row per (event, consumer) written IN the event's transaction; a lease relay claims + dispatches. Written through `platform/outbox` (`emitEvent` materializes, `OutboxRelay` dispatches) |
| `ProcessedEvent` | A database consumer's effectively-once record (Phase 2 Task 6) | id (uuid PK), consumer, eventId, processedAt. `@@unique([consumer, eventId])` — committed in the SAME transaction as the consumer's side effect; external (socket/push) consumers do not use it |
| `ProjectionCursor` | An ordered consumer's contiguous per-project checkpoint (Phase 2 Task 6) | id (uuid PK), consumer, projectId, **appliedPosition?** (BIGINT; NULL = none applied, next expected = appliedPosition+1), **status** (`live\|blocked`). `@@unique([consumer, projectId])` — advances contiguously (N+1 waits for N); a `dead` earlier position sets `blocked`, never a silent skip |
...
- **Referential unfiling is a database FK action (Phase 2 Task 7)** — deleting a location node, a phase or a governing activity UNFILES the records that pointed at it instead of a service nulling them in application code. The composite `(projectId, nodeId)` FKs on Activity/Inspection/Media/Drawing/SiteMaterial, `(projectId, activityId)` on Drawing, and `(projectId, phaseId)` on Activity are now **`ON DELETE SET NULL (<col>)`** (PostgreSQL 15+ column-list SET NULL — it nulls ONLY the referencing column and leaves the NOT-NULL tenant `projectId` intact, which a bare composite SET NULL could not). Prisma cannot express the column-list form, so migration `20261025000000_phase2_module_boundaries` is a raw-SQL, additive, row-free constraint swap (accepted-drift convention; `schema.prisma` keeps `NoAction` with a pointer). `Decision.(projectId,nodeId)` stays `NO ACTION` on purpose (nodes.remove guards it — the PMC must move/remove attached decisions first), as do `Inspection`/`GateOverride` `(projectId,activityId)` (they BLOCK an activity delete, surfaced as a friendly Conflict).
- **Civil dates (Phase 0 Task 6)** — domain dates are `DATE` columns (`Project.scheduleStartDate/scheduleEndDate`, `Activity`/`Phase` planned+actual dates, `DailyLog.logDate`, `Inspection.inspectionDate`) carried as ISO `YYYY-MM-DD` at every boundary and computed with UTC-only arithmetic (`packages/shared/src/lib/dates.ts`, imported at runtime by the API via `apps/api/src/common/civil-date.ts` — Phase 2 Task 2; the former pinned copy is retired). The server stamps "today" via an injected `Clock` in the **project's** time zone (`Project.timeZone`). `Project.scheduleStartDate` is THE SCHEDULE ANCHOR — the calendar day legacy offset 0 refers to.
...
- **Gates (Task 6)** — readiness is a READ-TIME DERIVATION from explicit links, serialized per activity as `readiness` (five gates, each `{v, source: derived|stored|override, reason}`): `decision` from the linked decision's lock state; `inspection` from NON-closing inspections linked by `activityId` (first-match truth table — an open correction chain reads `fail` before any plain `wait`; a co-located inspection with a different/null `activityId` is invisible); `drawing` per linked published drawing from the governing revision's FROZEN recipients ∩ active members (legacy discriminator = `recipientsFrozenAt` null), aggregated worst-wins `fail > wait > ok`. `material/team` remain STORED site flags (until Phases 3/4) and say so in their source. `Activity.gateInspection` is a DEPRECATED column — no route can set it (`scripts/readiness-delta-report.ts` lists stored-vs-derived deltas). `start()` requires all FIVE readiness values (`ok|na`), unexpired overrides considered. The derivation lives in `packages/shared/src/domain/readiness.ts` and is imported at runtime by both web and API (`apps/api/src/domain/transitions.ts` re-exports it from the built `@vitan/shared` — Phase 2 Task 2; the pinned copy is retired) — one implementation, the same row-by-row table tests.
...
4. **Complete activity (Task 5)**: a CAS CLAIM `in_progress → awaiting_signoff` recording `completionRequestedBy*` + `actualEnd` and creating the LINKED closing `Inspection` (`closing=true`, one default sign-off item, submitter = the completer) in the same transaction. ONLY the PMC's approval of that closing inspection writes `done` + `doneAt`; rejecting it returns the activity to `in_progress` and assigns the Task 4 reinspection chain to the recorded completer — but only while that identity still holds an ACTIVE engineer/contractor membership; otherwise an explicit eligible assignee is required. Legacy zero-item closings stay decidable via the `closing` flag.
...
```
Contracts will be authored with Zod in `@vitan/shared` (**Phase 2 Task 8**) and shared by both the NestJS handlers and the typed client. The package is already a **built runtime dependency** as of Phase 2 Task 2 — it shares the domain types, civil-date/format helpers, derived-readiness truth tables, authorization policy and seed today; the command/query contracts land in Task 8.
