...
- **Gap-safe event order (finding 1).** Event order is a **durable per-project stream position** assigned inside the owning mutation transaction (`ProjectEventStream` counter locked + incremented; `DomainEvent(projectId, streamPosition)` unique). `occurredAt` is display/audit only and is **never** a checkpoint or ordering key. Projection checkpoints are `(projectId → streamPosition)`; cross-project projections (Portfolio) keep one checkpoint per project. No projector may advance past a position it has not durably applied, and rebuild uses the generation-swap protocol (Task 6), never "replay since a timestamp".
- **Complete, tenant-safe envelope (finding 1).** Every event carries `eventId, eventType, payloadVersion, organizationId (NON-NULL), projectId, streamPosition, siteId?, actorId?, actorKind (human|system), entityType, entityId, occurredAt, correlationId, causedByEventId?, payload`. A composite constraint ties `(organizationId, projectId)` to the project's real org (a project with a null `orgId` is a Task-4 diagnostic **STOP** — backfill first). Attribution CHECK: `actorId IS NOT NULL OR actorKind = 'system'` (system events name a constant system actor). The event store is **append-only enforced at the database level** (a `BEFORE UPDATE OR DELETE` trigger that raises + a restricted writer role), not by convention.
- **One canonical write path (spec §9), idempotent (finding 5).** Every state-changing command: validated → **reserve/resolve its idempotency receipt** (`CommandExecution`) → owning-module transaction writing the canonical record + audit entry + `DomainEvent` (with its stream position) + the successful receipt, all in **one transaction** → per-consumer outbox delivery rows → asynchronous idempotent consumers → projections/WS-invalidation/push. Same key + same payload replays the committed result; same key + different payload → 409; a failed transaction leaves the key safely retryable.
- **At-least-once delivery, effectively-once effects (finding 2).** Delivery state is **per consumer** (`OutboxDelivery`, `UNIQUE(eventId, consumer)`), with leases, backoff and a terminal dead-letter status. A database consumer commits its side effect **and** its `ProcessedEvent` idempotency row in one transaction (effectively-once). WebSocket and Web Push are **at-least-once** — a crash after an external send but before acknowledgement may duplicate; `eventId` is the dedupe key where the provider supports it, and the residual duplicate behavior is documented, never claimed away as "exactly once".
...
_(Note: the projection **framework + full event catalog** are built in Tasks 4/6; Task 9 is where the first read path is switched to a projection on the frontend, paired with that module's frontend migration, so the projection cutover and the frontend cutover are reviewed together for one module before Task 10 repeats the pattern.)_
...
**Business outcome:** Every structural boundary and every projection Phase 2 will touch is pinned against today's observable behavior and its exact data dependencies, so each later task proves an intentional change and Task 6 cannot switch a read path onto an incomplete event set.
...
```text
CommandExecution  id, projectId, actorId, commandType, idempotencyKey, requestHash,
                  status ('reserved'|'succeeded'|'failed'), resultRef, createdAt, completedAt
  @@unique([projectId, commandType, idempotencyKey])
```

**Protocol:** reserve/resolve concurrent duplicates (insert the `reserved` row; a duplicate insert → `P2002` → the loser waits for/reads the winner's committed result); the canonical mutation + audit + `DomainEvent` + the `succeeded` receipt commit in **one transaction**; **same key + same `requestHash`** replays the committed result/effect; **same key + different `requestHash`** → **409**; a failed transaction leaves the key safely retryable (no `succeeded` receipt, `reserved` expires/cleared).

- [ ] **Step 1: Failing tests first (live PG)** — two concurrent identical commands → one execution + one event, the other returns the same result; same-key/different-payload → 409; a mid-transaction failure leaves the command retryable; the receipt commits atomically with the mutation.
- [ ] **Step 2: Schema + the reserve/execute/receipt wrapper** applied to the pillar commands; **map the existing offline outbox op IDs into `idempotencyKey`** (the frontend already generates client op ids — they become the key; `Media.clientKey` folds in).
- [ ] **Step 3: Additive rollout** — backend accepts-and-honors keys now; a missing key falls back to today's CAS behavior (no regression) so old clients keep working; enforcement (reject missing key) is a later, deliberately-gated step, not this phase.
- [ ] **Step 4: Verify** (+ API/browser retry probe: replay a submit/approve twice → one effect).

Run: `pnpm check` · integration suite (concurrency probes) · both acceptance suites (offline replay still idempotent) · `upgrade-proof`.

**Done criteria:** duplicate/concurrent commands are effectively-once with deterministic replay/conflict semantics; offline replay maps onto the ledger; old clients unaffected.
...
```text
OutboxDelivery  id, eventId (FK DomainEvent), consumer, status ('pending'|'succeeded'|'failed'|'dead'),
...
**Delivery contract:** when an event is appended, its `OutboxDelivery` rows for every registered consumer are materialized (in the same or a follow-on transaction keyed off the event, so materialization is itself idempotent). The relay claims work with a **short lease transaction** (`status='pending' AND nextAttemptAt<=now`, set `leaseOwner`/`leaseExpiresAt`, commit) — not a lock held across dispatch — dispatches, then records the terminal state; an expired lease is reclaimable; exhausted attempts → `dead`. **A database consumer commits its side effect AND its `ProcessedEvent` row in one transaction** (effectively-once). The contract is **at-least-once delivery with effectively-once database effects**. **WebSocket and Web Push are at-least-once** — a crash after send but before ack may duplicate; `eventId` is the dedupe key where the provider supports it; the residual is documented, never claimed as exactly-once.
...
- [ ] **Step 1: Failing tests first (live PG)** — delivery row committed with the event; two consumers with mixed success (one succeeds, one fails → per-row truth, neither masks the other); duplicate dispatch → one effect for a DB consumer; crash before/after effect; stale-lease reclaim; concurrent workers; one consumer exhausting retries without blocking another; the `Notification` row is visible in the mutation's immediate response; exactly one active socket/push sender across rollout→cutover→rollback; socket refetch is correct under duplicate invalidations.
- [ ] **Step 2: Schema + relay + consumer registration + lifecycle bootstrap** (in-process; lease-based).
...
- [ ] **Step 1: Projection base + rebuild protocol (finding 1).** Checkpoint per `(projectId → streamPosition)`; **online rebuild** = open a consistent canonical snapshot (repeatable-read), capture per-project high-water marks from that same snapshot, build a **replacement generation**, replay positions above each watermark, then atomically activate the generation; writes during rebuild are caught up before activation. Projection rows carry tenant + subject keys; **serving requires a current-membership/authorization check** (finding 6); membership removal/role change invalidates the affected user-specific rows.
...
- [ ] **Step 6: Verify** with the Phase-0 project-switch scenarios + the module's flow + rebuild-while-events-continue + role/removal/two-project/two-org authz probes.

Run: `pnpm check` · `api-e2e` (project-scope + the module's chain) · the coordinator's `snapshot-ordering` suite · projection rebuild + authz probes.

**Done criteria:** the first module runs on a rebuildable projection + module-owned query with query-time authz; shell + nav are manifest-driven; read ownership is XOR; project-switch/stale-response scenarios green unchanged; live == rebuild. **REVIEW STOP (first frontend migration — a narrow pattern review).**
...
- [ ] **Step 3: Full modular acceptance chain** — the Phase-1 pillar chain over extracted modules + outbox + projections, green unchanged; CI boots the API + relay and runs the API-backed browser flows.
- [ ] **Step 4: Waiver sweep** — the final gate **fails** if any boundary waiver from Task 7 is still present.
- [ ] **Step 5: Phase 2 review packet** (spec §27) + docs (`ARCHITECTURE`/`DATA_MODEL`/`ROADMAP` record Phase 2 implemented; facts/events unlocked for Phase 3 stated). **Step 6: Verify.**

Run: `pnpm check` · full integration suite · both acceptance suites · `upgrade-proof` · the per-module matrix.

**Done criteria:** all modules extracted behind contracts + events + projections + frontend queries with flows unchanged; the §21 matrix green per module; no outstanding boundary waivers; CI starts PostgreSQL, applies migrations from zero, boots the API + relay, runs the API-backed browser flows; the packet records the full evidence. **REVIEW STOP — Phase 2 gate.**
