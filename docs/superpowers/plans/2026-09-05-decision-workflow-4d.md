...
**Review lineage.** This document is the REPLACEMENT of PR #538 (`Replaces: #538`),
itself the replacement of PR #537, carrying the same docs-only unit and nothing else. Round 1 on #537's first head
...
4)". No design decision carried from `6a53aae` was reopened; every fix is a
...
follows a shell-level `rollout.decisionForwarding: 'reserved' | 'open'`
baked from that same read, so no client offers what the server refuses
...
- **Forwarding**: an append-only `DecisionForward` chain — `projectId`,
  ...
  **Forwarding SERIALIZES against approval and countersign** (round 1): each
  ...
  otherwise be sent to a removed architect); the consultation families keep
  their delivered 4c predicates. (ii) the consumer performs a FINAL same-row
  ...
  guard and never was. The residual is then PER RECIPIENT — a removal
  committing during that one user's provider call — the same disclosed class
  as (iii); (iii) the residual — an invalidating command
  ...
  consultee push surviving each. This is a change to 4c's cleared surface,
...
- **4d implementation follows as FOUR PRs — the dark migration 4d-i, the
  ...
  - **4d-ii, the service/role/UI unit**: the role fan-out (§A.1 — every
    ...
    while the forward reservation stands and the shell's
    `rollout.decisionForwarding` read the web affordance follows (review
    round 4); the existing targeted
    ...
    the socket consumer is not bumped (it carries no new contract). 4d-ii
  ...
  - **4d-iii, the reservation retirement**: a migration-only unit that
    ...
    every new `ChangeRequest` row (§A.3 obligation 6; review round 3),
