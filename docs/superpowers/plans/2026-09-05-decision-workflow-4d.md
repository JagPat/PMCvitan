...
**Review lineage — the ledger, not the narrative.** This document is the
REPLACEMENT of PR #552 (`Replaces: #552` — labelled by the orchestrator before
it closed, so the ledger holds its obligation; every predecessor closed at the
limit stays a labelled pending obligation until a MERGED unit names it, one
merge discharging one — the accepted gap of
`docs/reviews/replacement-lineage-repair.md`; #541 alone holds no label, having
closed before it could be labelled — the #514/#513 lesson in STATUS), itself
the replacement of #551, #550, #549, #548, #547, #546, #545, #544, #543, #542,
#541, #540, #539, #538 and #537 in turn — the same docs-only unit and nothing
else, sixteen times rebuilt from the same `main` `d7eed4ce`. Thirty-two
exact-head Codex rounds folded one hundred and sixty-eight findings, EVERY one
annotated in the body of this plan at the exact decision it forced ("(review
round N)"), which is where a reviewer should read them. The per-round
narrative earlier heads carried here is retired (review round 32 — the
owner's standing directive of 2026-09-07 asks that the review unit be
narrowed at safe seams rather than re-rolled with incremental wording, and
roughly 380 lines of PR history carried no design): `docs/STATUS.md` remains
the authoritative ledger and each closed PR's body the verbatim per-round
record.

| PR | finding-bearing heads | findings folded | review rounds |
| --- | --- | --- | --- |
| #537 | `d82d47f4`, `77ab82b1` | 8 + 6 | 1–2 |
| #538 | `cae3d167`, `cbbe9d23` | 7 + 5 | 3–4 |
| #539 | `91416864`, `0526eb8d` | 5 + 4 | 5–6 |
| #540 | `fcc6d15b`, `04b5491c` | 7 + 5 | 7–8 |
| #541 | `a05fdd80`, `6ec40f12` | 3 + 4 | 9–10 |
| #542 | `aafc0e17`, `182a09bf` | 5 + 3 | 11–12 |
| #543 | `7bfdc3e0`, `9c6dc588` | 6 + 6 | 13–14 |
| #544 | `4dda84fc`, `822d4838` | 7 + 3 | 15–16 |
| #545 | `d52af4f8`, `5efbfc06` | 5 + 3 | 17–18 |
| #546 | `0ae38c88`, `e60b540c` | 7 + 5 | 19–20 |
| #547 | `64b0c239`, `ad4352dc` | 5 + 7 | 21–22 |
| #548 | `29ee11af`, `966defdd` | 5 + 8 | 23–24 |
| #549 | `a87742ae`, `56a27029` | 6 + 6 | 25–26 |
| #550 | `27ea7871`, `6841e343` | 4 + 4 | 27–28 |
| #551 | `b5a27ef4`, `ee2d7503` | 7 + 4 | 29–30 |
| #552 | `af357a24`, `4b9a648f` | 5 + 3 | 31–32 |

Round 32 — #552's second head (`4b9a648f`), three findings (two P1, one P2),
folded here and annotated "(review round 32)": the no-chain approval arm
decides by CORRESPONDENCE with the derived canonical bundle, never by
counting same-transaction events (a lone forged or partial bundle is refused,
the service's canonical one admitted verbatim, an absent one written by the
seal); the countersign claim hook resolves the demand's audience inside the
lease transaction and persists it on the notice, and the send uses that
audience alone, so a holder activated after the notice's position can be
demanded only by the crossing handler's replacement; and the notification
twin is keyed on `Notification.eventId`/`kind`, columns 4d-i's repair
bootstrap transaction adds with their partial unique index, the feed row
staying project-wide and byte-identical to 4b's.

**The close-and-replace protocol stands unchanged; its next step is no
longer Board-gated.** Should THIS PR reach a second finding-bearing head, the
loop closes it unmerged and opens the next freshly scoped replacement from
current `main` under the owner's standing continuation authorization of
2026-09-07 (#482, comment 5563944111 — it supersedes the per-PR caps recorded
on #549, #550, #551 and #552 and installs no new one), narrowing the unit
further at safe seams rather than re-rolling it, and reports each replacement
on #482 with its links; the exact-head gate, the two-finding-head limit and
reviewer independence are unchanged. No design decision carried from
`6a53aae` was reopened; every fix is a precision this plan owed and had not
stated.
...
JWT while membership creation is still blocked; `AuthService.session`
therefore REFUSES an `architect` request BEFORE EITHER BRANCH — ahead of
the user lookup, not only in the fallback (review round 28: `User.role` is
an unconstrained string the audit did not inspect and the real-user branch
issues `real.role` verbatim, so a pre-existing or directly inserted dev
`User` with role `architect` would have bypassed a fallback-only refusal)
— 409 naming the drain directive — until `rollout.phase6_4d` is open, and
the synthetic `dev-<role>` fallback NEVER mints `architect` even after it
(review round 30: a synthetic actor has no account, so no fact it wrote
could satisfy the `actorName` seal §A.2 binds to the account's display
name; a dev architect is a seeded `User` fixture the real-user branch
serves once the rollout is open); the
4d-i audit ALSO names any `User` row with `role = 'architect'` and aborts,
the same shape as the `Membership` audit, its repair a re-role of the dev
fixture; and P28b covers this alternate token producer in BOTH arms) — which is what keeps a still-serving pre-4d instance safe
...
- **Forwarding**: an append-only `DecisionForward` chain — `projectId`,
  ...
  **And the architect can SEE what the architect may forward** (review round
  ...
  decision's `decider` AND `forward` deliveries by subject — locking those
  delivery rows `FOR UPDATE` in ascending id order BEFORE taking the
  decision row lock (review round 30: the delivery-then-decision order §A.2
  fixes for the crossing handlers binds EVERY path that holds a delivery
  and a decision together, since the relay's claim leases the delivery
  first and then takes the decision lock; a command holding the decision
  and then reaching for a leased delivery would deadlock and abort one
  side; so the canonical order becomes readiness → `Project` →
  `Membership` → the subject `OutboxDelivery` rows → `Decision`, and a
  delivery the command finds already leased by a claim is SKIPPED, never
  waited for — that claim, blocked on the decision lock the command holds
  next, resumes after the commit and its final pre-send re-read observes
  the transition and drops the send with the mark, the round-13/18
  backstop) — because a delivery claimed before the approval passes the
  ...
  delivery (the demand is gone, whichever way) under the SAME
  delivery-then-decision order, skipping a leased row for the claim's
  final re-read (review round 30), as do the PMC's `withdraw` and every
  transition that leaves the consultation-open set — P35's barrier gains
  each command against a concurrent claim in both orderings, no deadlock; **the PMC's `withdraw` cancels that
  ...
  CLAIM hook appends in the claim transaction, before the provider call
  (review rounds 25 and 28, §A.2 — the standing position the claim resolved
  holders against, captured durably ahead of the irreversible send, and —
  review round 32 — the resolved holders themselves, the ONLY audience the
  send may use;
  INSERT-sealed to that very delivery and to the register's position at
  the claim, review rounds 27–28): a demand
  resolved at or after the crossing, sent or still in flight, is left alone;
  ...
  to each (review round 32: for the COUNTERSIGN family that resolution
  moves INTO the claim hook and is persisted on the notice of §A.2, and the
  send reads the notice's `recipientUserIds` — the claim hook of §D), so with architects A and B active the countersign push claims,
...
- **Countersign, and the state that carries it** (round 1): under an active
  ...
  under the fact's freeze — and BOUND to the authenticated actor, never
  merely supplied (review round 28: the uniform provenance check equates
  only `CommandExecution.actorId` with the fact's actor and the standing
  seal proves only that the id held some authority, so a valid hand-run
  ...
  `platform_write_notification(projectId, eventId, kind, decisionId, text,
  color)`, canonical-pinned by the same tripwire discipline as
  `platform_emit_event` — idempotent per `(eventId, kind)` on a durable key
  the schema actually carries (review round 32: the delivered
  `Notification` row is the PROJECT-WIDE feed 4b writes — `id, projectId,
  text, color, time, at, decisionId`, no recipient and no event identity —
  so the earlier `(event, recipient)` promise named columns that do not
  exist and could not be implemented; 4d-i's repair bootstrap transaction
  therefore adds the nullable `eventId` (FK to `DomainEvent(eventId)`) and
  `kind` columns to the platform-owned `Notification` with a partial UNIQUE
  index over `(eventId, kind) WHERE eventId IS NOT NULL`, the twin inserts
  `ON CONFLICT DO NOTHING` on it, the feed row stays project-wide and
  byte-identical to 4b's, and a pre-4d writer's rows keep a null `eventId`
  and are never touched), so a repeated deferred-seal invocation appends
  nothing, the returned snapshot and the
  ...
  frozen actor — ARMED ONLY AT 4d-iii (review round 30: during the 4d-ii
  coexistence window a still-running pre-4d process executes the current
  ...
  `pending`/`change → approved` transition the lifecycle seal first
  DERIVES the canonical bundle for that transition from in-transaction
  state alone — the event kind (`decision.approved` when the revision's
  recorded `approvedFrom` is `pending`, `decision.reapproved` when it is
  `change`, the delivered `prior === 'change'` rule), the actor triple
  frozen on the revision and equal to the receipt's (`approvedById`,
  `approvedByRole`, `approvedByName`), the canonical payload (the chosen
  option's label and material, `onBehalfOf` exactly when the revision
  records one) and the dispatch intent the canonical-pinned twin produces
  for that kind, plus the `DecisionEvent` register row and the feed row
  the twin of §A.2 writes — and then decides by CORRESPONDENCE, never by
  count (review round 32: round 31 counted the same-transaction
  `decision.approved`/`reapproved` `DomainEvent` rows and deferred to any
  single one, so an admitted receipt-backed direct writer could insert ONE
  event of the wrong kind for a `change → approved` flip, or the right kind
  with a forged actor or payload, omit the register and feed rows, and
  have the canonical row commit with permanently false or missing
  evidence): NO same-transaction `decision.approved`/`reapproved` event
  for the decision (rows whose `xmin = txid_current()::text::xid`, the
  receipt-seal conversion) → the seal writes the whole derived bundle
  through the twins; exactly ONE → the seal REQUIRES it to equal the
  derived bundle field for field — kind, `actorId`/`actorRole`, payload
  and dispatch intent — AND requires the same-transaction `DecisionEvent`
  row naming that event and the same-transaction feed row for the decision
  whose `text` and `color` equal the canonical (the byte-identity the
  SQL-twin tripwires already assert; a pre-4d writer's row carries no
  `eventId`, so the feed row is matched by `(projectId, decisionId, text,
  color)` within the transaction, never by the key alone), refusing the
  commit on ANY divergence or missing row — the seal never completes,
  repairs or re-stamps a partial bundle, since a bundle that is not the
  canonical one is not the service's; TWO or more → refused at commit —
  so exactly one emitter holds by construction for every writer at every
  instant, and the one admitted emission is provably the canonical one:
  old and new service binaries keep emitting throughout the drain and the
  seal admits them because their bundle IS the derived bundle; a hand-run
  bundle that emits nothing has its event, register row and feed row
  written by the seal from 4d-i on, never a window without emission and
  nothing for 4d-iii to reconcile; the 4d-ii binary drops its service
  emission and notification at 4d-iii's marker as a CLEANUP, not a switch,
  because the seal already owns the case; P37's direct-bundle arm asserts
  the event, the register row, the feed row and the fold with the seal
  disarmed by nothing, its RED SITES a lone `decision.approved` inserted
  beside a `change → approved` flip (wrong kind), a lone event of the right
  kind naming another actor or a different option, a canonical event with
  its `DecisionEvent` row omitted, and one with its feed row omitted — each
  refused at commit — while the service's own bundle is admitted verbatim;
  and the handoff probe drives a service approve racing the marker's
  insertion in both orderings to exactly one event), and 4d-ii carries the
...
- **4d implementation follows as FOUR PRs — the dark migration 4d-i, the
  ...
  - **4d-i, the migration unit**: ONE additive migration, in one explicit
    ...
    standingPosition, recipientUserIds, at)` of §A.2 (review round 26 — a same-project
    composite FK to `Decision(projectId, id)`, an FK to `OutboxDelivery(id)`
    — that platform table's only candidate keys are `id` and `(eventId,
    consumer)`, and 4d-i adds no `(projectId, id)` unique key to a
    platform-owned table for a decisions-owned reader (review round 30:
    the composite FK as first written could not be created and would have
    stopped 4d-i from deploying) — with project equality
    (`OutboxDelivery.projectId = NEW."projectId"`) enforced by the
    correspondence seal below, UNIQUE
    ...
    delivery's `leased` write — the CLAIM — belongs to the CURRENT
    transaction (its `xmin = txid_current()::text::xid`, the receipt-seal
    conversion) — the notice is born in the claim transaction, BEFORE the
    provider call, or not at all (review round 28: round 27 wrote it in the
    success transaction against the register's CURRENT position, which the
    irreversible send races — resolve A at P, send, A removed and B
    activated at Q before the success write, and the notice either records
    P and is refused after the send, leaving the delivery retryable, or
    records Q and silences B's replacement; so the position is captured
    DURABLY at the claim, under the decision row lock, with no external
    call between the read and the write); and that `NEW."standingPosition"`
    equals the register's `lastCrossingPosition` for `(projectId,
    'architect')` read under the row lock in that same claim transaction;
    and (review round 32) that `NEW."recipientUserIds"` is NON-EMPTY and
    every element holds `architect` standing in `NEW."projectId"` at the
    claim (`phase6_user_holds_role(project, user, 'architect')`, read in
    that same transaction) — the audience the claim resolved, captured
    durably beside the position it resolved it at;
    UNIQUE per `(projectId, deliveryId, standingPosition)`, so a retried
    claim after a crossing appends its own row — RED SITES: a notice naming
    a decider-family or another decision's delivery; a notice inserted
    outside the claim transaction; a position not the register's at the
    claim; a notice with an empty audience or naming a non-holder), and the
    ...
    round 31, from the repair bootstrap transaction), the platform-owned
    `Notification.eventId`/`kind` columns with their partial unique key
    (review round 32, from the same bootstrap transaction) and the
    platform-owned
    `ProjectRoleStanding`
    ...
    freeze extended over them (review round 31), and `ALTER TABLE
    "Notification" ADD COLUMN IF NOT EXISTS` for the nullable `eventId`
    (FK to `DomainEvent(eventId)`) and `kind` with the partial UNIQUE index
    over `(eventId, kind) WHERE eventId IS NOT NULL` that keys
    `platform_write_notification` (review round 32), all additive and
    ...
    status; the same file then deploys AFTER the failed attempt is
    resolved (review round 28: the audit abort follows the committed
    bootstrap, door and enum statements, and Prisma records the attempt as
    failed, so a plain redeploy stops at P3009 — the recovery is repair →
    `prisma migrate resolve --rolled-back <the 4d-i migration>` → redeploy,
    exactly the repository's 4c-iii-r path in `apps/api/scripts/migrate.sh`
    (`report_4c_iiir_migration_failure`), which gains a
    `report_4d_i_migration_failure` twin printing those three steps when
    this file fails, and `docs/RUNBOOK.md` gains the section), and
    `upgrade-proof.sh` drives the REAL sequence through `migrate.sh` —
    deploy (abort, the failed attempt recorded) → repair (with NO
    4d-capable client running) → `migrate resolve --rolled-back` → redeploy
    — never a direct SQL re-run that bypasses Prisma's failed-migration
    ledger, for BOTH reserved values — a planted `architect` row re-kinded and a
    ...
    `migrate resolve --rolled-back` → redeploy through the real runner
    (review round 28), and one hostile insert per seal; `TRUNCATE_SEALS` gains TEN
  ...
  - **4d-ii, the service/role/UI unit** (review round 17: it also moves the
    ...
    **But OTHER OPEN TABS do** (review round 18): a client that loaded a
    ...
    decisionId, deliveryId, standingPosition, recipientUserIds, at)` fact (review round 25 —
    ...
    under the decision row lock, RESOLVES the demand's audience (the
    `architect` role's current holders through the orgs participant's
    `effectiveRoleHolderUserIds`, read in that same claim transaction) and
    appends the notice naming the delivery, the register's
    `lastCrossingPosition` it resolved the holders against, AND the resolved
    holders themselves as `recipientUserIds` (review round 28 — the pre-send
    snapshot captured durably with the claim, never the register's position
    at success, which the irreversible send races; review round 32 — the
    delivered `makePushConsumer` resolves `roleHolderUserIds` in `handle`,
    AFTER the lease transaction has committed, so a notice recording only
    the position bound nothing: with A active a claim could record P and
    commit, A be removed and B added at later crossings, and the send then
    resolve CURRENT holder B and deliver the ORIGINAL demand, after which
    the crossing handler read the notice at P as pre-dating B's activation
    and emitted a replacement — B demanded twice; so the countersign
    family's `handle` SENDS ONLY to its delivery's latest notice's
    `recipientUserIds`, never to a fresh resolution — the per-recipient
    pre-send re-check of §A.2 still SKIPS a listed holder whose standing
    ended since the claim, and a countersign delivery that reaches the send
    with NO notice is dropped with the cancellation mark, never sent from a
    live lookup; an empty resolved audience is the claim-time
    non-actionable verdict, marked and never noticed — so a holder activated
    after the notice's position is never in the original's audience, and
    the only demand that can reach B is the handler's replacement); the
    handler then SKIPS a decision whose latest notice
    names a `standingPosition` at or after this crossing (the holders were
    resolved at the current standing, whether the send has completed or is
    still in flight), and OTHERWISE — the latest notice pre-dates the
    crossing, so the demand was resolved for the displaced architect, or
