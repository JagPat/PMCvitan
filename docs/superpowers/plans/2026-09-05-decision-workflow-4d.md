...
10)". No design decision carried from `6a53aae` was reopened; every fix is a
...
labels; and the forward door's role-`toDesignation` arm
(`phase6_effective_role_standing ≥ 1`, §A.2) admits it unchanged. Dark until
...
- **4d implementation follows as FOUR PRs — the dark migration 4d-i, the
  ...
  - **4d-ii, the service/role/UI unit**: the role fan-out (§A.1 — every
    ...
    audience selectors and the labels — review round 10); the four commands
  ...
  - **The drain attestation** — the operator states that every process
    ...
    BEFORE the role is enabled: P29c gains the stale-client arm — a
    `recorded-v1`-only client under an active chain is refused on approve
    (never told "Approved & locked") and receives a snapshot with no
    awaiting rows, while a `countersign-v1` client sees and drives
    everything; the P38 startup-fence probe already covers the server
