...
"(review round 22)". No design decision carried from `6a53aae` was reopened;
...
- **Forwarding**: an append-only `DecisionForward` chain — `projectId`,
  ...
  **And the architect can SEE what the architect may forward** (review round
  ...
  never dead-letter) — and the archived READ and the park WRITE are ONE
  ...
  itself is the right instrument. P40's archive arm gains both orderings
...
- **Countersign, and the state that carries it** (round 1): under an active
  ...
  `requestedById` — decisions-owned rows) and the catalog's dispatch
  intent; the service emits NONE of these five itself (one emitter, the
  ...
  every writer; the service writes neither for a 4d transition (the
  delivered no-chain approve keeps 4b's in-transaction notification, a
  cleared surface), `decisions.inbox` folds
  ...
  path `membership.standing_changed` takes. The delivered no-chain
  `pending → approved` keeps 4b's service emission (a cleared surface this
  plan does not reopen), its direct-writer case covered by the send-time
  subject re-judge. P31/P34/P33 gain the hand-run-bundle arm: the bundle
...
- **4d implementation follows as FOUR PRs — the dark migration 4d-i, the
  ...
  - **4d-i, the migration unit**: ONE additive migration, in one explicit
    ...
    representable and unreserved): the three fact tables (`DecisionForward`,
    ...
    repair is a service-path edit or withdrawal of the named row (never an
    invented holder or status), and the same file then deploys. P28b's arm
    ...
    redeploy, and one hostile insert per seal; `TRUNCATE_SEALS` gains EIGHT
    entries across 4d-i and 4d-ii — the three new fact tables, the
    register's `ProjectRoleStanding_t4d_no_truncate`,
  ...
  - **4d-ii, the service/role/UI unit** (review round 17: it also moves the
    ...
    **But OTHER OPEN TABS do** (review round 18): a client that loaded a
    ...
    transaction emitted for this membership (review round 21) —
    ...
    4d-ii's catalog-data migration FILLS in the one transaction that
    registers the consumer (the catalog row itself is unchanged; registered
    ...
    so a BEFORE INSERT OR UPDATE OR DELETE seal, armed by the registration
    migration immediately AFTER its own fill, refuses every later INSERT as
    well as every rewrite, and the statement-level
    ...
    a cutover row for a post-registration project refused): a database
    holding historical
