...
turn, so it closed without a third head and THIS seventh replacement carries
all seventy-four fixes, the six annotated "(review round 14)". Round 15 —
this seventh replacement's own first head (`4dda84fc`), seven findings (six
...
tables never registered in the decisions manifest's ownership sets — is
folded on its second head, annotated "(review round 15)". No design decision
carried from `6a53aae` was reopened; every fix is a precision this plan owed
and had not stated.
...
- **EVERY entry into `approved` is DB-SEALED behind the chain** (round-5
  ...
  writes are, from 4d-i, SERVICE-ONLY — the orgs-owned
  `Membership_t4d_architect_service_only` seal refuses any direct write
  touching an architect row without the orgs service's `SET LOCAL` gate
  (review round 14, the re-notification paragraph below), so the
  direct-writer probes for that role assert REFUSAL, not serialization. **And the membership seal judges
...
- **4d implementation follows as FOUR PRs — the dark migration 4d-i, the
  ...
  - **4d-i, the migration unit**: ONE additive migration, in one explicit
    ...
    project, backfilled `active = false`, its deferred
    `DecisionChainStanding_t4d_mirror` seal, §A.2 — on the decisions
    manifest the same way) — with composite
    ...
    the PERMANENT orgs-owned `Membership_t4d_architect_service_only` seal
    (review round 14 — every architect write through the gated service
    path, so re-notification and the chain-changed signal cannot be
    bypassed; not dropped by 4d-iii); the widened `phase6_t4b2_decision_seal`
  ...
  - **4d-ii, the service/role/UI unit**: the role fan-out (§A.1 — every
    ...
    to widen the readers (review round 14): 4d-i installs the orgs-owned
    `Membership_t4d_architect_service_only` seal — a BEFORE INSERT OR UPDATE
    OR DELETE trigger refusing any write that inserts, re-roles into or out
    of, restores, soft-removes or hard-DELETEs a row whose OLD or NEW role is
    `architect` unless the transaction carries the orgs service's `SET LOCAL
    vitan.phase6_architect_mutation` gate (the `RolloutRetirement` marker's
    gate precedent), which ONLY the orgs role mutations set, under
    `lockProjectReadiness` where they already read the standing boundary;
    permanent, not transient — it outlives 4d-iii. Every architect
    activation and deactivation therefore crosses ONE boundary, and that
    boundary does both things: it invokes
    ...
    the boundary, and sealed against drift by a DEFERRED constraint trigger
    `DecisionChainStanding_t4d_mirror` asserting at commit `active =
    (phase6_effective_role_standing(projectId, 'architect') > 0)` — the
    orgs-owned SQL primitive the decisions seals already reach over the
    declared decisions → orgs edge, so the mirror can never disagree with
    the truth it mirrors (a direct flip of the row, and a membership write
    that skipped the participant, both refused at commit; the service-only
    seal makes the latter unreachable anyway). `countersignRequired` is then
    ...
    REFUSED (hostile probes: INSERT, role UPDATE into and out of
    `architect`, soft removal, hard DELETE — each without the gate refused;
    each through the service succeeding, upserting the mirror once per
    boundary crossing, re-notifying on zero-to-one); the operator repair
    ...
    nothing; a direct architect INSERT without the gate refused; a direct
    flip of the mirror row refused at commit; rebuild == live (the
