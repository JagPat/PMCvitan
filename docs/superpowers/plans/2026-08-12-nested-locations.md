...
| `P11` a saved element-root module placed under a `zone`, AND another under a `room` | both instantiate at their chosen anchor — the NEW zone-level shape works and the EXISTING room-level shape survives (a fix that remaps every element module to zone anchors passes the first and breaks every saved door/fixture module) | `anchorKindOf` → `'room'` + the placement guards + the zone-only graft |
