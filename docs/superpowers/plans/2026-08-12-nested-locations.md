...
| `P9` add-child from the Locations dialog, for BOTH new shapes | a `room` under a `room` AND an `element` directly under a `zone` are each creatable in the dialog — one child action is not enough | the dialog's three-icon row |
...
| `P11` a saved element-root module placed under a `zone`, AND another under a `room` — **through the public create-project path, not a graft helper** | both instantiate at their chosen anchor via the real selector contract. The NEW zone-level shape works, the EXISTING room-level shape survives, and the user can actually EXPRESS the room target: `moduleSelectionSchema` (`contracts.ts:337`) carries only `underZone` today and `ProjectSwitcher` mirrors it, so the selection contract gains a room-target field in this unit — a service-level probe that bypasses the selector proves a workflow nobody can reach | `anchorKindOf` → `'room'` + the placement guards + the zone-only graft + the `underZone`-only selection contract |
| `P12` decisions filed at `Zone > Room > Room`, at an `element` under a `zone`, directly on a `zone`, and on a `room` viewed in element grouping | kind-true grouping in presence AND absence: the nested-room decision groups under the room it was FILED at; the zone-level element never shows as a room; the zone-filed decision never shows as a room; the room-filed decision never shows as an element | the positional `seg[1]` / last-segment grouping |
| `P13` the filing picker on a nested tree — select AND create | a room under a room and an element directly under a zone are each reachable, selectable, AND creatable inline (the picker is where every filing flow creates nodes; rendering a seeded tree while inline-create still stops at three levels blocks users from the new shapes) | the picker's fixed `zone`/`room` state |
