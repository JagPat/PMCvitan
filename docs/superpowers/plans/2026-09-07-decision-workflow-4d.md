...
7. **Effect correspondence — a seal VERIFIES, it never EMITS.** Where the
   ...
   the mirror unexplained) — whose AFTER INSERT trigger is the ONLY writer
   of the mirror; `OutboxConsumerCatalog_t4d_rules` freezes `active` and
...
- **4d implementation follows as FOUR PRs — the dark migration 4d-i, the
  ...
  - **4d-i, the migration unit — MIGRATION-ONLY in the template's sense**:
    ...
    partial apply retries. `TRUNCATE_SEALS` gains ELEVEN entries across
