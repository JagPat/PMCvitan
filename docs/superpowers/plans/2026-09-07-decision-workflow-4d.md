...
P2): five folded on its ONE correction head, one DECLINED on the Board's
recorded decision, none dropped.** Three are the plan's own boundary rule
...
`platform_role_standing ≥ 1` — the standing read riding
...
7. **Effect correspondence — a seal VERIFIES, it never EMITS.** Where the
   ...
   **The feed row carries structured semantics the seal can judge, and the
   ...
   BEFORE `emitEvent` allocates the event, and 4d-ii stamps those inserts
   with the id `emitEvent` will use rather than reordering every writer;
