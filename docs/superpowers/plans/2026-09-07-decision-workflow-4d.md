...
7. **Effect correspondence — a seal VERIFIES, it never EMITS.** Where the
   ...
   decision, and whose `kind` equals that event's `eventType`. Zero of any
   ...
   **The kernel seals its own envelope, so a module seal never has to** (this
   ...
   is refused at commit. 4d-i SEEDS the catalog with the CURRENT compiled catalog
   ...
   **The audit row is as immutable as the fact** (this PR's review round 1,
   ...
   the seal installed and enabled afterwards. **And the row's WHOLE
...
- **4d implementation follows as FOUR PRs — the dark migration 4d-i, the
  ...
  - **4d-i, the migration unit**: ONE additive migration file in THREE
    ...
    `phase6_role_holder_user_ids`; the reset protocol in `prisma/seed.ts` and
    `test/integration/fixtures.ts` gaining `DecisionEvent_t4d_append_only`; `Notification.eventId` (nullable, FK to
