...
This document REPLACES PR #560 (`Replaces: #560` — labelled
...
having closed before it could be labelled). #560 was the SIXTH outing of
the NARROWED plan: it drew eleven findings on its first head (`01c6e819`;
ten folded on its one correction `847b5c40`, one — the drain attestation —
declined on the Board's recorded decision) and six more on that head, and
closed at the limit. #560 had replaced #558 (the fifth outing: seven on
...
P2), folded on its ONE correction head, none dropped.** One is the plan's
...
`ProjectUserStanding(projectId, userId, role)` register — a row present iff
that user holds ACTIVE standing in that role on that project (an active
membership in the role; or, for `pmc`, an org owner/admin with no active
membership on the project — the membership-less arm the token role and the
project access path already admit), same-project FK to `Project` `ON
...
`UserIdentity_t4d_no_truncate`. Their ONLY writers are the generic platform
primitives `platform_user_standing_apply(projectId, userId, role, present)`
and `platform_user_identity_apply(userId, displayName)`, knowing no role
and reading no table, called by ORGS-owned triggers that read only orgs
tables: the extended `Membership_t4d_role_standing` (a membership's
activation, deactivation, removal, restoration or re-role writes the
user's row for the old and the new role), a NEW
...
`User_t4d_identity` (a display-name change rewrites the identity row).
4d-i backfills both registers from the current orgs truth under the same
gated writer arm as the role register; `platform:verify` compares them
OFFLINE against the orgs tables; P28b/P29b assert both after every
transition shape. The kernel serves them through
`platform_user_holds_role(project, user, role)`,
`platform_user_orchestration_authority(project, user)` (a `pmc` or
`architect` row) and `platform_user_display_name(userId)` — every
...
`platform_user_holds_role(project, actor, 'pmc')` with the owner/admin arm
admitting `pmc` for a membership-less org owner/admin, the token role such an
actor carries — and the account's display name by `platform_user_display_name`,
...
nullable `DecisionConsultation.requestedByRole`, frozen with the row by the
delivered consultation seals (NULL only on legacy rows, which only a `pmc`
could request, so a NULL reads as `pmc` and a trailing 4d-iii arm requires
the column on every new row); 4d-ii's `consultation.request` writes the
role the INSERT seal judged the requester to hold, and the seal's requester
arm judges the frozen role exactly as it judges every other frozen role in
this plan (a value the requester does not hold is refused); the `respond`
emitter reads the frozen column and binds the response intent's `roles` to
it. P38's arm asserts an architect requester's response push carries
...
7. **Effect correspondence — a seal VERIFIES, it never EMITS.** Where the
   ...
   `platform_tx_event`, and the converse refuses the event without its fact
   ...
   'decision.awaiting_countersign')`, the handler's decisive predicate
   ...
   `targetUserIds` EQUAL, as a set, to the active architects
   `phase6_role_holder_user_ids` resolves at commit (a stale re-emit whose
   ...
   payload preserved — and admits `cancelledAt` ONLY as that NULL →
   timestamp write, in the mark's own statement or as the delivered
...
| `DecisionConsultation` (4c) | — | `requestedById` ACTIVE pmc **+ architect (4d)**; consultee ACTIVE at insert | open (`pending`/`change` **+ `awaiting_countersign` (4d)**) AND published | delivered | `decision.consultation_requested` naming the consultation — SEALED in 4d-i in both directions (#561's review round 1, finding 3) | P25/P27/P37 |
| `DecisionConsultationResponse` (4c) | — (UNIQUE per consultation) | responder is the named consultee | the same predicate re-judged at response, cycle-frozen | delivered | `decision.consultation_responded` naming the consultation — SEALED in 4d-i in both directions (#561's review round 1, finding 3) | P23/P25/P27/P37 |
...
- **4d implementation follows as FOUR PRs — the dark migration 4d-i, the
  ...
  - **4d-i, the migration unit — MIGRATION-ONLY in the template's sense**:
    ...
    then memberships, under their seals disabled by name), the
    ...
    finding 1); the widened `phase6_t4b2_decision_seal` (its role arms),
    ...
    partial apply retries. `TRUNCATE_SEALS` gains FOURTEEN entries across
  ...
  - **4d-iii, the reservation retirement**: a migration-only unit that drops
    ...
    INSERT-time seals — `sourceCommandId` AND the frozen
    ...
    open for a standard request), `Notification.eventId` AND
    ...
    `decision.approved`/`reapproved` event and `requestedByRole` on every
    new `DecisionConsultation` row, installs the THREE orgs-owned
