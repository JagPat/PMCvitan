...
This document REPLACES PR #561 (`Replaces: #561` — labelled
...
having closed before it could be labelled). #561 was the SEVENTH outing
of the NARROWED plan: it drew seven findings on its first head
(`b2e556c3`, folded on its one correction `d5646595`) and nine more on
that head, and closed at the limit. #561 had replaced #560 (the sixth
...
7. **Effect correspondence — a seal VERIFIES, it never EMITS.** Where the
   ...
   `platform_role_holder_user_ids` resolves at commit (a stale re-emit whose
...
- **4d implementation follows as FOUR PRs — the dark migration 4d-i, the
  ...
  - **4d-i, the migration unit — MIGRATION-ONLY in the template's sense**:
    ...
    `platform_role_standing_apply`; the kernel reads
