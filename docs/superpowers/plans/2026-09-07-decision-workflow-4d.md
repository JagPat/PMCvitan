...
**BOARD DECISION 2026-09-08 — the human drain attestation is LIFTED, in
the Board's own words, and THIS head folds it** (#482 comment 5577872836:
posted under the owner account with no coordinator marker, naming the
choice; it supersedes the default at comment 5569586836 and every decline
that cited it — #558, #560, #562, #563, #564, #565, #566, #567 and #568's
round 1). How the decision was reached is part of the record. A GitHub
Watch relay under the shared owner login (#482 comment 5577525644, nudged
to #566 as comment 5577525720) first presented it as agent prose; this
lineage's session folded that relay on #566's head `1a2ba97`; Delivery's
hold (#482 comment 5577732007) established that no direct user decision
stood behind the relay, and the fold was reverted byte-for-byte on head
`5921f22` — a coordinator relay cannot lift a gate the user retained, and
the session said so on #566 and #482 naming what would: the Board's own
unmarked words. The Board then decided directly at comment 5577872836. The
...
comment — both re-verified by 4d-iii's migration preflight; an operator
attestation is ACCEPTED as additional evidence and never required. The
runner clears the directive itself when both pieces of evidence hold, and
...
**Review round 2 on #572 (head `e2fd243e`) — two findings, both P1, folded
here, none dropped.** One is a dependency artifact my own push carried and is
not about the plan at all; the other is this unit's recurring shape once more —
a closed enumeration that describes one branch of a row governing two:

| finding | where it lands | the answer |
|---|---|---|
| 1 (P1) `e2fd243e` added a tracked root `node_modules` symlink (mode `120000`) pointing at a machine-local path; on a runner it dangles and `pnpm install --frozen-lockfile` fails at `mkdir node_modules` with ENOTDIR, so `automation`, `web`, `api`, `e2e` and `api-e2e` all failed before a test body ran | not the plan — the repository's ignore rules | removed on `8acf5862`, and the recurrence prevented rather than just repaired: `.gitignore` carried `node_modules/`, and a TRAILING SLASH matches a directory only, so a symlink of that name was untracked-but-not-ignored and `git add -A` staged it. It is now `node_modules`, which still ignores the real directory at any depth. Reproduced both ways in a disposable repository before the change |
| 2 (P1) the `ChangeRequest` origin row gave actor standing only to an ACTIVE architect under an ACTIVE chain or the resolving pmc, and named the awaiting subject as its eligibility — but the SAME row governs `origin = 'standard'`, which `requestChange` opens from `approved` and which the delivered `decision.change` policy permits to `pmc`, `client`, `contractor`, `engineer` and `consultant`; an implementation following the table would refuse an active engineer's ordinary change request once the frozen-pair checks came on | §A.3 obligation 7 (the fact table's `ChangeRequest` row); P33 | the row states BOTH branches: the rejection branch unchanged, and `standard` carrying the delivered role set with `approved → change` eligibility. P33 gains the positive arm — a non-PMC caller of each delivered role opening a standard request through the SHIPPED service after the trailing seals and COMMITTING with its frozen pair and receipt — because every existing arm on this row proves only that something is refused, which a rule too narrow to admit the legitimate caller also satisfies |
...
7. **Effect correspondence — a seal VERIFIES, it never EMITS.** Where the
   ...
   event-commit interleaving against an inactive row included;
...
| `ChangeRequest` origin (4d) | `countersign_rejection` ⟷ the exact `awaiting_countersign → change` transition, AND with its producer's fact (P33b); `standard` ⟷ the exact `approved → change` transition, in BOTH directions (#568's review round 1, finding 3) | PER ORIGIN. `countersign_rejection`: `requestedById` an ACTIVE architect under an ACTIVE chain, or the resolving pmc; frozen role + name judged. `standard`: the DELIVERED `decision.change` role set (`pmc`, `client`, `contractor`, `engineer`, `consultant` — `packages/shared/src/domain/policy.ts`), frozen role + name judged against the caller's live standing exactly as elsewhere (#572's review round 2, finding 2: one row governs both origins, and stating only the rejection branch's standing would have made an active engineer's ordinary change request unimplementable the moment the frozen-pair checks came on) | PER ORIGIN. `countersign_rejection`: the awaiting subject. `standard`: `approved → change`, which is what `requestChange` already opens from | REQUIRED for `countersign_rejection`; admissible NULL on `'standard'` rows only until 4d-iii | `decision.change_requested` + `change_requested`, + the change-request notice for the `countersign_rejection` origin ONLY — the standard request's transition owes none (the delivered path writes none; the notice arm is keyed on the paired request's `origin`) | P29b/P33/P33b |
...
- **4d implementation follows as FIVE PRs — the dark migration 4d-i, the
  dark SERVER unit 4d-ii-a, the CLIENT unit 4d-ii-b, the autonomous drain
  gate, and the trailing reservation retirement 4d-iii — each honouring the
  ...
  - **4d-i, the migration unit — MIGRATION-ONLY in the template's sense**:
    ...
    `DomainEvent_t4d_pairing_claimed` seal — with the orgs-owned
    `MembershipTransition` seal and every decisions-owned fact seal of the
    §A.3 table CLAIMING their events, no peer-owned trigger on the kernel
    table (#568's review round 1, finding 2); the reset protocol in
  ...
  - **4d-ii, the service/role/UI unit — SPLIT into 4d-ii-a, the SERVER
    ...
    the five reservation doors stand, every shape the `countersign-v1`
  ...
  - **The drain gate — autonomous and fail-closed** (BOARD DECISION
    2026-09-08, #482 comment 5577872836 — the Board's own words — superseding
    the default at comment 5569586836): the directive
    `phase-6-4d-previous-release-drained` states that every process older
    than the 4d-ii-a release (the server release carrying the bumped
    consumer contracts; 4d-ii-b is client-only) is stopped or drained, and
    it CLEARS on the two pieces of trusted autonomous evidence below, both
    verified fail-closed by the runner — an operator's `OPERATOR-ATTESTATION`
    on the controlling issue is ACCEPTED as additional evidence and never
    required. The evidence covers processes ALREADY RUNNING, which the code
    of those processes cannot observe about itself; the bumped consumer
    contracts fence every process that STARTS, which keeps the drained
    state durable afterwards; browser tabs stand behind the
    `countersign-v1` boundary (§A.2). **What 4c decided and what 4d decides
    differently**: 4c (2026-08-29, on PR #480) kept an operator-declared
    directive with NO automated drain actor; 4d keeps "no actor" — nothing
    here drains anything — and, on the Board's 2026-09-08 decision, lets the
    runner clear the directive on evidence it can verify. Why the drain
    ...
    a review finding cannot lift a recorded Board decision, so the plan
    declined it on the default recorded at #482 comment 5569586836 and put
    the contradiction with `AGENTS.md` L72–75 to the Board at comment
    5575748015; a relayed "Board GO" at comment 5577525644 was folded and
    reverted for want of direct provenance; the Board answered in its own
    words on 2026-09-08 at comment 5577872836 — the human attestation is
    lifted and this change authorized — and the user permitted the fold in
    the session). Two pieces of trusted autonomous evidence, both shipped in
    4d-ii:
    ...
    This is the Board's 2026-09-08 decision in its own words (#482 comment
    5577872836), superseding the default at comment 5569586836; it stands
    until the Board decides otherwise, and this plan neither widens it (no
    actor drains anything; no evidence is asserted in prose) nor narrows it
    (no human step is reintroduced).
