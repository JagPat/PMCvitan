...
| 2 (P2) [MOVED TO #580] round 8's own answer — `(consumer, seq)` uniqueness with `ON CONFLICT DO NOTHING` under the catalog row lock makes a repeated activation "a no-op by construction" — is false. The head-lock trigger requires `NEW.seq = activationSeq + 1`, so a retry re-deriving the sequence appends a SECOND fact and a retry resending its original sequence is refused as stale before `ON CONFLICT` is reached | §A.2 the activation register; the round-8 ledger row above; P29b | the operator asserts the STATE, not a sequence: `seq` leaves the operator surface entirely, and under the same row lock the command either observes the mirror already at the requested `active` and appends nothing, or appends at the derived next sequence. The trigger's check stays as the floor under DIRECT writers |
...
### Review round 12 (head `d443aa18`) — five findings; two EXTRACTED to #580, three open here

Recorded rather than folded, because JagPat directed a change of approach from
per-finding correction to closing the underlying gaps, and this round is the
evidence for it: three of the five are this plan's OWN previous fixes failing.

| finding | disposition |
|---|---|
| 1 (P1) the closure UPDATE arm fires only when `resolvedById` goes NULL → non-NULL, so a writer sets `withdrawn` while LEAVING it NULL and never reaches round 11's origin prohibition — the invalid closure commits without provenance and strands the decision in `change` | OPEN here. Round 11's own fix, placed inside an arm gated on a column the writer simply does not set |
| 2 (P2) the baseline backfill covers pre-existing rows, but `decisions.effects` is registered after it and has no head | **EXTRACTED to #580**, fixed there (backfill runs last) |
| 3 (P1) resolving the WHOLE frozen pair from the registers breaks the window rule: a membership-less owner/admin has no fanned-out `pmc` row during 4d-i → 4d-iii, so a legitimate command cannot resolve its role | OPEN here. Round 11's own fix, over-reached — the NAME comes from `UserIdentity` under lock, the ROLE must keep the race-free derivation until 4d-iii |
| 4 (P2) the state-only no-op undoes a later operator's intent after a lost response | **EXTRACTED to #580**, fixed there (stable request token) |
| 5 (P2) the `MembershipTransition` append-only seal has no project-cascade exception, so the claimed hard delete of an event-free project hits it | OPEN here |

**The split, and why here.** #580 is unit 1 of four: the activation register,
which nothing else depends on and which everything else depends on. This plan
keeps units 2–4 in dependency order — kernel pairing + registers, then
`ChangeRequest` provenance + closure authority, then the decision workflow
states. No PR was closed or recreated, no review count reset, and every finding
above keeps its round and its number.
...
installs, and withdraw somebody else's request. So the closure UPDATE arm of
`ChangeRequest_t4d_provenance_required` gains the predicate: for a
...
7. **Effect correspondence — a seal VERIFIES, it never EMITS.** Where the
   ...
   **And the closure carries its OWN receipt, because the row's only
   ...
   trailing seal whenever `resolvedById` is present.
   ...
   **ITS WRITERS ARE DERIVED FROM THE COLUMN IT QUALIFIES, NOT LISTED**
   ...
   So the rule is stated over the column: **`resolvedByCommandId` is owed by
   EVERY command that writes `resolvedById`, and a command that closes a
   request without reserving its own receipt is a defect.** Derived today, as
   ...
   **AND RECORDING A RESOLVER OWES THE WHOLE CLOSURE SET, NOT WHICHEVER PART A
   ...
   in its own unit, `docs/superpowers/plans/2026-09-09-outbox-consumer-activation.md`
   (PR #580), extracted from this plan on JagPat's instruction as unit 1 of
   four.** That unit owns the register, its seals, its baseline backfill, the
   mirror's sole writer, and the `outbox:consumer` operator protocol with its
   retry identity; it carries forward every finding this material has drawn
   (#560 rounds 1–2, #561 round 1, and #572 rounds 8, 9, 10 and 12), including
   the two current-head findings of round 12 that land there. **THIS PLAN
   DEPENDS ON THAT UNIT AND MUST NOT LAND BEFORE IT**: the obligation set below
   is the ACTIVE set, and the ACTIVE set is what that register defines. Nothing
   about the register is restated here, so the two cannot drift.
...
- **4d implementation follows as FIVE PRs — the dark migration 4d-i, the
  ...
  - **4d-iii, the reservation retirement**: a migration-only unit whose
    ...
    seals — `ChangeRequest_t4d_provenance_required`, in TWO ARMS because this
    table is written by two different OPERATIONS and an INSERT-time seal
    reaches only one of them (#572's review round 8, finding 2). **The INSERT
    arm**: `sourceCommandId` AND the frozen
    `requestedByRole`/`requestedByName` pair required on every new
    `ChangeRequest` row whatever its origin. **The UPDATE arm**: whenever
    `resolvedById` goes from NULL to non-NULL — which is what a CLOSURE is,
    for the withdrawal and the re-approval alike — the COMPLETE closure set of
    §A.3 must be present in the same statement (`resolvedByCommandId` and the
