...
This document REPLACES PR #556 (`Replaces: #556` — labelled
...
having closed before it could be labelled). #556 was the THIRD outing of
the NARROWED plan: it drew three findings on its first head (`bb307c74`,
folded on its one correction head `dd2da64b`) and five more on that head,
and closed at the limit. #556 had replaced #555 (the second outing: five
findings on `3e5a85a2` folded on `41ea41d2`, four more there), #555 had
replaced #554 (the first outing: seven findings on `cbfdaacb` folded on
`82a497a5`, seven more there), and #554 had replaced #552, the fifteenth
replacement of the ORIGINAL docs-only unit: #537 → #538 → #539 →
...
requester standing at the time of the request, `pmc` or `architect` — so
the immutable intent and the claim-time predicate agree; P38's arm asserts
an architect requester's response push carries `roles: ['architect']` and a
PMC requester's is byte-identical to today's. The cycle
...
`phase6_role_holder_user_ids(project, role)` carries the SAME org-owner arm
...
sees every committed project. The lock order org → project is the ONLY
...
7. **Effect correspondence — a seal VERIFIES, it never EMITS.** Where the
   ...
   `decision.reapproved` — tied to the exact revision the event's payload
   names, EITHER a same-transaction `DecisionApprovalRevision` (the direct
   no-chain approve inserts one — the delivered approve writes it
   in-transaction, so the previous release is compatible through the drain)
   OR, since a countersign and a `completed` stranded resolution FLIP the
   existing provisional revision's `finalized` and insert no row (#557's
   review round 1, finding 1), a same-transaction `DecisionCountersign`
   fact or a `completed` `DecisionStrandedResolution` fact naming that
   revision, whose `finalized` flipped in this transaction (the row's `xmin`
   current and `finalized = true`) — one of the three, never none, never two; `decision.change_requested` — a
   ...
   **The kernel seals its own envelope, so a module seal never has to** (this
   ...
   row whose `retiredAt` IS NULL, that row's `eventType` equals `NEW."eventType"`, its `invalidate`
...
- **4d implementation follows as FOUR PRs — the dark migration 4d-i, the
  ...
  - **4d-ii, the service/role/UI unit**: the role fan-out (§A.1 — every
    ...
    actual role; `lockOrgStanding` taken by project creation and by every
    owner/admin `OrgMembership` writer before the project keys; the kinded
  ...
  - **4d-iii, the reservation retirement**: a migration-only unit that drops
    ...
    7's last row), installs the two orgs-owned STANDING-WRITER seals —
    `Membership_t4d_readiness` on every `Membership` row write and
    `OrgMembership_t4d_readiness` on every owner/admin `OrgMembership` write
    trying `phase6_try_org_readiness` and then the org's project keys
    ascending (§A.2 the push families — after the drain, so no
    legacy two-write path ever meets them), RETIRES the pre-4d-ii coverage
