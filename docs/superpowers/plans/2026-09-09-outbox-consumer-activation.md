...
| P-A7 | the WHITESPACE CHECK at the database boundary: a raw `INSERT` whose `reason` is a single space, a tab, a vertical tab, a form feed, a carriage return or a newline is refused, one arm of the CHECK removed at a time; the same for `actorId`; **and the same for `requestToken`, including the empty string** — the round-4 contract put the token under this discipline and said it joins this probe, and the probe exercised only the other two strings (#572's review round 25, finding 1). That gap is not cosmetic: `requestToken` is the RETRY IDENTITY, so a migration that omits or weakens its CHECK while zod stays correct leaves every stated probe green and lets a direct writer persist `''` or a tab as a permanent identity that unrelated requests then collide with on `(consumer, actorKind, requestToken)`. Its CHECK arm is mutation-tested independently of `reason`'s and `actorId`'s, so removing any ONE of the three is RED on its own row; and the zod layer refused independently | a `btrim()`-only check, which passes every zod probe and still persists a tab-only reason from a direct writer (#580's review round 1, finding 10) |
