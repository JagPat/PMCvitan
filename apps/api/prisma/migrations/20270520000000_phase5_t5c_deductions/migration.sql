...
-- §H/§F (Codex P2) — `NET_PAYABLE(bill)` in SQL, so the status shadow rule below can ask the FOLD
-- rather than merely "does a certificate exist". Task 6 subtracts its approval and payment folds
-- here, in this one function, rather than teaching the trigger a second source of truth.
CREATE OR REPLACE FUNCTION phase5_t5c_net_payable(p_project text, p_bill text)
RETURNS numeric AS $$
DECLARE
  v_cert      text;
  v_certified numeric;
...
BEGIN
  ...
  IF v_cert IS NULL THEN RETURN NULL; END IF;

  SELECT COALESCE(SUM(d."amount"), 0) - COALESCE((
           SELECT SUM(r."amount") FROM "BillDeductionRelease" r
            JOIN "BillDeduction" d2 ON d2."projectId" = r."projectId" AND d2."id" = r."deductionId"
            WHERE d2."projectId" = p_project AND d2."certificateId" = v_cert
         ), 0)
    INTO v_withheld
    FROM "BillDeduction" d
   WHERE d."projectId" = p_project AND d."certificateId" = v_cert;
...
-- §B (Codex P2) — the mover VOCABULARY gains the two acts §H introduces. `raisedBy` is the durable
-- explanation a human reads months later, so it must name what actually moved: a release-raised
-- exception labelled `claim` sends a PMC hunting for a vendor claim that never changed.
...
BEGIN
  ...
  IF NEW."status" IN ('certified', 'paid') AND OLD."status" NOT IN ('certified', 'paid') THEN
    ...
    END IF;
  END IF;
  -- Codex P2 — and `paid` is the projection of a FOLD, not merely of a certificate. Opening the
  -- arrow without checking it would let bypass SQL mark an ordinary ₹100 payable settled: the
  -- lifecycle would accept the arm and the projection check would accept the live certificate while
  -- `NET_PAYABLE` still read ₹100. §F says the status IS the derivation, so the database asks the
  -- derivation. At this tree `PAID` is structurally zero (Task 6 ships payment rows), so the
  -- condition is `NET_PAYABLE = 0`; Task 6 extends `phase5_t5c_net_payable` and this comparison
  -- follows it rather than growing a second rule.
  IF NEW."status" = 'paid' AND OLD."status" IS DISTINCT FROM 'paid' THEN
    IF COALESCE(phase5_t5c_net_payable(NEW."projectId", NEW."id"), -1) <> 0 THEN
      RAISE EXCEPTION 'A bill is `paid` because NOTHING REMAINS PAYABLE, not because a status says so — net payable is % (%)', COALESCE(phase5_t5c_net_payable(NEW."projectId", NEW."id")::text, '(no live certificate)'), OLD."id";
    END IF;
  END IF;
  RETURN NEW;
END;
