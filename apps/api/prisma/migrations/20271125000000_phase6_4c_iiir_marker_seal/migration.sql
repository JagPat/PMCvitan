...
BEGIN
    ...
    FROM (
      SELECT e.fn,
             CASE
               ...
               -- The body is not all of what a function DOES. proconfig carries per-function GUC
               -- settings applied on entry, so ALTER FUNCTION ... SET
               -- vitan.phase6_4c_iiir_repair = 'on' makes the gate's current_setting return 'on'
               -- for every invocation, including a forged INSERT, while md5(prosrc) is unchanged.
               -- Without this the adoption test could permanently bless a marker written through
               -- exactly that hole. SECURITY DEFINER and a changed language are the same class.
               WHEN p.proconfig IS NOT NULL
                 THEN e.fn || ': the function carries execution settings this migration never'
                        || ' installs (proconfig=' || p.proconfig::text || ') - they change what it'
                        || ' does without touching its body'
               WHEN p.prosecdef
                 THEN e.fn || ': the function is SECURITY DEFINER, which this migration never installs'
               WHEN p.prolang <> (SELECT oid FROM pg_language WHERE lanname = 'plpgsql')
                 THEN e.fn || ': the function language is not plpgsql'
  ...
  IF v_unenforced <> '' THEN
    RAISE EXCEPTION
      ...
      'same transaction. THEN, BEFORE REDEPLOYING, run `prisma migrate resolve --rolled-back '
      '20271125000000_phase6_4c_iiir_marker_seal`: this file aborts inside its own transaction so '
      'the schema is untouched, but `migrate deploy` still RECORDS the attempt as failed, and the '
      'next deploy stops at P3009 instead of re-running this migration - the operator would be '
      'stuck at exactly the moment this message is meant to help. Only then redeploy and let the '
      'repair earn a new marker. See docs/RUNBOOK.md section P64CIIIR.',
