...
BEGIN
  -- Codex round-6 P2 — LOCK the certificate before deciding it is open. An unlocked read is a
  -- decision about a row another transaction may be changing: this trigger fires at COMMIT, so an
  -- appending transaction could read the certificate live, a concurrent supersession could stamp
  -- and commit, and the completeness check — which returns early for history — would then wave the
  -- append through onto a certificate that had become history in between. Two predicates each
  -- individually correct, admitting a state neither would allow.
  --
  -- `FOR UPDATE` makes supersession and late evidence serialize on the certificate row: whichever
  -- reaches it first, the second sees the committed truth and one of the two is refused. It is the
  -- same correction round 4 made to the quantity bound, at the seal round 5 added — which is why it
  -- is stated here rather than left to the reader to infer from the other one.
...
--   * do the same as the evidence RECORDER — every per-row trigger passes, because none of them
--     knows who certified;
...
BEGIN
  ...
  -- (c) §I — the actor who recorded the evidence a certificate rests on may NOT certify it.
  --
  -- This unit states the RULE. The attributable override that lets a two-person practice operate
  -- anyway is the next unit's (§I's `SodException`, its approver-standing predicate and the
  -- biconditional that admits exactly one exception per rule). The refusal ships FIRST on purpose:
  -- until the override exists this seal is strictly more restrictive than the finished rule, so no
  -- intermediate state of the system permits an act the finished rule would refuse. Shipping the
  -- override first, or shipping neither, would both open a window; shipping the refusal first
  -- cannot.
  --
  -- The actor set comes from `phase5_t5_evidence_actors` — the SAME function
  -- `CommercialCertificationService` calls, over the same frozen rows, so the two layers cannot
  -- answer this differently. That single authority is the closure this PR's convergence audit
  -- names: the question had been implemented twice and only the copy a finding named ever moved.
  SELECT EXISTS (
    SELECT 1 FROM phase5_t5_evidence_actors(p_project, p_certificate) a WHERE a.actor = v_certifier
  ) INTO v_recorder;
  IF v_recorder THEN
    RAISE EXCEPTION 'Certificate % was certified by %, who recorded evidence it rests on — §I refuses the act', p_certificate, v_certifier;
  END IF;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  SELECT (SELECT COUNT(*) FROM "BillCertificate")
       ...
       INTO v_rows;
  IF v_rows <> 0 THEN
    RAISE EXCEPTION 'phase5_t5: expected the three new tables to upgrade row-free, found % row(s)', v_rows;
  END IF;
END $$;
