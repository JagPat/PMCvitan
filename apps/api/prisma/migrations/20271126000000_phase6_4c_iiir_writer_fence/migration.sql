...
-- TRUNCATE FIRES NO ROW TRIGGER (Codex on `de9fa3b7`) — measured: the register emptied and every
-- generation stayed unfenced, so a caught-up generation went on serving an authoritative EMPTY
-- decisions register. The marker seal has carried a TRUNCATE guard for this exact reason since it
-- was written; this fence was built without one.
--
-- It STAMPS rather than REJECTS, unlike the marker seal's guard, and the difference is deliberate:
-- `OutboxOperatorAction` is never legitimately truncated, while `DecisionProjection` is — the seed
-- and every integration harness wipe it. In all of them it is truncated in the SAME statement as
-- `ProjectionGeneration`, so the stamps this writes are removed with the generations that carry
-- them and no harness needs to change; a truncate of the projection rows ALONE, which is the
-- hazard, leaves the stamps behind exactly as it should.
...
DROP TRIGGER IF EXISTS "DecisionProjection_4c_iiir_writer_fence_truncate" ON "DecisionProjection";
CREATE TRIGGER "DecisionProjection_4c_iiir_writer_fence_truncate"
BEFORE TRUNCATE ON "DecisionProjection"
...
BEGIN
   ...
   WHERE ns.nspname = 'public'
     AND ((c.relname = 'DecisionProjection'
       ...
       OR (c.relname = 'DecisionProjection'
           ...
           AND t.tgtype = 34)                       -- BEFORE(2) + TRUNCATE(32), per statement
  ...
  IF n <> 3 THEN
    RAISE EXCEPTION
      '4c-iii-r writer fence did not install (found % of the 3 required triggers). The deploy is refused rather than starting with an unfenced — or an unsealed — decisions.inbox register.', n;
  END IF;
END
$verify$;
