...
--   tgisinternal         CHECKED  filtered out: the five foreign keys' internal triggers share
...
BEGIN
  ...
  -- ── 1g'. THE INSTALL BARRIER, compared by DEFINITION rather than found by NAME ──────────────
  -- The barrier is section 2's proof that an unfinished install cannot be written at all, and this
  -- block is the only thing that reads that proof. Asked as `is a constraint of that name present?`
  -- it proves nothing, because a name is not a rule — the defect this file has now met six times.
  --
  -- Concretely, and reproduced before this was written: an autocommit retry meets an empty
  -- incomplete table whose same-named barrier is a hollow `CHECK (TRUE)`. Presence-only, this
  -- block concludes "unfinished but unwritable", lets the resume proceed, and RELEASES ITS LOCK at
  -- the end of this statement. A second session then inserts an already-revoked edge — accepted,
  -- because the barrier judges nothing — and the run goes on to arm every seal over it. Arming a
  -- trigger validates nothing already in the table, so the fabricated withdrawal is permanent and
  -- the no-delete seal now protects it. Measured: exit 0, five seals armed, the row surviving.
  --
  -- So the three states are the same three every other object gets:
  --   ABSENT                     -- either a finished install, or a partial from before the
  --                                 barrier existed; the rest of this block decides which.
  --   PRESENT and CANONICAL      -- this install has not finished, and the table really is shut.
  --   PRESENT and DIFFERENT      -- REFUSE. A barrier of the right name and the wrong definition
  --                                 is not a barrier: the table was open the whole time this file
  --                                 believed it was closed, so nothing here can be said about what
  --                                 reached it. `convalidated` travels too — a `NOT VALID` CHECK
  --                                 judges only what arrives next, which is not what a barrier is.
  SELECT regexp_replace(pg_get_constraintdef(k.oid), '[[:space:]]+', ' ', 'g')
         || CASE WHEN k.convalidated THEN '' ELSE ' [NOT VALID]' END
    INTO v_barrier
    FROM pg_constraint k
  ...
  IF v_barrier IS NOT NULL THEN
    IF v_barrier = 'CHECK ((id !~ ''^''::text))' THEN
      v_missing := COALESCE(v_missing || ', ', '') || 'the install barrier is still in place';
    ELSE
      v_wrong := COALESCE(v_wrong, format('"ActivityDependency" carries a constraint named "ActivityDependency_install_incomplete_check" that is NOT the install barrier this migration writes: present as %s, expected CHECK ((id !~ ''^''::text)). That name is the ONLY thing standing between a half-installed dependency graph and a writer, so a same-named constraint that judges anything less means this table was writable while its seals were absent — and nothing here can say what reached it. Drop that constraint and the incomplete table, or restore the barrier verbatim, then re-run.', v_barrier));
    END IF;
  END IF;

  -- ── 1g''. A VALIDATED FOREIGN KEY IS NOT AN ENFORCED ONE ────────────────────────────────────
  -- Every containment key above was compared by deparse, by `convalidated` and by `confrelid` —
  -- and all three can be right while the key enforces NOTHING. A foreign key is implemented as
  -- internal triggers, and `ALTER TABLE ... DISABLE TRIGGER ALL` — which a failed restore, a bulk
  -- load or any superuser maintenance may leave behind — switches them off without touching the
  -- catalog row. `pg_get_constraintdef` still deparses identically, `convalidated` is still true,
  -- and ordinary inserts then store nonexistent endpoints and unknown authorship.
  --
  -- Reproduced before this was written: seals re-enabled by name, the internal triggers left
  -- disabled, replay exit 0 — and an edge naming a nonexistent activity AND a nonexistent creator
  -- committed. Containment gone; every check this file had said yes.
  --
  -- Asked through `tgconstraint`, so it covers BOTH sides of each key without naming triggers this
  -- file did not create: the check triggers on `ActivityDependency` (which refuse a bad endpoint on
  -- INSERT) and the action triggers on `Project` / `Activity` / `Membership` (which carry the
  -- ON DELETE RESTRICT that stops a project being deleted out from under its edges).
  --
  -- This is WRONG rather than MISSING, and deliberately: re-enabling a disabled enforcement trigger
  -- validates nothing that arrived while it was off, exactly as arming a seal validates nothing
  -- already in the table. Repairing it here would be certifying rows this file never judged.
  SELECT string_agg(DISTINCT k.conname, ', ' ORDER BY k.conname) INTO v_bad
    FROM pg_constraint k
    JOIN pg_trigger g ON g.tgconstraint = k.oid
   WHERE k.conrelid = 'public."ActivityDependency"'::regclass
     AND k.contype = 'f'
     AND g.tgenabled <> 'O';
  IF v_bad IS NOT NULL THEN
    v_wrong := COALESCE(v_wrong, format('the containment key(s) %s are present and validated but NOT ENFORCED: an internal trigger implementing them is disabled (tgenabled <> ''O''), which is what `ALTER TABLE ... DISABLE TRIGGER ALL` leaves behind. The catalog row is unchanged, so the deparse and `convalidated` both read correct while ordinary inserts can store nonexistent endpoints and unknown authorship. This migration will not re-enable them, because re-enabling validates nothing that arrived while they were off. Re-enable them deliberately (ALTER TABLE "ActivityDependency" ENABLE TRIGGER ALL), reconcile any rows written meanwhile, then re-run.', v_bad));
  END IF;
...
BEGIN
  ...
  SELECT string_agg(x.what, ', ' ORDER BY x.what) INTO v_missing FROM (
    ...
    UNION ALL
    -- ENFORCEMENT, not merely validation. A foreign key is internal triggers; disable them and the
    -- catalog row is untouched while nothing is checked. Asked through `tgconstraint` so it covers
    -- the check triggers on this table AND the action triggers on each referenced relation, and
    -- asked HERE as well as in section 1 because the barrier must not be lifted over a graph whose
    -- containment is switched off — including on the FRESH-install path, where section 1 returned
    -- before it could ask anything.
    SELECT 'enforcement enabled for constraint "' || k.conname || '"'
      FROM pg_constraint k
      JOIN pg_trigger g ON g.tgconstraint = k.oid
     WHERE k.conrelid = 'public."ActivityDependency"'::regclass
       AND k.contype = 'f' AND g.tgenabled <> 'O'
  ) x;
  ...
  -- THE BARRIER IS DROPPED BY DEFINITION TOO. `DROP CONSTRAINT IF EXISTS` matches on the NAME, so
  -- if something of that name is present and is NOT the barrier this file installed, dropping it
  -- would silently remove somebody else's rule as the final act of a successful migration. Section
  -- 1 already refuses that state on the resume path; this is the fresh-install path's guard, and it
  -- costs one lookup.
  IF EXISTS (SELECT 1 FROM pg_constraint k
              WHERE k.conname = 'ActivityDependency_install_incomplete_check'
                ...
                AND (regexp_replace(pg_get_constraintdef(k.oid), '[[:space:]]+', ' ', 'g')
                       <> 'CHECK ((id !~ ''^''::text))'
                     OR NOT k.convalidated)) THEN
    RAISE EXCEPTION 'schedule B1: a constraint named "ActivityDependency_install_incomplete_check" is present at the end of this migration but is not the install barrier this file writes, so it will not be dropped — dropping it would remove a rule this migration did not install. Procedure: docs/RUNBOOK.md section B1.';
  END IF;
