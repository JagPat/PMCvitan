...
--
-- THE ROUND THAT PRODUCED THIS PARAGRAPH FOUND TWO MORE, and both landed where the sweep had not
-- looked rather than on a judgement it had recorded — which is the outcome the sweep was built to
-- produce, and is worth stating plainly rather than quietly repairing:
--
--   * the INSTALL BARRIER was checked by PRESENCE, by the one part of this file that never got
--     the treatment the CONSTRAINT entry below prescribes, because the barrier was not IN the
--     constraint list. A rule that is applied to a list rather than to a kind of object has a hole
--     the exact size of whatever the list omits. It is now in the list.
--   * the five FOREIGN KEYS were checked by definition AND by target OID AND by `convalidated` —
--     three attributes, all of them the right ones for "is this the key I meant" — and not by
--     whether the internal triggers that IMPLEMENT them are switched on. Every attribute that
--     names the object can be right while the object does nothing.
--
-- So the question below is asked in the harder form: not "which columns identify this object",
-- but "which catalog state can differ while these checks still pass AND the object stops
-- enforcing". Enforcement is a property of a different catalog from identity, for constraints and
-- triggers alike, and the sweep now says so.
--
...
--   THE DEFINITION,      CHECKED  IN SECTION 9 AS WELL, which is where it was missing. Section 1e
--   in section 9 too              compares all ten by `pg_get_constraintdef`, but only on the
--                                 RESUME path — a fresh install returns at the existence test
--                                 without reaching it, and once the migration is recorded nothing
--                                 re-reads section 1 ever again. Section 9's arm asked for a
--                                 constraint of each NAME that was `convalidated`, and a CHECK is
--                                 the one seal here where that is not enough: `CHECK (true)` is a
--                                 perfectly valid, validated CHECK. MEASURED: swap it in for
--                                 `ActivityDependency_attribution_check` and the inventory passed,
--                                 the deploy-time verifier reported sealed, and an edge whose
--                                 `createdByName` was three spaces — an attribution answerable to
--                                 nobody — was accepted and then frozen. Section 9 is the arm that
--                                 runs in BOTH places (it gates the barrier, and the verifier
--                                 extracts and re-executes it), so the definitions belong there.
--   ENFORCEMENT          CHECKED  for the five FOREIGN KEYS, in section 1e'' and again in section 9,
--   (each internal              ONE EXPECTED TRIGGER AT A TIME. A foreign key is not
--    `RI_ConstraintTrigger`     self-executing: PostgreSQL implements it as internal
--    row, by `tgfoid`,          `RI_ConstraintTrigger` rows — MEASURED on PostgreSQL 16.13, FOUR
--    `tgtype`, `tgisinternal`   per key: `RI_FKey_check_ins` and `RI_FKey_check_upd` on the
--    and `tgenabled`)           referencing table, and the ON UPDATE and ON DELETE actions on the
--                               referenced one. `ALTER TABLE ... DISABLE TRIGGER ALL`, which a
--                               failed restore runs, sets all four to 'D'; MEASURED, the deparsed
--                               definition is then BYTE-IDENTICAL and `convalidated` is STILL
--                               true, so every check above accepts a key that enforces nothing.
--                               THE PREVIOUS HEAD ASKED THIS PER SIDE — "is there a trigger here,
--                               and is it on" — WHICH WAS STILL TOO FEW ATTRIBUTES. Delete only
--                               `RI_FKey_check_upd` for `ActivityDependency_revokedBy_fkey` and
--                               `RI_FKey_check_ins` answers the side-level EXISTS: MEASURED, this
--                               file re-ran and exited 0, then an UPDATE set `revokedById` to a
--                               member that does not exist and COMMITTED, and section 6 made that
--                               forged attribution permanent. So all twenty are enumerated by
--                               name. The expected function is stated LITERALLY rather than
--                               derived from `confupdtype`/`confdeltype`, which makes the
--                               referential ACTIONS a pin here too; `tgtype` pins the event each
--                               is wired to; `pg_catalog` resolution rejects a same-named decoy;
--                               `tgisinternal` rejects a user trigger standing in; and EXACTLY ONE
--                               is expected per job. 'O' and 'A' are accepted (both fire for
--                               ordinary writes); 'D' and 'R' are refused.
--   THE INSTALL BARRIER  CHECKED  by DEFINITION and `convalidated`, in section 1g, and this is the
--   (`ActivityDependency_          first of the two. It is a CHECK constraint and was the one
--    install_incomplete_check`)    constraint the list above did not contain, so it was looked up
--                                  by NAME and taken as present-therefore-enforcing. A hollow
--                                  `CHECK (TRUE)` of that name reads as "this table is unwritable"
--                                  while admitting every INSERT — see section 1g for the exact
--                                  interleaving that fabricates an immutable row through it. It is
--                                  compared like every other constraint now; and an install that
--                                  is unfinished with the barrier ABSENT has it RE-ARMED (1i),
--                                  because refusing there would leave the table unguarded.
...
--   session_replication_ EXCLUDED DELIBERATELY, and named here because it is the obvious next
--   role = 'replica'              question after the enforcement check above. It suppresses every
--                                 `tgenabled = 'O'` trigger — the five seals AND the foreign keys'
--                                 internal ones — for the session that sets it. It is NOT catalog
--                                 state: it is a GUC belonging to a session that does not exist
--                                 yet when this file runs, so no check here can observe it and no
--                                 check here should pretend to. What this file does about that
--                                 class is structural instead: the install barrier is a CHECK
--                                 rather than a trigger precisely because `replica` cannot switch
--                                 a CHECK off, and so are the four rules on shape and attribution.
--                                 The seals that ARE triggers remain suppressible by a session
--                                 that sets it, and that is a property of PostgreSQL rather than
--                                 of this install — stated rather than papered over.
--
...
--   tgisinternal         CHECKED  in BOTH directions, and the second half is new. The five SEALS
--                                 are matched with `NOT tgisinternal`, so a foreign key's internal
--                                 trigger can never be mistaken for one of them. The INTERNAL rows
--                                 are then asked SEPARATELY, under CONSTRAINT above: they are the
--                                 five foreign keys' enforcement, which the earlier reading of
...
-- WHEN EACH OF THESE IS ASKED, WHICH IS THE OTHER HALF OF THE SAME LESSON.
--
-- Every line above says WHAT is compared. None of them said WHEN, and the answer used to be "once,
-- while this file runs". That is a real gap and not a theoretical one: `prisma migrate deploy`
-- proves the LEDGER is complete, so the moment `20270930000000` is recorded, nothing re-reads this
-- file — and a database that has since been through a bad restore has a complete ledger and no
-- guards. MEASURED at the previous head: with `ActivityDependency_frozen` and
-- `ActivityDependency_no_delete` DISABLED, `scripts/migrate.sh` exited 0 and reported the deploy
-- good, after which an UPDATE rewrote the immutable evidence row and a DELETE removed it. An
-- object accepted on fewer attributes than determine whether it enforces is the failure this file
-- has spent five rounds on; an object accepted at a moment that has since passed is the same
-- failure in the time dimension.
--
--   AT INSTALL, by this file        sections 1 to 9: adopt-or-abort over what was found, then the
--                                   whole inventory once more over what now exists, and only then
--                                   is the write barrier lifted.
--   ON EVERY DEPLOY, by the         `apps/api/src/activities/b1/b1-seals.ts`, run from
--   compiled B1 verifier            `scripts/migrate.sh` on BOTH success paths. It does not keep a
--                                   second list: it EXTRACTS section 9's inventory from this file
--                                   between the `B1-SEAL-INVENTORY` markers and re-executes it, so
--                                   the two can never disagree. It adds the three questions
--                                   section 9 structurally cannot ask about itself — that the
--                                   install BARRIER is gone (section 9 drops it AFTER the
--                                   inventory, so at that moment it is still there), that each
--                                   function's `prosrc` is still the body this file installed
--                                   (sections 4 to 7 compared it minutes earlier; `CREATE OR
--                                   REPLACE FUNCTION` can replace it minutes later while keeping
--                                   every property section 9 re-asks), and that each function is
--                                   still owned by the table's owner (`pg_restore` sets ownership).
--
-- WHAT THE DEPLOY-TIME VERIFIER DELIBERATELY DOES NOT ASK, and why:
--
--   `proisstrict`,       NOT RE-ASKED  They are pinned at install and are part of the identity
--   `proparallel`,                     `CREATE OR REPLACE FUNCTION` refuses to change, so no
--   `prokind`, `proretset`             restore or in-place replacement can move them. Re-asking
--                                      them would add rows to a deploy report that no operator
--                                      action can produce.
--   anything about ROWS  NOT ASKED     This verifies the guards, and the guards' whole job is that
--                                      the rows are already what they say. A row-level diagnostic
--                                      belongs in a preflight, and this deliberately is not one:
--                                      the migration completes its own install, so there is
--                                      nothing useful to ask BEFORE Prisma runs.
--   the CALLER'S        ASKED, BUT   `transaction_isolation` is a GUC of the calling session too,
--   ISOLATION LEVEL     BY THE SEAL   and therefore invisible to every check in this file — but
--                       ITSELF        unlike `session_replication_role` it is readable AT THE
--                                     MOMENT IT MATTERS, from inside the trigger, and that is
--                                     where it is now asked. The no-truncate seal permits TRUNCATE
--                                     on an EMPTY table, and MEASURED, its emptiness test is not
--                                     trustworthy under a fixed snapshot: the ACCESS EXCLUSIVE
--                                     lock serializes the STATEMENT, not the SNAPSHOT, so a
--                                     REPEATABLE READ transaction that read the table while it was
--                                     empty erased a concurrently committed edge. The seal now
--                                     refuses any isolation level but READ COMMITTED before taking
--                                     that path. Catalog state cannot answer a question about a
--                                     session; the object that acts can.
--   `session_replication_ NOT ASKABLE  Unchanged from the CONSTRAINT note above: it is a GUC of a
--   role`                              session that does not exist yet, not catalog state. The
--                                      structural answer — the barrier is a CHECK, and so are the
--                                      four rules on shape and attribution — is the same at deploy
...
DECLARE
  ...
  v_barrier  TEXT;
  v_barrier_absent BOOLEAN := FALSE;
  v_path     TEXT;
  v_rel      RECORD;
BEGIN
  ...
  -- ── 1e''. AND EVERY ONE OF ITS ENFORCEMENT TRIGGERS MUST BE THERE AND SWITCHED ON ──────────
  -- 1e compared each foreign key's DEFINITION and 1e' compared its TARGET by OID. Neither asks
  -- whether the key ENFORCES, and that is a separate fact stored in a separate catalog.
  --
  -- A foreign key is not self-executing. PostgreSQL implements it as internal `RI_ConstraintTrigger`
  -- rows — MEASURED on PostgreSQL 16.13: FOUR per key, and each one does a different job.
  --
  --   on the REFERENCING table  `RI_FKey_check_ins`   an INSERT may not name a row that is not there
  --                             `RI_FKey_check_upd`   and neither may an UPDATE
  --   on the REFERENCED table   the ON UPDATE action, and the ON DELETE action
  --
  -- `ALTER TABLE ... DISABLE TRIGGER ALL`, which a failed restore and several ordinary superuser
  -- recovery procedures run, sets `tgenabled = 'D'` on all four. MEASURED: after it,
  -- `pg_get_constraintdef` renders the key IDENTICALLY and `convalidated` is STILL true — so 1e,
  -- 1e' and section 9 all accept it — while an ordinary INSERT stores a nonexistent activity, an
  -- endpoint in another project, or a `createdById` that names no membership.
  --
  -- EACH OF THE FOUR IS NAMED AND ASKED FOR SEPARATELY, and that is the correction this round
  -- makes. The earlier head asked one question per SIDE — "is there any trigger here, and is it
  -- on" — and MEASURED, that accepts a key that has lost one of the two. Delete only
  -- `RI_FKey_check_upd` for `ActivityDependency_revokedBy_fkey` (`allow_system_table_mods`, one
  -- DELETE from `pg_trigger`, which is what a catalog repair or a partial restore leaves) and
  -- `RI_FKey_check_ins` still answers the side-level EXISTS: this file re-ran and exited 0, and
  -- then an UPDATE set `revokedById` to a member that does not exist and COMMITTED — after which
  -- section 6 makes that forged attribution permanent. Enforcement is not a property of a SIDE. It
  -- is a property of each trigger, so each trigger is what is compared.
  --
  -- The expected function is stated LITERALLY per key rather than derived from `confupdtype` /
  -- `confdeltype`, which makes the referential ACTIONS a pin as well: a key quietly re-created
  -- `ON DELETE CASCADE` carries `RI_FKey_cascade_del`, and the row below expects
  -- `RI_FKey_restrict_del` — so deleting a project would silently delete its dependency edges, and
  -- that is refused here rather than discovered later. `tgtype` is pinned with it (5 = ROW/INSERT,
  -- 17 = ROW/UPDATE, 9 = ROW/DELETE), so a function cannot be wired to an event other than the one
  -- it is named for; the function is resolved in `pg_catalog`, so a same-named decoy in another
  -- schema is not it; and `tgisinternal` is required, so a user trigger cannot stand in for the
  -- real machinery. EXACTLY ONE is expected: two rows for the same job is not a state PostgreSQL
  -- produces, and a duplicate is a catalog someone has been editing.
  --
  --   'O'  origin — what `ADD FOREIGN KEY` produces, and what enforces ordinary writes. Accepted.
  --   'A'  always — enforces ordinary writes AND replicated ones. STRICTLY STRONGER, so accepted.
  --        (The five SEALS above are held to 'O' exactly, because this file installs those and
  --        adopts nothing else; a foreign key's internal triggers are not nameable objects an
  --        operator manages, so the question here is only whether they fire.)
  --   'D'  disabled — never fires. Refused.
  --   'R'  replica — fires ONLY under `session_replication_role = replica`, which is not how the
  --        application writes. An unenforced key for every real caller. Refused.
  --
  -- BOTH SIDES ARE ASKED, because half the machinery lives on `Project`, `Activity` and
  -- `Membership`: with the referenced side off, `ON DELETE RESTRICT` stops restricting and a
  -- project can be deleted out from under its own dependency edges.
  --
  -- REFUSED rather than re-enabled. `ENABLE TRIGGER ALL` is a one-line repair an operator can run
  -- deliberately, and it re-enables user triggers too — including any this file knows nothing
  -- about. A migration silently switching enforcement back on would also hide the fact that the
  -- database has been through a restore that left it unenforced, which is the thing worth knowing.
  --
  -- A key that is ABSENT is not reported here — 1e refused it already, by definition. This asks
  -- only about the keys that exist.
  SELECT string_agg(x.what, ', ' ORDER BY x.what) INTO v_bad FROM (
      ...
      FROM (
        SELECT k.name, k.side, k.fn, k.tgtype,
          ...
          FROM (VALUES
            ...
            ('ActivityDependency_projectId_fkey',              'referencing side',        'RI_FKey_check_upd',    17::SMALLINT),
            ('ActivityDependency_projectId_fkey',              'referenced ON UPDATE',    'RI_FKey_cascade_upd',  17::SMALLINT),
            ('ActivityDependency_projectId_fkey',              'referenced ON DELETE',    'RI_FKey_restrict_del',  9::SMALLINT),
            ('ActivityDependency_projectId_predecessorId_fkey','referencing side',        'RI_FKey_check_ins',     5::SMALLINT),
            ('ActivityDependency_projectId_predecessorId_fkey','referencing side',        'RI_FKey_check_upd',    17::SMALLINT),
            ('ActivityDependency_projectId_predecessorId_fkey','referenced ON UPDATE',    'RI_FKey_noaction_upd', 17::SMALLINT),
            ('ActivityDependency_projectId_predecessorId_fkey','referenced ON DELETE',    'RI_FKey_noaction_del',  9::SMALLINT),
            ('ActivityDependency_projectId_successorId_fkey',  'referencing side',        'RI_FKey_check_ins',     5::SMALLINT),
            ('ActivityDependency_projectId_successorId_fkey',  'referencing side',        'RI_FKey_check_upd',    17::SMALLINT),
            ('ActivityDependency_projectId_successorId_fkey',  'referenced ON UPDATE',    'RI_FKey_noaction_upd', 17::SMALLINT),
            ('ActivityDependency_projectId_successorId_fkey',  'referenced ON DELETE',    'RI_FKey_noaction_del',  9::SMALLINT),
            ('ActivityDependency_createdBy_fkey',              'referencing side',        'RI_FKey_check_ins',     5::SMALLINT),
            ('ActivityDependency_createdBy_fkey',              'referencing side',        'RI_FKey_check_upd',    17::SMALLINT),
            ('ActivityDependency_createdBy_fkey',              'referenced ON UPDATE',    'RI_FKey_noaction_upd', 17::SMALLINT),
            ('ActivityDependency_createdBy_fkey',              'referenced ON DELETE',    'RI_FKey_noaction_del',  9::SMALLINT),
            ('ActivityDependency_revokedBy_fkey',              'referencing side',        'RI_FKey_check_ins',     5::SMALLINT),
            ('ActivityDependency_revokedBy_fkey',              'referencing side',        'RI_FKey_check_upd',    17::SMALLINT),
  ...
  IF v_bad IS NOT NULL THEN
    v_wrong := COALESCE(v_wrong, format('"ActivityDependency" exists and its foreign keys read as valid, but their enforcement machinery is incomplete or switched off: %s. A foreign key is enforced by four internal triggers, and both `ALTER TABLE ... DISABLE TRIGGER ALL` — which a failed restore runs — and the loss of a single trigger row leave the constraint definition and its validated flag exactly as they were, while ordinary writes can store nonexistent or cross-project endpoints and creator or revoker identities that name no membership. This migration will not seal a table whose containment does not act. Restore the key (ALTER TABLE public."ActivityDependency" ENABLE TRIGGER ALL re-enables a disabled one; a MISSING trigger row needs the key dropped and re-added, which revalidates it), confirm what was written while it was unenforced, then re-run.', v_bad));
  END IF;
  ...
  -- THE INSTALL BARRIER, COMPARED BY DEFINITION — because its whole value is what it REFUSES ────
  -- While the canonical barrier stands every INSERT is refused, so a table still carrying it
  -- CANNOT have acquired a row after it was created; section 9 lifts it only once every seal is
  -- armed. That is why its presence can MEAN "this install has not finished".
  --
  -- IT MEANS THAT ONLY IF IT IS THE BARRIER THIS FILE WROTE, and an earlier head asked for it by
  -- NAME ALONE. That is the same substitution this file already refuses everywhere else — the
  -- lesson section 1e states about constraints, not applied to the one constraint section 1e's
  -- list did not contain. The barrier is a first-class constraint and is now compared like one.
  --
  -- The interleaving the presence test admitted, exactly:
  --
  --     an autocommit retry meets an EMPTY, INCOMPLETE table whose
  --     `ActivityDependency_install_incomplete_check` is a hollow `CHECK (TRUE)`
  --     T1  reads "barrier present" → "unwritable" → resume, and COMMITS this DO block,
  --         releasing the ACCESS EXCLUSIVE lock it has held since 1b
  --     T2  INSERTs an already-revoked edge — the hollow CHECK admits it, and
  --         `ActivityDependency_born_live` is not armed yet
  --     T1  arms the remaining seals and section 9 reports the install complete
  --
  -- The row survives, permanently (section 6 will not touch a revoked row), and every later seal
  -- reads as armed over fabricated immutable evidence. So a barrier that is present and NOT the
  -- canonical one is WRONG, not missing: an abort whether or not the table holds rows, because
  -- what it proves about writability is false in either case.
  --
  -- `convalidated` is part of the comparison for the reason it is everywhere else here: a
  -- `NOT VALID` barrier is a barrier somebody re-added, and this file adopts nothing it did not
  -- install. It still refuses new rows, so this is the conservative direction.
  SELECT regexp_replace(pg_get_constraintdef(k.oid), '[[:space:]]+', ' ', 'g')
         || CASE WHEN k.convalidated THEN '' ELSE ' [NOT VALID]' END
    INTO v_barrier
    FROM pg_constraint k
  ...
  -- ── 1i. RE-ARM A BARRIER THAT IS ABSENT FROM AN INSTALL THAT HAS NOT FINISHED ───────────────
  -- Past 1h, an incomplete table holds NO rows — `v_missing` with `v_rows > 0` aborted above. So
  -- the write exclusion this file depends on for the rest of the run is simply not there, and the
  -- same interleaving the hollow barrier admitted is open: this block COMMITS (the caller may be
  -- in autocommit), the ACCESS EXCLUSIVE lock goes with it, and sections 2 to 8 install seals over
  -- whatever arrived in between.
  --
  -- Re-arming is not adoption. It is this file's own object, re-created from this file's own text,
  -- and `ALTER TABLE ... ADD CONSTRAINT` VALIDATES the existing rows — over an empty table that is
  -- a no-op, and over a populated one it would fail loudly rather than certify anything. It runs
  -- under the lock taken in 1b, so it is committed together with the reads that justified it.
  --
  -- Section 2's `CREATE TABLE IF NOT EXISTS` declares the barrier INLINE, so this file's own
  -- partial apply always carries it; reaching here means somebody dropped it deliberately off an
  -- unfinished install. That is a state to REPAIR, not to refuse: refusing would leave the table
  -- unguarded, which is the opposite of what the barrier is for.
  IF v_barrier_absent AND v_missing IS NOT NULL THEN
    ALTER TABLE public."ActivityDependency"
      ADD CONSTRAINT "ActivityDependency_install_incomplete_check" CHECK ("id" !~ '^');
    RAISE NOTICE 'schedule B1: the install barrier was missing from an unfinished install and has been re-armed; "ActivityDependency" stays unwritable until section 9 proves every seal.';
  END IF;
...
BEGIN
  ...
  --
  -- THE LOCK SERIALIZES THE STATEMENT, NOT THE SNAPSHOT, and the earlier version of this comment
  -- claimed otherwise. TRUNCATE does take an ACCESS EXCLUSIVE lock before firing this trigger, so
  -- no concurrent transaction can insert BETWEEN the test and the erasure — but that is not the
  -- race. Under a FIXED-SNAPSHOT isolation level the snapshot is taken when the transaction first
  -- reads, and the wait for the lock does not refresh it. MEASURED on PostgreSQL 16.13: T1 opens
  -- REPEATABLE READ and reads this table while it is empty; T2 inserts an edge and COMMITS; T1
  -- then issues TRUNCATE, acquires the lock uncontended, and the `EXISTS` below — running on T1's
  -- snapshot from before T2 existed — answers "empty". The seal returned NULL, the truncate
  -- proceeded, and a committed edge with both its attributions was erased by one statement, which
  -- is the exact outcome this seal exists to prevent.
  --
  -- This is the same failure as `provolatile` one level up, and worth naming as such: there a
  -- STABLE clone made the function reuse the CALLING STATEMENT's snapshot; here the TRANSACTION's
  -- isolation level fixes it, and no property of the function can override that. So the isolation
  -- level is asked directly, before the fast path is taken at all.
  --
  -- READ COMMITTED and nothing else. Under it every statement takes a fresh snapshot, and a
  -- VOLATILE function's queries take one per query — so this `EXISTS` runs on a snapshot taken
  -- AFTER the lock was granted, which means after any competing writer committed and released it.
  -- That is what makes the fast path genuinely race-free, and it is the isolation the application,
  -- Prisma and psql all use by default. REPEATABLE READ and SERIALIZABLE are REFUSED rather than
  -- silently downgraded: a caller that chose a fixed snapshot did so for a reason, and quietly
  -- reading outside it would be its own bug.
  --
  -- The fast path is kept rather than replaced by an unconditional refusal, because what must not
  -- be destroyed is the RECORD, and an empty table has none. Refusing unconditionally would refuse
  -- every fixture reset that CASCADEs through "Activity" on a database where no edge was ever
  -- written, and push those callers into disabling this seal routinely — which is how a seal
  -- becomes a formality. Precise, not merely strict.
  IF current_setting('transaction_isolation') <> 'read committed' THEN
    RAISE EXCEPTION 'schedule: "ActivityDependency" cannot be truncated from a % transaction. This seal permits TRUNCATE only when the table is EMPTY, and under a fixed-snapshot isolation level that emptiness test would run on a snapshot taken before the transaction waited for its lock — a concurrent writer''s committed edge would be invisible to it and would be erased. Run the reset under READ COMMITTED, or, for a sanctioned destructive reset, disable "ActivityDependency_no_truncate" by name for the duration.',
...
--   * the five foreign keys' ENFORCEMENT, read from `pg_trigger.tgenabled` on the internal
--     `RI_ConstraintTrigger` rows of each key, on the referencing AND the referenced side. Target
--     and definition say what a key MEANS; only this says whether it ACTS. `DISABLE TRIGGER ALL`
--     leaves the first two untouched and turns the third off;
...
BEGIN
  ...
  -- The text between these two markers is the WHOLE physical-seal question, and it is extracted
  -- and re-executed verbatim by `apps/api/src/activities/b1/b1-seals.ts` on every production
  -- deploy — see section 9's note on why an install-time proof is not enough. There is ONE copy of
  -- this inventory and both readers run that copy, so it cannot drift the way a hand-kept second
  -- list would. The only edit the extractor makes is to drop the plpgsql assignment, which turns the
  -- assignment into a plain query; it depends on nothing else in this DO block. That phrase must
  -- appear exactly once between the markers, and the extractor refuses the file if it does not —
  -- including if a comment here were to repeat it, which is why this one spells out neither.
  SELECT string_agg(x.what, ', ' ORDER BY x.what) INTO v_missing FROM (
    ...
    UNION ALL
    -- THE FOREIGN KEYS' ENFORCEMENT MACHINERY, TRIGGER BY TRIGGER. The row above proves each key
    -- POINTS at the right relation; this proves it ACTS. A foreign key is implemented as FOUR
    -- internal `RI_ConstraintTrigger` rows — two on the referencing side (the INSERT and the UPDATE
    -- check) and two on the referenced side (the ON UPDATE and the ON DELETE action) — and
    -- `ALTER TABLE ... DISABLE TRIGGER ALL` sets them to 'D' while leaving both
    -- `pg_get_constraintdef` and `convalidated` exactly as they were, so every other check in this
    -- block, and every one in section 1, accepts a key that enforces nothing.
    --
    -- Each of the four is named and asked for on its own, for the reason section 1e'' measures: a
    -- side-level "is there a trigger here" accepts a key that has lost one of the two on that side,
    -- and a lost `RI_FKey_check_upd` is the difference between a revoker identity that must exist
    -- and one that need not. Stating the expected function LITERALLY also pins the referential
    -- ACTIONS — `RI_FKey_restrict_del` is not `RI_FKey_cascade_del` — and `tgtype` pins the event
    -- each is wired to. The function is resolved in `pg_catalog` and the trigger must be internal,
    -- so neither a decoy function nor a user trigger can stand in for the machinery. While any one
    -- of the twenty is wrong the barrier is not lifted and the table stays unwritable.
    SELECT format('foreign key "%s" is not enforced on its %s: expected exactly one enabled internal trigger of type %s running pg_catalog."%s"() on %s, found %s',
                  e.name, e.side, e.tgtype::TEXT, e.fn, e.rel::REGCLASS::TEXT, e.found)
      FROM (
        SELECT k.name, k.side, k.fn, k.tgtype,
               CASE WHEN k.side = 'referencing side' THEN c.conrelid ELSE c.confrelid END AS rel,
               (SELECT CASE WHEN COUNT(*) = 0 THEN 'none'
     ...
     WHERE e.found <> 'exactly that'
    UNION ALL
    -- THE TEN CONSTRAINTS, BY DEFINITION — not by name and `convalidated`, which is what this arm
    -- used to ask and is the finding it closes. A CHECK is the only seal here that a restore or a
    -- catalog repair can replace with something of the same name that enforces NOTHING while
    -- reading as perfectly healthy: `CHECK (true)` is a valid, validated CHECK constraint.
    -- MEASURED: swap `ActivityDependency_attribution_check` for a validated `CHECK (true)` and
    -- this inventory passed, the deploy-time verifier reported `"sealed": true` and exit 0, and an
    -- edge whose `createdByName` was three spaces — an attribution answerable to nobody — was
    -- accepted and then frozen permanently by section 6.
    --
    -- Section 1e compares these same ten by definition, but only on the RESUME path; a fresh
    -- install returns at the existence test and never reaches it, and after the install nothing
    -- re-reads section 1 at all. This arm is the one that runs in both places — section 9 executes
    -- it before lifting the barrier, and `b1-seals.ts` extracts and re-executes it on every
    -- deploy — so the definitions belong here. The canonical strings are the same ten section 1e
    -- carries, compared the same way, with whitespace normalised so a re-render cannot differ
    -- cosmetically. `pg_get_constraintdef` runs under this block's `pg_catalog` pin, so every
    -- foreign-key target renders schema-qualified and a decoy binding is textually different.
    SELECT format('constraint "%s" is %s, expected %s', c.name,
                  COALESCE((SELECT 'present as '
                                   || regexp_replace(pg_get_constraintdef(k.oid), '[[:space:]]+', ' ', 'g')
                                   || CASE WHEN k.convalidated THEN '' ELSE ' [NOT VALID]' END
                              FROM pg_constraint k
                             WHERE k.conname = c.name
                               AND k.conrelid = 'public."ActivityDependency"'::regclass),
                           'ABSENT'),
                  regexp_replace(c.canonical, '[[:space:]]+', ' ', 'g'))
      FROM (VALUES
        ('ActivityDependency_pkey',
         'PRIMARY KEY (id)'),
        ('ActivityDependency_attribution_check',
