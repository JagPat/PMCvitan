-- Phase 6 task 4d, unit 4d-i-b — THE PAIRING SWITCH-ON
-- (docs/superpowers/plans/2026-09-07-decision-workflow-4d.md §A.3 obligation 7, and §D's 4d-i-b
-- bullet — the unit JagPat carved out of 4d-i at #582's review round 6, finding 3).
--
-- 4d-i installed the pairing MECHANISM and left it switched off: the `DomainEventPairingClaim`
-- register with its writer-depth, append-only and no-TRUNCATE seals, the
-- `platform_claim_event_pairing` primitive, the kernel-owned deferred
-- `DomainEvent_t4d_pairing_claimed` seal, and the catalog's `pairingRequired` column seeded
-- `false` on every row — so no event type demanded a claim and the mechanism was dark in the
-- same sense the rest of 4d-i is. THIS FILE TURNS IT ON, and it is one unit because its three
-- pieces are inseparable in the only direction that matters: flipping a type to
-- `pairingRequired` without its claimant refuses every legitimate event of that type at commit,
-- and installing a claimant for a type still at `false` claims into a register nothing reads.
--
-- ITS INVENTORY IS EXACTLY §D's (a), (b) and (c):
--
--   (c) THE FLIP, delivered as a NEW COVERAGE GENERATION. Inside 4d-i the six rows would have
--       been born `true`; split out, they are already committed `false` at `842cc9fc…` and
--       `ExternalEffectCatalog_t4d_sealed` admits no UPDATE but the retirement stamp — its own
--       refusal says a definition changes by a NEW coverage version, never in place. So
--       `pairingRequired` joined the compiled catalog's `canonicalCatalog()` PREIMAGE
--       (`apps/api/src/platform/external-effects.ts`), which moves `effectCoverageVersion()`
--       from `842cc9fc…` to the generation seeded below, `pairingRequired` true on EXACTLY
...
--       `decision.consultation_responded`, every other column of every key equal to 4d-i's row.
--       That is the ONE successor shape 4d-i's catalog audit admits by derivation
--       (`_t4d_catalog_successor`, #582 rounds 36 and 38), and the audit below proves this seed
--       IS that shape against the deployed prior generation rather than assuming it.
--       The two generations 4d-i seeded stay exactly as they are: a still-serving 4d-i process
--       keeps resolving its own rows through the version its intents carry, which is the
--       mechanism the column was built for. NOTHING IS RETIRED HERE — retirement of the old
--       generations is 4d-iii's act, once no lease serves them.
--
--   (a) `ChangeRequest_t4d_paired`, the ONE deferred pairing seal on `ChangeRequest`: the
--       OPENING bundle in both directions (#568's review round 1, finding 3), the
--       closure-and-restoration bundle in both directions (#558's review round 1, finding 2),
--       the `open → resolved` closure paired with its reapproval (#558's review round 2,
...
--       queued: a bundle that writes its FACT before its EVENT (every delivered writer —
--       `decisions.service.ts` inserts the revision, the request or the consultation and reaches
--       `emitEvent` several statements later) queues the fact's deferred claimant AHEAD of the
--       event's deferred check, so a deferred claimant alone would serve it; a bundle that writes
--       its EVENT first queues the check ahead, and a deferred claimant alone would let the check
--       run against an empty register and refuse a valid bundle. So each unconditional claimant
--       is installed TWICE on one function — an immediate AFTER INSERT trigger that claims when
--       the event is already there, and a DEFERRED constraint trigger that claims at commit when
--       the event arrived later — through `platform_claim_event_pairing_once`, which makes the
--       second call a no-op for the row that already holds the claim and still meets the
--       register's per-event UNIQUE for any OTHER row. The ONE conditional claim — a
--       `countersign_rejection` request, which claims only when the bundle's primary is not a
--       `returned` `DecisionStrandedResolution` — is decided at COMMIT, because only then is the
--       whole bundle visible; its reject-back writer is a 4d-ii command that writes its request
--       before its event like every other, and the harness drives that order.
--       The request's two unconditional claims get the same immediate half
--       (`ChangeRequest_t4d_claim`), beside the deferred seal that judges the bundle.
--
-- WHAT IS NOT HERE, each by §D's own ordering:
--   · the three 4d-ii types (`decision.forwarded`, `decision.awaiting_countersign`,
--     `membership.standing_changed`) — not compiled yet; the unit that adds them declares them
--     with their claimants beside them (the provisional revision's, the forward's, the
--     transition's);
--   · the `DecisionCountersign` and `DecisionStrandedResolution` claimants for the
--     `approved`/`reapproved`/`change_requested` branches they are primary on — §D's (b) names
--     the three claimants above and no more, and round 11's rule is about the WINDOW: a
--     claimant may not arrive later than the flip because legitimate events would be refused in
--     between, and no legitimate event of those branches exists until 4d-iii drops the
--     `awaiting_countersign` reservation. 4d-ii installs them beside the commands that write
--     the facts, before any such event can be emitted;
--   · any service, controller, query, emitter, participant or UI change — the diff surface is
--     `prisma/` (this file and the seed's named DL-003 bypass), `test/`, and under `src/`
--     EXACTLY the compiled catalog's six `pairingRequired` declarations and the preimage change
--     (c) requires. It adds no table and no column: every seal below is over data 4d-i shaped.
--
-- ORDERED immediately after both 4d-i halves and DEPENDENT ON BOTH: it seeds beside 4d-i's
-- generations, claims through 4d-i's primitive, reads 4d-i's carrier and is judged by 4d-i's
-- kernel seal. The prerequisite audit below refuses to run over a database that holds none of
-- that. RE-RUNNABLE (`CREATE OR REPLACE`, `DROP TRIGGER IF EXISTS`, `ON CONFLICT DO NOTHING`)
-- and MARKER-AWARE on the same terms as its partners: the retirement snapshot is re-taken per
-- file, against an artifact THIS file creates, so a forged marker reads false and a genuine one
-- stands the retirement-gated audits down. The seals here are PERMANENT — §A.3 names
-- `ChangeRequest_t4d_paired` "the PERMANENT 4d-i deferred pairing seal" — so nothing in this
-- file is skipped on a retired database; 4d-iii replaces by name what it replaces, and pins it.
...
-- clean and re-runnable. This transaction is then never a waiting party on these tables, so it
-- cannot be one side of a deadlock with a serving command, whatever order that command locks in.
--
-- The set is derived from this file's own DDL and enforced by the harness's round-36 and
-- round-37 oracles (the census reads `pg_locks`): the five tables that gain a trigger below.
-- The catalog is written by INSERT only (RowExclusive), which contends with nothing here.
DO $t4d_window$
  ...
  LOOP
    ...
    EXCEPTION WHEN lock_not_available THEN
      ...
      IF attempts >= 600 THEN
        RAISE EXCEPTION 'phase6 4d-i-b: could not obtain the deployment window on the five pre-existing tables after % attempts — retry the deploy when writer traffic quiets. Nothing has been changed. See docs/RUNBOOK.md §P6T4D.', attempts;
      END IF;
...
-- ── THE PREREQUISITES ARE 4d-i's, AND THEY ARE VERIFIED, NOT ASSUMED ────────────────────────
-- Every claimant below CALLS one of these. A plpgsql body is not validated at CREATE time, so a
-- claimant calling a primitive that is not there would fail on the first real approval, in
-- production, rather than here. The set is the PRIMITIVES the bodies call — five from the
-- registers half, two from the decisions half, so a database missing either half is named —
-- and deliberately not 4d-i's TABLES or TRIGGERS: each function's own file creates the table it
-- reads, so a present function witnesses its table, and a 4d-i seal is 4d-i's to verify (the
-- seal-stripped harness omits any one of them from the whole unit, and this file must still
-- apply over that database so the arm measures the seal it names). UNGATED: the dependency
-- holds at every stage. It runs BEFORE the retirement snapshot below, because the snapshot reads
-- `RolloutRetirement`, which 4d-i creates: over a database that never ran 4d-i, this audit is the
-- one that gets to speak, not a bare "relation does not exist".
DO $t4dib_prereq$
...
-- and round 16, finding 2 for why each file re-takes it): the halves' settings died with their
-- commits, and an unset predicate reads "not retired", which is the wrong answer on a genuinely
-- retired database. Round 7's design, applied per file: the verdict names an artifact THIS file
-- creates and is taken here, before it exists. A forged marker on a db-push baseline therefore
-- reads FALSE (no such function yet) and every audit runs; a genuine retirement — which by
-- construction happened over a database that had run this file — reads TRUE.
DO $snapshot$
BEGIN
...
-- ════════════════════════════════════════════════════════════════════════════════════════════
-- PART 2 — THE SHARED PRIMITIVES: an idempotent claim, and the change lifecycle's recorder
-- ════════════════════════════════════════════════════════════════════════════════════════════
...
-- The converse, without which "both directions" is one direction: a transition performed with
-- NO request. After 4d-iii drops the doors a direct `approved → change` commits with nothing
