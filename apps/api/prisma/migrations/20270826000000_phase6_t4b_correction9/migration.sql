-- ═══════════════════════════════════════════════════════════════════════════════════════════
-- Phase 6 task 4b-i — Codex round 9, R9-1: restoration proof must PREDATE the change request
--
-- Round 8 narrowed the restoration exemption to require an `approved`/`reapproved` `DecisionEvent`,
-- reasoning that every genuine approval has written one since Phase 1. The reasoning was right and
-- THE PROOF WAS FORGEABLE: nothing stops such an event being inserted while the parent sits in
-- `change`, so a direct writer could plant `approvedById`/`approvedOption`, write the event, flip
-- the status, and have the exemption permanently excuse an unattributed approval.
--
-- THIS CORRECTION'S FIRST ATTEMPT WAS ITSELF AN OVER-REACH, and it is recorded here because it is
-- the more useful half of the lesson. It forbade inserting an approval event unless the parent was
-- already `approved`. The battery refuted it before it was pushed: `phase6-t4a-withdraw` (4 tests),
-- `phase3-requirements` R2-1 and `phase6-t4b-correction` F2 all failed, and `upgrade-proof.sh`
-- failed on the legacy fixture — because a non-approved decision CARRYING an approval event is
-- exactly the legacy/laundering shape 4a's seals exist to refuse, and making it unrepresentable
-- stops 4a from constructing the scenarios it defends against. Narrowing a rule until it breaks a
-- real path is the failure round 7 committed and round 8 had to undo; doing it again inside the
-- head that names the pattern is worth stating plainly.
--
-- THE RULE THAT WORKS. Do not constrain the event; constrain what counts as RESTORATION PROOF. A
-- genuine restoration's approval happened BEFORE the change request was opened — `withdrawChange`
-- closes a request against an approval that already existed. A planted event, written while the row
-- sits in `change`, necessarily comes AFTER. So `v_restores` now requires an approval event
-- `at` EARLIER than the open request's `createdAt`. Every legacy shape stays representable, 4a
-- keeps its fixtures and its defences, and the forgery stops being usable as proof.
--
-- Additive: one `CREATE OR REPLACE` of `decision_t4b_recorded_seal`, carrying `20270823000000`'s
-- body forward with only the `v_events` predicate changed. `20270815000000` … `20270825000000` are
-- byte-for-byte unchanged, no row is written, and a re-run is a no-op.
-- ═══════════════════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- R9-1: the event must PREDATE the change request it claims to have been approved before.
  -- Round 8 asked only that such an event EXIST, and an event can be written at any time — so the
  -- forger wrote one while the row sat in `change` and borrowed the exemption. A genuine
  -- restoration's approval necessarily happened BEFORE the change was requested; a planted one
  -- necessarily comes after. Ordering is the part of the evidence a forger cannot backdate through
  -- the ordinary write paths, and it leaves every legacy shape representable — 4a's seals exist
  -- precisely because a non-approved decision CAN carry an approval event, and forbidding that
  -- outright (this correction's first attempt) broke 4a, phase-3 legacy reapproval and the upgrade
  -- proof.
  SELECT COUNT(*) INTO v_events FROM "DecisionEvent" e
   WHERE e."decisionId" = OLD.id
     AND e."type" IN ('approved', 'reapproved')
     AND e."at" < COALESCE(
           (SELECT MIN(cr."createdAt") FROM "ChangeRequest" cr
             WHERE cr."decisionId" = OLD.id AND cr."status" = 'open'),
