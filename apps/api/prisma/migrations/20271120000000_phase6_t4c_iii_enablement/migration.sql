...
--
-- ATOMICITY IS THIS FILE'S OWN, and the BEGIN below is why. §D requires the handover to be
-- indivisible: the moment the reservation is dropped and the moment the preservation seal is
-- installed must not be separately observable, or a generic capability writer can slip a
-- `consultation` row with a forged human `enabledById` into the gap — and the backfill's
-- ON CONFLICT DO NOTHING would then preserve that forgery rather than correct it, with the new
-- seal freezing it in place.
--
-- An earlier draft carried no BEGIN and argued the runner supplied one, on the strength of a
-- measurement: `CREATE TABLE x; SELECT 1/0;` under `prisma migrate deploy` leaves no trace of the
-- first statement. The measurement is real but it is the wrong KIND of evidence. Prisma documents
-- the opposite guarantee — migrations are not automatically wrapped in a transaction — so what was
-- measured is an implementation detail of one version, and a seal whose indivisibility depends on
-- undocumented behaviour loses it silently at the next upgrade, with no test failing. The file now
-- states the boundary it needs instead of inferring it, which is also what the two other
-- explicit-BEGIN migrations in this repository already do.
--
-- Verified both ways under the real runner: an explicit BEGIN/COMMIT migration applies and is
-- recorded, and one that raises mid-transaction leaves NO object behind.
BEGIN;
...
-- The handover is indivisible: everything above becomes visible together, or not at all.
COMMIT;
