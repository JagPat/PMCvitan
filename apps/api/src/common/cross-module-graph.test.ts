...
const SERVICES: Record<string, { domain: string; foreign: Record<string, number>; dispatch: number }> = {
  ...
  // Phase 6 unit 4c-ii — +2: `consultations.request` and `consultations.respond`, each emitting
  // its own signal event and dispatching it through the SINGLE external-effect sender.
...
const NON_PILLAR_WRITERS: Record<string, string> = {
  ...
  'labour/t2c/t2c-repair.service.ts': 'labour infra (Phase 4 Task 2 round-2 correction) — the OFFLINE operator repair for pre-correction labour-commercial (F2/F3/F4/F5) violations: one bounded maintenance transaction disables ONLY the named frozen/lifecycle triggers the plan requires, applies an explicit operator plan (own-module labour tables only, via raw SQL), re-enables+verifies every labour-commercial immutability trigger, re-runs the diagnostics, and writes the SHARED T2CRepairAction before-image evidence; never invoked from a request path, no signal',
  'labour/t3c/t3c-repair.service.ts': 'labour infra (Phase 4 Task 3 round-3 correction) — the OFFLINE operator repair for pre-20270220 blank-manualReason musters: one bounded maintenance transaction disables ONLY LabourAttendance_append_only, MARKS AND REVOKES the row (it NEVER deletes an attendance row and never invents the missing reason), re-enables+verifies every §C immutability trigger, re-runs the diagnostics, and writes the SHARED T3CRepairAction before-image evidence; never invoked from a request path, no signal',
};
...
const CONTROLLER_ROUTES: Record<string, string[]> = {
  ...
  'decisions/decisions.controller.ts': ['Post()', "Post(':decisionId/publish')", "Patch(':decisionId/draft')", "Post(':decisionId/approve')", "Post(':decisionId/change')", "Post(':decisionId/withdraw')", "Post(':decisionId/change/withdraw')", "Post(':decisionId/consultations')", "Post(':decisionId/consultations/:consultationId/respond')"],
...
describe('Phase 2 Task 1 — cross-module call-graph classifier', () => {
  ...
  describe('PR C Task 2 — the in-request sender is gone; each command dispatches through the SINGLE ExternalEffectDispatcher', () => {
    ...
    it('84 external-effect dispatch sites total across the pillar services (82 + Phase-6 unit 4c-ii consultations.request/respond)', () => {
