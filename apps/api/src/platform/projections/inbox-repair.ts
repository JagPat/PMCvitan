...
export function readIdentityConfig(
  ...
  if (!anchor || !minimumRaw || !systemIdentifier) {
    ...
    return {
      ...
      refusal: {
        ...
        message:
          `${missing.join(', ')} ${missing.length > 1 ? 'are' : 'is'} unset while this database `
  ...
  return {
    ok: true,
    config: { anchorProjectId: anchor, expectedMinProjects, expectedSystemIdentifier: systemIdentifier },
  };
}
...
export async function runInboxRepairStep(
  ...
  try {
    ...
    if (config) {
      ...
      const [{ system_identifier: liveSystemIdentifier, current_database: liveDatabase }] =
        await prisma.$queryRaw<Array<{ system_identifier: bigint; current_database: string }>>`
      ...
      log(`[4c-iii-r] database identity ok: cluster ${config.expectedSystemIdentifier}, database "${liveDatabase}"`);
