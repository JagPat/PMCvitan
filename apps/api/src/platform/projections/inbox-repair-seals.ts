...
/** The canonical `prosrc` PostgreSQL stores for the fence function — the migration's own body. */
export function canonicalWriterFenceBody(): string {
  const sql = readWriterFenceMigrationSql();
  const match = /AS \$fence\$\n([\s\S]*?)\n\$fence\$;/u.exec(sql);
  if (!match) throw new Error(`${WRITER_FENCE_MIGRATION}: cannot extract the fence function body`);
  return `\n${match[1]}\n`;
}
...
export async function verifyWriterFence(prisma: SealCatalogReader): Promise<WriterFenceReport> {
  ...
  if (row.prosrc !== canonicalWriterFenceBody()) {
