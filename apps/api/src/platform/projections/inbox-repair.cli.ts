 ...
 *
 *   node dist/platform/projections/inbox-repair.cli.js seals repair
 *       Reinstalls the three canonical seals from the migration's own statements and verifies the
 *       result, in ONE transaction. Safe with a genuine marker present — which is exactly the
 *       database that needs it, and exactly the one the migration refuses to re-run. Where the
 *       seal that failed is one a forged marker could have come through (the insert gate or the
 *       row seal), the markers are INVALIDATED in that same transaction and reported as
 *       `markersInvalidated`; the truncate guard alone never invalidates anything.
 *
...
async function main(): Promise<void> {
  ...
  try {
    if (process.argv[2] === 'seals' && process.argv[3] === 'repair') {
      // The RECOVERY for a partial restore, and deliberately not the migration: the migration's
      // first act refuses any marker it cannot show was written under an ENFORCING seal, so on the
      // database that actually needs repairing — one that HAS a marker and lost a trigger —
      // re-running it always aborts. This asks the narrower question, and where the seal that
      // failed was one a forgery could have come through, it invalidates the marker in the same
      // transaction (`markersInvalidated`) rather than sealing a row it cannot vouch for.
      const report = await repairMarkerSeals(prisma);
      process.stdout.write(`${JSON.stringify({ ok: report.sealed, repaired: true, ...report }, null, 2)}\n`);
      if (!report.sealed) {
        process.stderr.write(
          `\n[4c-iii-r] the seals could NOT be restored:\n${summarizeMarkerSeals(report)}\n\nSee docs/RUNBOOK.md §P64CIIIR.\n`,
        );
        process.exitCode = 3;
      }
      return;
    ...
    if (process.argv[2] === 'seals') {
      ...
      // THE WRITER FENCE IS VERIFIED ON THE SAME CALL (Codex P1 on `44f2520`). `migrate.sh` already
      // runs `seals` before the repair on BOTH deploy paths and fails closed on a non-zero exit, so
      // checking here needs no change to the runner and cannot be skipped by one. A fence that is
      // absent, disabled, hollowed or `proconfig`-altered fences nothing, and a deploy that started
      // over one would serve whatever an already-running previous-release relay writes.
      const fence = await verifyWriterFence(prisma);
      process.stdout.write(`${JSON.stringify({ ok: report.sealed && fence.installed, ...report, writerFence: fence }, null, 2)}\n`);
      ...
      if (!fence.installed) {
        process.stderr.write(
          `\n[4c-iii-r] the decisions.inbox WRITER FENCE is not enforcing:\n`
