 ...
 *       result. Rows are untouched, so it is safe with a genuine marker present — which is exactly
 *       the database that needs it, and exactly the one the migration now refuses to re-run.
 *
...
async function main(): Promise<void> {
  ...
  try {
    if (process.argv[2] === 'seals' && process.argv[3] === 'repair') {
      ...
      // first act refuses any marker that predates the seal, so on the database that actually needs
      // repairing — one that HAS a genuine marker and lost a trigger — re-running it always aborts.
