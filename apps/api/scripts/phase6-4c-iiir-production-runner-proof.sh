...
# its own pre-seal diagnostic and abort — the previous version of this step discarded that failure
# and passed only because PostgreSQL had kept the pre-abort DDL, which is not a recovery. The
# compiled `seals repair` reinstalls the canonical seals, touches no row, and verifies afterwards.
...
[ "$(markers)" = "1" ] && ok "and the genuine marker is untouched by the repair" || bad "the repair changed the marker rows ($(markers))"
OUT="$(DATABASE_URL="$URL" PHASE6_4C_IIIR_ANCHOR_PROJECT_ID="$ANCHOR" \
       PHASE6_4C_IIIR_EXPECTED_MIN_PROJECTS=2 PHASE6_4C_IIIR_EXPECTED_SYSTEM_IDENTIFIER="$(sysid)" PHASE6_4C_IIIR_EXPECTED_DATABASE_OID="$(dboid)" \
       sh scripts/migrate.sh 2>&1)"; RC=$?
