...
assert "B1: all five seals are VOLATILE SECURITY INVOKER functions, so waiting on the lock buys a fresh snapshot" \
  ...
  "5"
# ENFORCED, not merely VALID. A foreign key is implemented as internal `RI_ConstraintTrigger` rows,
# and `ALTER TABLE ... DISABLE TRIGGER ALL` — which a failed restore runs — leaves every deparsed
# definition and every `convalidated` flag exactly as it was while the key stops acting. A legacy
# upgrade is precisely where that residue would be inherited, so the state is asked directly. Both
# sides: the INSERT/UPDATE checks live on "ActivityDependency", the ON UPDATE / ON DELETE actions
# live on "Project", "Activity" and "Membership".
# ASKED PER TRIGGER, NOT PER SIDE AND NOT AS A COUNT. Twenty enabled triggers is not the same claim
# as the RIGHT twenty: losing `RI_FKey_check_upd` while something else supplies a twentieth row
# would satisfy a count, and the two answers below only agree when every expected (key, relation,
# function) identity is present exactly once and enabled. The expected functions are named
# LITERALLY, which pins the referential ACTIONS too — `RI_FKey_restrict_del` is not
# `RI_FKey_cascade_del`, and a legacy upgrade is exactly where a re-created key could differ.
assert "B1: every containment and attribution key is ENFORCED — each of the twenty expected internal triggers is present, in pg_catalog, and on" \
  "SELECT (SELECT COUNT(*) FROM (VALUES ('ActivityDependency_projectId_fkey','\"ActivityDependency\"','RI_FKey_check_ins'),('ActivityDependency_projectId_fkey','\"ActivityDependency\"','RI_FKey_check_upd'),('ActivityDependency_projectId_fkey','\"Project\"','RI_FKey_cascade_upd'),('ActivityDependency_projectId_fkey','\"Project\"','RI_FKey_restrict_del'),('ActivityDependency_projectId_predecessorId_fkey','\"ActivityDependency\"','RI_FKey_check_ins'),('ActivityDependency_projectId_predecessorId_fkey','\"ActivityDependency\"','RI_FKey_check_upd'),('ActivityDependency_projectId_predecessorId_fkey','\"Activity\"','RI_FKey_noaction_upd'),('ActivityDependency_projectId_predecessorId_fkey','\"Activity\"','RI_FKey_noaction_del'),('ActivityDependency_projectId_successorId_fkey','\"ActivityDependency\"','RI_FKey_check_ins'),('ActivityDependency_projectId_successorId_fkey','\"ActivityDependency\"','RI_FKey_check_upd'),('ActivityDependency_projectId_successorId_fkey','\"Activity\"','RI_FKey_noaction_upd'),('ActivityDependency_projectId_successorId_fkey','\"Activity\"','RI_FKey_noaction_del'),('ActivityDependency_createdBy_fkey','\"ActivityDependency\"','RI_FKey_check_ins'),('ActivityDependency_createdBy_fkey','\"ActivityDependency\"','RI_FKey_check_upd'),('ActivityDependency_createdBy_fkey','\"Membership\"','RI_FKey_noaction_upd'),('ActivityDependency_createdBy_fkey','\"Membership\"','RI_FKey_noaction_del'),('ActivityDependency_revokedBy_fkey','\"ActivityDependency\"','RI_FKey_check_ins'),('ActivityDependency_revokedBy_fkey','\"ActivityDependency\"','RI_FKey_check_upd'),('ActivityDependency_revokedBy_fkey','\"Membership\"','RI_FKey_noaction_upd'),('ActivityDependency_revokedBy_fkey','\"Membership\"','RI_FKey_noaction_del')) AS e(k,rel,fn) WHERE (SELECT COUNT(*) FROM pg_constraint c JOIN pg_trigger g ON g.tgconstraint=c.oid JOIN pg_proc p ON p.oid=g.tgfoid JOIN pg_namespace pn ON pn.oid=p.pronamespace WHERE c.conname=e.k AND c.conrelid='\"ActivityDependency\"'::regclass AND g.tgrelid=e.rel::regclass AND g.tgisinternal AND pn.nspname='pg_catalog' AND p.proname=e.fn AND g.tgenabled IN ('O','A'))=1)::text || '/' || (SELECT COUNT(*) FROM pg_constraint k JOIN pg_trigger g ON g.tgconstraint=k.oid WHERE k.conrelid='\"ActivityDependency\"'::regclass AND k.contype='f')::text;" \
  "20/20"
# THE INSTALL BARRIER, BY DEFINITION. Section 9 drops it on proof, so a finished install carries
# none — but the name is the file's own proof that an unfinished install is unwritable, and a hollow
# same-named CHECK would read as that proof while admitting every INSERT. Asked here as "there is no
# constraint of that name, canonical or otherwise", which is the only state a completed install has.
assert "B1: no constraint of the install-barrier NAME survives a completed install, hollow or canonical" \
  "SELECT COALESCE(string_agg(pg_get_constraintdef(oid), ','), 'none') FROM pg_constraint WHERE conname='ActivityDependency_install_incomplete_check' AND conrelid='\"ActivityDependency\"'::regclass;" \
  "none"
...
assert "4c-i: both owned ORGS primitives are installed" \
  ...
  "2"
# NAMED rather than counted (Phase 6 unit 4c-iii, applying the lesson this file already recorded
...
assert "4c-iii: the reservation is REPLACED — dropped, with the preservation seal in its place" \
  "SELECT COUNT(*)::text FROM pg_trigger WHERE tgname = 'ProjectCapability_t4c_reserved';" \
  "0"
