...
# could be adopted.
# A VALIDATED KEY IS NOT AN ENFORCED ONE. A foreign key is implemented as internal triggers, and
# `ALTER TABLE ... DISABLE TRIGGER ALL` switches them off without touching the catalog row — the
# deparse is identical and `convalidated` stays true while ordinary inserts store nonexistent
# endpoints and unknown authorship. Asked through `tgconstraint`, so it covers the check triggers on
# this table AND the action triggers on each referenced relation.
assert "B1: every containment key is ENFORCED, not merely validated — no internal trigger is disabled" \
  "SELECT COUNT(*)::text FROM pg_constraint k JOIN pg_trigger g ON g.tgconstraint=k.oid WHERE k.conrelid='\"ActivityDependency\"'::regclass AND k.contype='f' AND g.tgenabled <> 'O';" \
  "0"
assert "B1: no constraint of the install-barrier NAME survives the deploy, canonical or otherwise" \
  "SELECT COUNT(*)::text FROM pg_constraint WHERE conname='ActivityDependency_install_incomplete_check' AND conrelid='\"ActivityDependency\"'::regclass;" \
  "0"
