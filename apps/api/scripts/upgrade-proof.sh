...
# A KEY THAT DOES NOT FIRE IS NOT A KEY. PostgreSQL implements a foreign key as internal triggers,
# and `ALTER TABLE ... DISABLE TRIGGER ALL` switches them off while leaving `pg_constraint`
# byte-identical — same definition, same target, still `convalidated`.
assert "B1: every containment and attribution key ENFORCES, not merely exists and validates" \
  "SELECT COUNT(*) FILTER (WHERE g.tgenabled NOT IN ('O','A'))::text FROM pg_constraint k JOIN pg_trigger g ON g.tgconstraint=k.oid WHERE k.conrelid='\"ActivityDependency\"'::regclass AND k.contype='f';" \
  "0"
# The install barrier is LIFTED on a finished install (asserted above). What must never be present
# is a same-named constraint that refuses nothing — the migration would read it as proof the table
# is shut and seal whatever arrived through it.
assert "B1: no hollow stand-in for the install barrier survives the upgrade" \
  "SELECT COALESCE((SELECT pg_get_constraintdef(oid) FROM pg_constraint WHERE conname='ActivityDependency_install_incomplete_check' AND conrelid='\"ActivityDependency\"'::regclass), 'absent');" \
  "absent"
