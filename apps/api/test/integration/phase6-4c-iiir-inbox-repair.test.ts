...
describe('Phase 6 unit 4c-iii-r — deploy-time decisions.inbox repair (live PG)', () => {
  ...
  it('R6 — a MARKED database whose register regressed REFUSES instead of skipping on the marker', async () => {
    ...
    // AND THE ROW-LEVEL HEAL DOES NOT CLEAR IT (round 22). Deleting the stale row makes the stored
    // rows match canonical again, so every comparison in this step now calls the register healthy —
    // which is exactly the blind spot `R14-3` describes, because a v1 write is not distinguishable
    // from a v2 one for a threadless decision. The write above was an UNDECLARED one (a still-live
    // legacy relay is what this probe simulates), so the writer fence STAMPED the generation, and
    // that stamp survives the heal. The deploy therefore still refuses.
    const healed = await runInboxRepairStep(single, ops, env);
    expect(healed.ok).toBe(false);
    expect(healed.refusal?.code).toBe('concurrent-corruption');
    expect(healed.refusal?.message).toContain('fenced');

    // …and a REBUILD is what clears it, because the stamp is a property of the generation the
    // legacy writer touched, not of the project.
    await putInService();
    const rebuilt = await runInboxRepairStep(single, ops, env);
    expect(rebuilt.ok).toBe(true);
    expect(rebuilt.action).toBe('skipped-marker-present');
  });
    ...
    try {
      ...
      const rows = await single.$queryRawUnsafe<Array<{ fn: string; owner: string }>>(
        `SELECT p.proname AS fn, pg_get_userbyid(p.proowner) AS owner FROM pg_proc p
          ...
          WHERE n.nspname = 'public' AND p.proname = ANY($1::text[])`, MARKER_SEAL_TRIGGERS.map((t) => t.fn));
  ...
  // ── R22: the four findings from `44f2520` ───────────────────────────────────────────────────
  it('R22-1 — an inheritance CHILD of the marker table is rejected, and the repair refuses over it', async () => {
    // THE FINDING, measured before it was fixed: with all three seals installed and the verifier
    // reporting `sealed: true`, a table created `INHERITS ("OutboxOperatorAction")` takes a marker
    // row that the PARENT lookup finds (a plain SELECT reads children; only `FROM ONLY` excludes
    // them) while NONE of the parent's triggers fire for DML against the child. The forged marker
    // was then freely UPDATEd and DELETEd. Gate, row seal and truncate guard, all bypassed by
    // writing one table over.
    const child = 'OutboxOperatorAction_probe_child';
    try {
      await single.$executeRawUnsafe(`CREATE TABLE "${child}" () INHERITS ("${MARKER_SEAL_TABLE}")`);

      // the forgery the parent's seals cannot see
      await single.$executeRawUnsafe(
        `INSERT INTO "${child}" ("id","action","operatorIdentity","reason") VALUES ($1,$2,$3,$4)`,
        'probe-forged-child', PHASE6_4C_IIIR_MARKER_ACTION, 'attacker', 'forged in a child',
      );
      expect(await markerCount()).toBe(1);                       // the parent lookup TRUSTS it
      await single.$executeRawUnsafe(`UPDATE "${child}" SET "reason" = $1 WHERE "id" = $2`,
        'rewritten past the row seal', 'probe-forged-child');    // …and it is rewritable

      // the verifier now says so, and classes it as a forgery window so the marker is not evidence
      const report = await verifyMarkerSeals(single);
      expect(report.sealed).toBe(false);
      const finding = report.findings.find((f) => f.problem === 'inherited-child');
      expect(finding).toBeDefined();
      expect(finding?.trigger).toContain(child);
      expect(opensForgeryWindow(finding!)).toBe(true);

      // and the repair REFUSES rather than reinstalling triggers that would not fence the child
      await expect(repairMarkerSeals(single as never)).rejects.toThrow(SealRepairPrivilegeError);
      await expect(repairMarkerSeals(single as never)).rejects.toThrow(/inheritance child/u);

      // NOTE what this does and does not claim: the deploy is stopped by `migrate.sh`'s `seals`
      // call, which exits non-zero on the finding above and never reaches the repair — NOT by the
      // step, which does not verify seals itself. The runner proof covers that half.
    } finally {
      await single.$executeRawUnsafe(`DROP TABLE IF EXISTS "${child}"`);
      await clearMarkers();
      await restoreMarkerSeals();
    }
    // with the child gone the table is sealed again, so the probe leaves nothing behind
    expect((await verifyMarkerSeals(single)).sealed).toBe(true);
  });

  it('R22-2 — the writer fence marks a generation unservable when the writer does not declare its serializer', async () => {
    // The hazard the drain prerequisite exists for: a previous-release relay that was ALREADY
    // RUNNING when the migration landed. `syncConsumerCatalog` only asks a process at ITS start and
    // `catalogVersion` only fences a generation a previous release BUILDS, so neither reaches it.
    await t.prisma.$executeRawUnsafe(
      `DELETE FROM "DecisionProjection" WHERE "id" = ANY($1::text[])`, ['r22-legacy-write', 'r22-fenced-write']);
    const fence = await verifyWriterFence(single);
    expect(fence.findings).toEqual([]);
    expect(fence.installed).toBe(true);

    const gen = await activeGeneration(f.projectA.id);
    ...
    // A WRITE WITHOUT THE DECLARATION — what an already-running v1 relay's upsert looks like. An
    // INSERT rather than an UPDATE, so the probe does not depend on the fixture already holding
    // projection rows: a relay refreshing a generation writes one row per decision either way.
    await t.prisma.$executeRawUnsafe(
      `INSERT INTO "DecisionProjection" ("id","generationId","projectId","decisionId","status","dto","updatedAt")
       VALUES ($1,$2,$3,$4,'published','{}'::jsonb, now())`,
      'r22-legacy-write', row.id, f.projectA.id, 'r22-decision');
    const stamped = await t.prisma.projectionGeneration.findFirstOrThrow({
      where: { id: row.id }, select: { fencedAt: true, cursorStatus: true },
    });
    expect(stamped.fencedAt).not.toBeNull();
    // NOT cursorStatus: the relay rewrites that column itself right after the handler returns, so a
    // fence written there would be erased by the transaction it fences.
    expect(stamped.cursorStatus).toBe('live');
    // and the register stops being served from the projection — reads fall back to canonical, which
    // is always current. Asserted only in the REFUSING direction: a generation can be unservable for
    // reasons that have nothing to do with the fence (an event-less project has no applied
    // position), so a `not.toBeNull()` here would be testing the fixture, not the fence.
    expect(await readServableGeneration(t.prisma, DECISIONS_PROJECTION, f.projectA.id)).toBeNull();

    // A DECLARED write leaves a fresh generation alone: this release's writer is not fenced.
    await putInService();
    const rebuilt = await t.prisma.projectionGeneration.findFirstOrThrow({
      where: { consumer: DECISIONS_PROJECTION, projectId: f.projectA.id, status: 'active' },
      select: { fencedAt: true },
    });
    expect(rebuilt.fencedAt).toBeNull();
  });

  it('R22-3 — a generation the fence STAMPED fails the deploy after the commit, as a fact not an inference', async () => {
    await t.prisma.$executeRawUnsafe(
      `DELETE FROM "DecisionProjection" WHERE "id" = ANY($1::text[])`, ['r22-legacy-write', 'r22-fenced-write']);
    await putInService();
    const first = await runInboxRepairStep(single, ops, env);
    ...
    // Stamp the fence the way an already-running v1 relay does, then REMOVE the row it wrote. The
    // rows now match canonical again — which is the whole point: `R14-3` shows a v1 write is not
    // distinguishable from a v2 one for a threadless decision, so a row comparison would call this
    // register healthy. The STAMP is durable and says a session that did not declare this release's
    // serializer wrote here. On the next start the marker is present and the diagnosis is clean, so
    // this generation would be skipped and SERVED — except that the post-commit check reads the
    // stamp directly.
    const row = await t.prisma.projectionGeneration.findFirstOrThrow({
      where: { consumer: DECISIONS_PROJECTION, projectId: f.projectA.id, status: 'active' },
      select: { id: true },
    ...
    await t.prisma.$executeRawUnsafe(
      `INSERT INTO "DecisionProjection" ("id","generationId","projectId","decisionId","status","dto","updatedAt")
       VALUES ($1,$2,$3,$4,'published','{}'::jsonb, now())`,
      'r22-fenced-write', row.id, f.projectA.id, 'r22-fenced-decision');
    await t.prisma.$executeRawUnsafe(`DELETE FROM "DecisionProjection" WHERE "id" = $1`, 'r22-fenced-write');
    const stamped = await t.prisma.projectionGeneration.findFirstOrThrow({
      where: { id: row.id }, select: { fencedAt: true },
    });
    expect(stamped.fencedAt).not.toBeNull();

    const refused = await runInboxRepairStep(single, ops, env);
    expect(refused.ok).toBe(false);
    expect(refused.refusal?.code).toBe('concurrent-corruption');
    expect(refused.refusal?.message).toContain('fenced');

    await clearMarkers();
    await putInService();
  });

  it('R22-4 — the step refuses when its session lock is not actually held by this backend', async () => {
    // The transaction-pooling hazard, asserted where it matters rather than declared away: with the
    // lock genuinely held the check passes, and on a connection that does NOT hold it — which is
    // what a PgBouncer transaction-mode backend looks like after `DISCARD ALL` — it refuses.
    await single.$executeRawUnsafe(`SELECT pg_advisory_lock(${PHASE6_4C_IIIR_LOCK_KEY}::bigint)`);
    try {
      await expect(assertLockStillHeld(single, 'in the probe')).resolves.toBeUndefined();
      // `t.prisma` is a DIFFERENT connection: it never took this lock, exactly as a pooled backend
      // that was handed out after the lock was discarded has not.
      await expect(assertLockStillHeld(t.prisma as never, 'in the probe')).rejects.toThrow(PooledSessionError);
      await expect(assertLockStillHeld(t.prisma as never, 'in the probe')).rejects.toThrow(/TRANSACTION-POOLING/u);
    } finally {
  ...
  const plantDeclared = async (generationId: string, id: string, decisionId: string) => {
    ...
    await t.prisma.$transaction(async (tx) => {
      ...
      await tx.$executeRawUnsafe(
        `INSERT INTO "DecisionProjection" ("id","generationId","projectId","decisionId","status","dto","updatedAt")
         VALUES ($1,$2,$3,$4,'published','{}'::jsonb, now())`,
        id, generationId, f.projectA.id, decisionId);
    });
  };
  const fencedAtOf = async (id: string) =>
    (await t.prisma.projectionGeneration.findFirstOrThrow({ where: { id }, select: { fencedAt: true } })).fencedAt;

  it('R24-1 — a row MOVED to another generation fences the SOURCE it left, not just the destination', async () => {
    // THE FINDING: the expression stamped only NEW."generationId". Moving a row to a generation id
    // that does not exist removes the decision from the source while the destination stamp updates
    // nothing at all — the source stays caught-up, unfenced, and serves an incomplete register.
    await putInService();
    const src = await t.prisma.projectionGeneration.findFirstOrThrow({
      where: { consumer: DECISIONS_PROJECTION, projectId: f.projectA.id, status: 'active' },
      select: { id: true },
    });
    await plantDeclared(src.id, 'r24-move', 'r24-move-decision');
    expect(await fencedAtOf(src.id)).toBeNull();

    await t.prisma.$executeRawUnsafe(
      `UPDATE "DecisionProjection" SET "generationId" = $1 WHERE "id" = $2`, 'r24-no-such-generation', 'r24-move');
    expect(await fencedAtOf(src.id)).not.toBeNull();

    await t.prisma.$executeRawUnsafe(`DELETE FROM "DecisionProjection" WHERE "id" = $1`, 'r24-move');
    await putInService();
  });

  it('R24-2 — TRUNCATE fences every generation that held rows, and leaves the others alone', async () => {
    // THE FINDING: PostgreSQL fires no ROW trigger for TRUNCATE — measured, the register emptied and
    // every generation stayed unfenced, so a caught-up generation served an authoritative EMPTY
    // decisions register. The MARKER seal has carried a TRUNCATE guard since it was written.
    //
    // It STAMPS rather than rejecting because `DecisionProjection` is legitimately truncated (the
    // seed and every integration harness), always in the SAME statement as `ProjectionGeneration` —
    // so the stamps go with the generations and no harness changes. Asserted in a transaction that
    // rolls back, because this truncates a shared table.
    await putInService();
    const gen = await t.prisma.projectionGeneration.findFirstOrThrow({
      where: { consumer: DECISIONS_PROJECTION, projectId: f.projectA.id, status: 'active' },
      select: { id: true },
    });
    await plantDeclared(gen.id, 'r24-trunc', 'r24-trunc-decision');
    expect(await fencedAtOf(gen.id)).toBeNull();

    const single2 = new PrismaClient({ datasourceUrl: singleConnectionUrl(process.env.DATABASE_URL!) });
    try {
      await single2.$executeRawUnsafe(`TRUNCATE "DecisionProjection"`);
      expect(await fencedAtOf(gen.id)).not.toBeNull();
    } finally {
      await single2.$disconnect();
    }
    await putInService();
  });

  it('R24-3 — a hollowed stamp-seal BODY and an inheritance CHILD are both rejected by the verifier', async () => {
    // THE FINDING (body): the round that added the stamp seal verified only that the trigger
    // EXISTED — never its body or owner — so `CREATE OR REPLACE FUNCTION ... RETURN NEW` turned it
    // into a no-op while every other check still passed and the stamp became clearable again.
    expect((await verifyWriterFence(single)).installed).toBe(true);
    try {
      await single.$executeRawUnsafe(
        `CREATE OR REPLACE FUNCTION phase6_4c_iiir_fence_stamp_sealed() RETURNS trigger
         LANGUAGE plpgsql AS $probe$ BEGIN RETURN NEW; END $probe$`);
      const hollow = await verifyWriterFence(single);
      expect(hollow.installed).toBe(false);
      expect(hollow.findings.join(' ')).toMatch(/stamp seal function body is not the migration/u);
    } finally {
      await single.$executeRawUnsafe(
        `CREATE OR REPLACE FUNCTION phase6_4c_iiir_fence_stamp_sealed() RETURNS trigger
         LANGUAGE plpgsql AS $sealed$${canonicalFenceBody('sealed')}$sealed$`);
    }
    expect((await verifyWriterFence(single)).installed).toBe(true);

    // THE FINDING (inheritance): rows in a child are returned by the read path's unqualified parent
    // query while none of the parent's triggers fire for DML against it.
    const child = 'DecisionProjection_r24_child';
    try {
      await single.$executeRawUnsafe(`CREATE TABLE "${child}" () INHERITS ("DecisionProjection")`);
      const inherited = await verifyWriterFence(single);
      expect(inherited.installed).toBe(false);
      expect(inherited.findings.join(' ')).toMatch(/inheritance child/u);
    } finally {
      await single.$executeRawUnsafe(`DROP TABLE IF EXISTS "${child}"`);
    }
    expect((await verifyWriterFence(single)).installed).toBe(true);
  ...
  it('R24-4 — the TRUNCATE arm is held to the same closed rule as the fence itself', async () => {
    expect((await verifyWriterFence(single)).installed).toBe(true);
    try {
      await single.$executeRawUnsafe(`DROP TRIGGER "${WRITER_FENCE_TRUNCATE_TRIGGER}" ON "DecisionProjection"`);
      const missing = await verifyWriterFence(single);
      expect(missing.installed).toBe(false);
      expect(missing.findings.join(' ')).toMatch(/TRUNCATE arm .* is ABSENT/u);
    } finally {
      await single.$executeRawUnsafe(
        `CREATE TRIGGER "${WRITER_FENCE_TRUNCATE_TRIGGER}" BEFORE TRUNCATE ON "DecisionProjection"
         FOR EACH STATEMENT EXECUTE FUNCTION phase6_4c_iiir_fence_decision_projection_truncate()`);
    }
    expect((await verifyWriterFence(single)).installed).toBe(true);
  });

  it('R24-5 — a stamp landing AFTER the rows are read discards them, so the gate protects the fetch', async () => {
    // THE FINDING: `readServableGeneration` and the caller's `findMany` are separate READ COMMITTED
    // statements, so a legacy relay can commit its rewrite AND the stamp in the gap — the gate sees
    // an unfenced caught-up generation and the fetch returns exactly the rows the fence exists to
    // keep off the wire. Checking before the read does not protect the read.
    await putInService();
    const gen = await t.prisma.projectionGeneration.findFirstOrThrow({
      where: { consumer: DECISIONS_PROJECTION, projectId: f.projectA.id, status: 'active' },
      select: { id: true },
    });
    expect(await stillServableAfterRead(t.prisma, gen.id)).toBe(true);

    // the stamp arrives — exactly as it would from a writer committing mid-read
    await plantDeclared(gen.id, 'r24-race', 'r24-race-decision');
    await t.prisma.$executeRawUnsafe(`DELETE FROM "DecisionProjection" WHERE "id" = $1`, 'r24-race');
    expect(await fencedAtOf(gen.id)).not.toBeNull();

    // …and the post-read question now answers NO, so the caller discards its rows
    expect(await stillServableAfterRead(t.prisma, gen.id)).toBe(false);
    await putInService();
  });
