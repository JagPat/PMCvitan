...
describe('PR B Task 4 — outbox operations (live PG)', () => {
  ...
  const emit = (p: string, e: string) => t.prisma.$transaction((tx) => emitEvent(tx, { projectId: p, actor: human, eventType: 'decision.published', entityType: 'Decision', entityId: e, effectKey: 'decision.published', dispatch: {} }));
