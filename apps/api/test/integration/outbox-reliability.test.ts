...
describe('PR B Task 1 — durable outbox constraints (live PG)', () => {
  ...
  const emit = (projectId: string) =>
    t.prisma.$transaction((tx) => emitEvent(tx, { projectId, actor: human, eventType: 'decision.published', entityType: 'Decision', entityId: 'D-1', effectKey: 'decision.published.record', dispatch: {} }));
