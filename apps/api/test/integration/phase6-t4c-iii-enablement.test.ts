...
/** The shipped migration wraps itself in an explicit `BEGIN`/`COMMIT` (its atomicity is its own,
 *  not the runner's). Every consumer here replays the file INSIDE a transaction it already opened,
 *  where a nested `BEGIN` is a no-op warning and a `COMMIT` would end the caller's transaction
 *  early — so the transaction-control statements are dropped from the replay while every DDL and
 *  DML statement is kept verbatim. The probes still drive the SHIPPED file; they simply supply the
 *  boundary themselves instead of executing the file's own. */
