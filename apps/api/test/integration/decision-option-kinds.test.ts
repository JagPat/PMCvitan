...
describe('A1-i — the option kind vocabulary (live PG)', () => {
  ...
  const wipe = async (): Promise<void> => {
    ...
    // The selection notes cascade from the kind, but the guard refuses a DIRECT delete by the
    // transaction that wrote one — so committed notes are removed by deleting their kind, and
    // this step exists only to sweep notes whose kind is NOT this suite's (there are none today;
    // it is here so a future probe that writes one cannot silently strand it).
  ...
  /** Re-apply this unit's migration file, as a deploy would. Returns '' on success, else stderr. */
  const applyMigration = (): string => {
  ...
  /** The pre-baseline shape: the TABLE exists (schema.prisma describes it) but none of this
   *  migration's guards do, so its rows were never governed by anything. */
  const ungovernTheRegistry = async (): Promise<void> => {
    await t.prisma.$executeRawUnsafe(`DO $$
      DECLARE t text;
      BEGIN
        FOR t IN SELECT tgname FROM pg_trigger
                  WHERE NOT tgisinternal AND tgrelid='"DecisionOptionKind"'::regclass
        LOOP EXECUTE format('DROP TRIGGER %I ON "DecisionOptionKind"', t); END LOOP;
      END $$`);
  };
  ...
  it('P21 two ordinary option writes touching the same kinds in opposite order do not deadlock', async () => {
    ...
    // Two sessions, opposite orders, held at a REAL barrier between their two writes.
    //
    // A sleep here would not be coordination, and this suite has already paid for that lesson once
    // (P9, whose 500 ms wait was replaced by an observed mid-flight check). With only a pause, a
    // loaded runner can let one transaction finish BOTH writes before the other starts its first —
    // and then nothing is interleaved, the probe passes, and a regression that removed
    // `SKIP LOCKED` would sail through it. A probe that cannot fail for the reason it names is not
    // evidence.
    //
    // The barrier is two-sided because the hazard is two-sided: each transaction must be holding
    // its FIRST note lock before either attempts its second, which is the only arrangement in
    // which the plain DELETE deadlocks. Each signals after its first write and both wait for the
    // pair before proceeding, so the interleaving is a fact of the test rather than a hope.
    let arrived1!: () => void; let arrived2!: () => void;
    const first1 = new Promise<void>((r) => { arrived1 = r; });
    const first2 = new Promise<void>((r) => { arrived2 = r; });
    const bothHoldTheirFirstLock = Promise.all([first1, first2]);
  ...
  it('P15 the built-in seed refuses a conflicting row instead of adopting it', async () => {
    ...
    // trigger installed afterwards made it PERMANENT. Reproduced at the previous head: with
    // `material` carrying baseKind='technology' the file re-applied with exit 0.
    await ungovernTheRegistry();
    await t.prisma.$executeRawUnsafe(
      `UPDATE "DecisionOptionKind" SET "baseKind"='technology' WHERE "code"='material'`);
    expect(applyMigration(), 'a wrong base classification on a built-in must ABORT the deploy')
      .toMatch(/base classification is not the canonical one: material/u);
    // and it did not overwrite the row — this migration cannot know what the options meant
    expect((await one<{ baseKind: string }>(
      `SELECT "baseKind"::text AS "baseKind" FROM "DecisionOptionKind" WHERE "code"='material'`)).baseKind)
      .toBe('technology');

    await ungovernTheRegistry();
    await t.prisma.$executeRawUnsafe(
      `UPDATE "DecisionOptionKind" SET "baseKind"='material', "active"=false WHERE "code"='material'`);
    expect(applyMigration(), 'a RETIRED column-default kind breaks the serving release and must ABORT')
      .toMatch(/is the column default this migration installs, and this database already has it RETIRED/u);

    // PRECISION — retiring a NON-default built-in is a legitimate act, and a re-run must not
    // resurrect it. That is the whole difference between corruption and a decision.
    await ungovernTheRegistry();
    await t.prisma.$executeRawUnsafe(
      `UPDATE "DecisionOptionKind" SET "active"=true WHERE "code"='material'`);
    await t.prisma.$executeRawUnsafe(
      `UPDATE "DecisionOptionKind" SET "active"=false WHERE "code"='solution'`);
    expect(applyMigration(), 'a legitimately retired non-default built-in must not abort the deploy').toBe('');
    expect((await one<{ active: boolean }>(
      `SELECT "active" FROM "DecisionOptionKind" WHERE "code"='solution'`)).active)
      .toBe(false);
    await t.prisma.$executeRawUnsafe(
      `UPDATE "DecisionOptionKind" SET "active"=true WHERE "code"='solution'`);
  });

  // ── P16 (#370 round 2, F2) ──────────────────────────────────────────────────────────────────
  it('P16 the kind registry cannot be truncated', async () => {
    // Every other rule here is a ROW trigger and TRUNCATE fires none of them. The blast radius is
    // not "the registry": measured on a populated database, `TRUNCATE ... CASCADE` reached the
    // options, the approval register, and through the requirement specs into the labour and
    // commercial tables — while the approved decisions survived still saying `approved`.
    expect(await refusal(`TRUNCATE "DecisionOptionKind" CASCADE`))
      .toMatch(/cannot be truncated/u);
    expect(await refusal(`TRUNCATE "DecisionOptionKind"`)).toMatch(/cannot be truncated|foreign key/u);
    const kinds = await one<{ n: bigint }>(`SELECT COUNT(*) AS n FROM "DecisionOptionKind"`);
    expect(Number(kinds.n)).toBeGreaterThanOrEqual(4);
  });

  // ── P17 (#370 round 2, F3) ──────────────────────────────────────────────────────────────────
  it('P17 a kind selected in this transaction cannot be retired in the same transaction', async () => {
    // The option-side check is a CONSTRAINT trigger, and a constraint trigger is USER-deferrable:
    // `SET CONSTRAINTS ... IMMEDIATE` discharges it early, after which retiring the kind queues no
    // check of its own. Reproduced at the previous head — the transaction committed, leaving an
    // option classified by a kind that was active in NO committed state.
    //
    // This is the THIRD round on this one rule, and each time I had armed only the direction I was
    // looking at. The rule is now stated once and armed on BOTH sides.
    const code = `a1-${run}-${seq++}`;
    await t.prisma.$executeRawUnsafe(
      `INSERT INTO "DecisionOptionKind"("code","baseKind","labelKey","displayOrder")
       VALUES ('${code}','other','option.kind.p17',930)`);
    const d = await issue();
    const id = `opt-a1-${run}-${seq++}`;
    ...
    const attempt = await t.prisma.$transaction(async (tx) => {
      await tx.$executeRawUnsafe(
        `INSERT INTO "DecisionOption"("id","decisionId","label","optionKey","material","delta","swatch","kindCode")
         VALUES ('${id}','${d}','O','p17','Teak',0,'brown','${code}')`);
      await tx.$executeRawUnsafe(
        `SET CONSTRAINTS "DecisionOption_kind_selectable_ins" IMMEDIATE`);
      await tx.$executeRawUnsafe(
        `UPDATE "DecisionOptionKind" SET "active"=false WHERE "code"='${code}'`);
      return 'committed';
    }).catch((e: unknown) => String(e));
    expect(attempt).toMatch(/was selected for an option in this transaction/u);

    // nothing landed: no option, and the kind is still on the menu
    expect(Number((await one<{ n: bigint }>(
      `SELECT COUNT(*) AS n FROM "DecisionOption" WHERE "id"='${id}'`)).n)).toBe(0);
    expect((await one<{ active: boolean }>(
      `SELECT "active" FROM "DecisionOptionKind" WHERE "code"='${code}'`)).active).toBe(true);

    // PRECISION — the same two acts in SEPARATE transactions are still legal, and retiring a kind
    // options already carry is the whole point of retiring rather than deleting.
    await t.prisma.$executeRawUnsafe(
      `INSERT INTO "DecisionOption"("id","decisionId","label","optionKey","material","delta","swatch","kindCode")
       VALUES ('${id}','${d}','O','p17','Teak',0,'brown','${code}')`);
    await t.prisma.$executeRawUnsafe(
      `UPDATE "DecisionOptionKind" SET "active"=false WHERE "code"='${code}'`);
    expect((await one<{ kindCode: string }>(
      `SELECT "kindCode" FROM "DecisionOption" WHERE "id"='${id}'`)).kindCode).toBe(code);

    // …and the note is evidence, which only means anything INSIDE the transaction that wrote it:
    // a note is deleted by deleting its kind (cascade), and the guard's job is to stop a
    // transaction blinding the retirement check it is about to face. So the delete has to be
    // attempted in the SAME transaction as the write — attempted afterwards it matches no note at
    // all, succeeds trivially, and proves nothing.
    const erase = await t.prisma.$transaction(async (tx) => {
      const d2 = `DL-a1-${run}-${seq++}`;
      await tx.$executeRawUnsafe(
        `INSERT INTO "Decision"("id","projectId","title","room","status","photoSwatch","authorId")
         VALUES ('${d2}','${f.projectA.id}','Q','Hall','pending','stone','${f.memberUser.id}')`);
      await tx.$executeRawUnsafe(
        `INSERT INTO "DecisionOption"("id","decisionId","label","optionKey","material","delta","swatch","kindCode")
         VALUES ('opt-a1-${run}-${seq++}','${d2}','O','p17b','Teak',0,'brown','${code}')`);
      await tx.$executeRawUnsafe(
        `DELETE FROM "DecisionOptionKindSelection" WHERE "kindCode"='${code}' AND "txid"=txid_current()`);
      return 'erased';
    }).catch((e: unknown) => String(e));
    expect(erase).toMatch(/cannot be erased by the transaction that wrote it/u);
  });
