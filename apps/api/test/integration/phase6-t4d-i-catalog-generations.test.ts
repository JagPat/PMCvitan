...
describe('phase 6 4d-i / 4d-i-b — the catalog carries every generation still being emitted', () => {
  ...
  it('4d-i-b: the SWITCH-ON is a property of the generation, not of the key — the same type is unclaimed-admitted at the prior generation and refused at the current one', async () => {
    ...
    // Nothing else in this transaction: no request, no transition, no claimant. It names a HUMAN
    // actor so U1's `DomainEvent_t4d_pairing_actor` seal passes (a pairingRequired event must name
    // one) and the arm reaches the KERNEL claim seal it is about, rather than being turned away at
    // the event boundary by the actor rule.
    const unclaimedAt = (version: string, eventId: string) => t.prisma.$transaction(async (tx) => {
      ...
      await tx.$executeRawUnsafe(
        `INSERT INTO "DomainEvent" ("eventId","eventType","payloadVersion","organizationId","projectId","streamPosition","actorKind","systemActor","actorId","entityType","entityId","dispatchIntent")
         SELECT $1,'decision.change_requested',1,$2,$3,s."nextPosition" - 1,'human',NULL,$5,'Decision','cg-dec',
