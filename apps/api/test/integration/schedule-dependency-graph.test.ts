...
describe('Schedule B1 — the acyclic activity dependency graph (live PG)', () => {
  ...
  // ── P31 ────────────────────────────────────────────────────────────────────────────────────
  it('P31 a HOLLOW install barrier is refused, because a name is not a rule', async () => {
    // THE FINDING (round 2 of #411, migration.sql:748). The barrier lookup asked only whether a
    // constraint of that name existed. A same-named `CHECK (TRUE)` therefore read as "unfinished
    // but unwritable" — and the table was open the whole time. That reopens the exact interleaving
    // the barrier was introduced to close: this block releases its lock when the statement ends,
    // a second session inserts an already-revoked edge before `born_live` is armed, and the run
    // goes on to arm every seal over it.
    //
    // MEASURED at `a222e91`, on this container, driving that sequence: T1 mid-file exit 0, the
    // barrier swapped for `CHECK (true)`, T2's fabricated withdrawal ACCEPTED, T1 resumed to exit
    // 0 with five seals armed, the row surviving, and `DELETE` then refused by the no-delete seal.
    const head = join(__dirname, '..', '..', '..', '..',
    ...
    try {
      ...
      // The forgery: same name, judges nothing.
      await t.prisma.$executeRawUnsafe(
      ...
      // It really does leave the half-built table open — asserted, not assumed, because that is
      // what makes the refusal below load-bearing rather than fussy.
      const [a, b] = [await activity(), await activity()];
      await t.prisma.$executeRawUnsafe(
        `INSERT INTO "ActivityDependency"("id","projectId","predecessorId","successorId","createdById","createdByName","revokedAt","revokedById","revokedByName")
         VALUES ('dep-b1-hollow-${run}','${f.projectA.id}','${a}','${b}','${f.memberUser.id}','PMC',now(),'${f.memberUser.id}','PMC')`);
      expect(await edgeCount(), 'a hollow barrier is no barrier at all').toBe(1);
      await t.prisma.$executeRawUnsafe(
        `DELETE FROM "ActivityDependency" WHERE "id" = 'dep-b1-hollow-${run}'`);

      const err = applyMigration({ autocommit: true });
      expect(err, 'a non-canonical barrier must abort the migration').not.toBeNull();
      expect(err, 'naming both definitions')
        .toMatch(/is NOT the install barrier this migration writes: present as CHECK \(true\)/u);
      expect(err, 'and saying why the name alone cannot be trusted')
        .toMatch(/the ONLY thing standing between a half-installed dependency graph and a writer/u);
      expect(err, 'and must point at the operator procedure').toMatch(/docs\/RUNBOOK\.md section B1/u);

      // NOTHING was armed over the open table.
      ...
      expect(Number(armed[0]!.n), 'no seal may be armed while the barrier is a forgery').toBe(0);

      // Restore the real barrier verbatim and the SAME file completes — a guard, not a dead end.
      await t.prisma.$executeRawUnsafe(
        `ALTER TABLE "ActivityDependency" DROP CONSTRAINT "ActivityDependency_install_incomplete_check"`);
      await t.prisma.$executeRawUnsafe(
        `ALTER TABLE "ActivityDependency" ADD CONSTRAINT "ActivityDependency_install_incomplete_check" CHECK ("id" !~ '^')`);
      expect(applyMigration({ autocommit: true }), 'and the file completes over the real one').toBeNull();
      expect(await installed(), 'with every seal armed').toBe(COMPLETE);
      const barrier = await t.prisma.$queryRawUnsafe<Array<{ n: bigint }>>(
        `SELECT COUNT(*) AS n FROM pg_constraint
          WHERE conname = 'ActivityDependency_install_incomplete_check'
            AND conrelid = '"ActivityDependency"'::regclass`);
      expect(Number(barrier[0]!.n), 'and the barrier lifted').toBe(0);
    } finally {
      rmSync(head, { force: true });
    }
  }, 180_000);
  ...
  // ── P32 ────────────────────────────────────────────────────────────────────────────────────
  it('P32 a validated foreign key whose enforcement triggers are DISABLED is refused', async () => {
    // THE FINDING (migration.sql:607). A foreign key is implemented as internal triggers.
    // `ALTER TABLE ... DISABLE TRIGGER ALL` — what a failed restore or a bulk load can leave
    // behind — switches them off without touching the catalog row: `pg_get_constraintdef`
    // deparses identically, `convalidated` stays true, `confrelid` still points at `public`.
    // Every check this file had said yes, and containment was gone.
    //
    // MEASURED at `a222e91`: the five named seals re-enabled, the internal triggers left disabled,
    // replay exit 0 — and an edge naming a NONEXISTENT activity and a NONEXISTENT creator
    // committed.
    const enforcement = async (): Promise<string> => (await t.prisma.$queryRawUnsafe<Array<{ s: string }>>(`
      SELECT COALESCE(string_agg(DISTINCT k.conname, ',' ORDER BY k.conname), '<all enabled>') AS s
       ...
       WHERE k.conrelid = '"ActivityDependency"'::regclass AND k.contype = 'f'
         AND g.tgenabled <> 'O'`))[0]!.s;

    expect(await enforcement(), 'a healthy install enforces every key').toBe('<all enabled>');

    await t.prisma.$executeRawUnsafe(`ALTER TABLE "ActivityDependency" DISABLE TRIGGER ALL`);
    try {
      // THE DAMAGE, EXECUTED FIRST, while the no-delete seal is off too so the fixture can be
      // cleaned up: containment really is gone in this state. An edge naming a NONEXISTENT
      // activity and a NONEXISTENT creator commits.
      const b = await activity();
      await t.prisma.$executeRawUnsafe(
        `INSERT INTO "ActivityDependency"("id","projectId","predecessorId","successorId","createdById","createdByName")
         VALUES ('dep-b1-ghost-${run}','${f.projectA.id}','GHOST-ACTIVITY','${b}','GHOST-USER','Nobody')`);
      expect(await edgeCount(), 'a disabled enforcement trigger checks nothing').toBe(1);
      await t.prisma.$executeRawUnsafe(
        `DELETE FROM "ActivityDependency" WHERE "id" = 'dep-b1-ghost-${run}'`);

      // Now re-arm the five NAMED seals, so what remains under test is the INTERNAL triggers
      // rather than the `tgenabled` check that already covers the seals.
      for (const tg of ['born_live', 'no_delete', 'no_truncate', 'frozen', 'acyclic']) {
        await t.prisma.$executeRawUnsafe(
          `ALTER TABLE "ActivityDependency" ENABLE TRIGGER "ActivityDependency_${tg}"`);
      }
      expect(await installed(), 'every seal now reads as armed').toBe(COMPLETE);
      expect(await enforcement(), 'while all five keys are unenforced').toBe(
        ['ActivityDependency_createdBy_fkey', 'ActivityDependency_projectId_fkey',
         'ActivityDependency_projectId_predecessorId_fkey',
         'ActivityDependency_projectId_successorId_fkey',
         'ActivityDependency_revokedBy_fkey'].join(','));

      const err = applyMigration();
      expect(err, 'an unenforced containment key must abort the migration').not.toBeNull();
      expect(err, 'naming every key it found unenforced')
        .toMatch(/present and validated but NOT ENFORCED/u);
      expect(err, 'and naming them').toMatch(/ActivityDependency_projectId_predecessorId_fkey/u);
      expect(err, 'and refusing to repair what it cannot judge')
        .toMatch(/will not re-enable them, because re-enabling validates nothing that arrived while they were off/u);
      expect(err, 'and must point at the operator procedure').toMatch(/docs\/RUNBOOK\.md section B1/u);
    } finally {
      await t.prisma.$executeRawUnsafe(`ALTER TABLE "ActivityDependency" ENABLE TRIGGER ALL`);
    }
    expect(await enforcement(), 'the repair the abort names restores enforcement').toBe('<all enabled>');
    expect(applyMigration(), 'and the same file then applies').toBeNull();

    // And containment binds again, which the catalog alone does not prove.
    const c = await activity();
    expect(await refusal(
      `INSERT INTO "ActivityDependency"("id","projectId","predecessorId","successorId","createdById","createdByName")
       VALUES ('dep-b1-ghost2-${run}','${f.projectA.id}','GHOST-ACTIVITY','${c}','${f.memberUser.id}','PMC')`))
      .toMatch(/violates foreign key|foreign key constraint/u);
  }, 180_000);
});
