...
describe('Schedule B1 — the acyclic activity dependency graph (live PG)', () => {
  ...
  // ── P31 ────────────────────────────────────────────────────────────────────────────────────
  it('P31 a foreign key that no longer ENFORCES is refused, not read as present and validated', async () => {
    // THE FINDING, reproduced. A foreign key is not self-executing: PostgreSQL implements it as
    // INTERNAL triggers, and `ALTER TABLE ... DISABLE TRIGGER ALL` — which a failed restore or a
    // bulk load leaves behind — switches them off while touching `pg_constraint` not at all. Same
    // deparsed definition, same `confrelid`, still `convalidated`. Every check in this file passed.
    //
    // MEASURED against the earlier head, on this container: `DISABLE TRIGGER ALL`, the five named
    // seals re-armed, re-run — exit 0, `0/10` enforcement triggers enabled, and this stored
    // without complaint:
    //
    //     INSERT ... VALUES('RED-A','p1','NO-SUCH-ACTIVITY','ALSO-NOT-REAL','NO-SUCH-USER','Ghost')
    //     INSERT 0 1
    //
    // The earlier head's enumeration recorded `tgisinternal` as deliberately EXCLUDED — "none of
    // this file's business". It was wrong: those triggers are not a neighbour's object that
    // happens to sit on this table, they ARE the five keys this file verifies.
    const enforcement = async (): Promise<string> => (await t.prisma.$queryRawUnsafe<Array<{ s: string }>>(`
      SELECT COUNT(*) FILTER (WHERE g.tgenabled IN ('O','A'))::text || '/' || COUNT(*)::text AS s
        FROM pg_constraint k JOIN pg_trigger g ON g.tgconstraint = k.oid
       WHERE k.conrelid = '"ActivityDependency"'::regclass AND k.contype = 'f'`))[0]!.s;

    const armed = await enforcement();
    expect(armed.split('/')[0], 'every enforcement trigger starts enabled').toBe(armed.split('/')[1]);

    await t.prisma.$executeRawUnsafe(`ALTER TABLE "ActivityDependency" DISABLE TRIGGER ALL`);
    // The five named seals are re-armed, so ONLY the internal enforcement is off — which is
    // precisely the state every other check in this file cannot see.
    await t.prisma.$executeRawUnsafe(`ALTER TABLE "ActivityDependency" ENABLE TRIGGER USER`);
    try {
      expect(await installed(), 'the five seals read as armed').toBe(COMPLETE);
      // `DISABLE TRIGGER ALL` acts on THIS table, so the halves of each key that live on the
      // referenced tables stay enabled. That is the point: the damage is partial and invisible,
      // and every key still reads as present, validated and correctly targeted.
      const [live, total] = (await enforcement()).split('/').map(Number);
      expect(live, 'the referencing half of every key is off').toBeLessThan(total);

      const err = applyMigration();
      expect(err, 'a declared-but-unenforcing key must abort').not.toBeNull();
      expect(err, 'saying the keys do not enforce')
        .toMatch(/carries its foreign keys but they DO NOT ENFORCE/u);
      expect(err, 'and naming one').toMatch(/ActivityDependency_projectId_predecessorId_fkey/u);
      expect(err, 'and telling the operator how to repair it').toMatch(/ENABLE TRIGGER ALL/u);

      // THE CONSEQUENCE, driven: with enforcement off the table really does take an edge whose
      // endpoints and author do not exist. This is what the refusal is protecting.
      await t.prisma.$executeRawUnsafe(
        `INSERT INTO "ActivityDependency"("id","projectId","predecessorId","successorId","createdById","createdByName")
         VALUES ('dep-b1-ghost-${run}','${f.projectA.id}','NO-SUCH-ACTIVITY','ALSO-NOT-REAL','NO-SUCH-USER','Ghost')`);
      expect(await edgeCount(), 'a ghost edge is storable while the keys do not fire').toBe(1);
    } finally {
      // The wipe uses TRUNCATE, which needs the seal disabled by name — do it here explicitly so a
      // failure above cannot leave enforcement off for the suites that follow.
      await t.prisma.$executeRawUnsafe(
        `ALTER TABLE "ActivityDependency" DISABLE TRIGGER "ActivityDependency_no_truncate"`);
      await t.prisma.$executeRawUnsafe(`TRUNCATE TABLE "ActivityDependency"`);
      await t.prisma.$executeRawUnsafe(`ALTER TABLE "ActivityDependency" ENABLE TRIGGER ALL`);
    }
    expect(await enforcement(), 'enforcement restored').toBe(armed);
    expect(applyMigration(), 'and the file applies once the keys fire again').toBeNull();
    expect(await installed()).toBe(COMPLETE);
  }, 180_000);

  // ── P32 ────────────────────────────────────────────────────────────────────────────────────
  it('P32 the install barrier is compared by DEFINITION: a hollow one is refused, not trusted', async () => {
    // THE FINDING, reproduced. Section 1g asked only whether a constraint of that NAME existed —
    // the one object this file trusted a name test for, and the one whose entire purpose is to be
    // load-bearing. A same-named `CHECK (true)` refuses nothing, so:
    //
    //     T1  reaches section 1 on an EMPTY table with a seal missing, reads "barrier present",
    //         concludes the table is safely unwritable, and proceeds (MISSING-only = resume)
    ...
    // fabricated withdrawal ACCEPTED, and it is then permanent — the no-delete seal, armed on top
    // of it, refuses to remove it. The row guard catches it only on a LATER run, which is after
    // the damage rather than instead of it.
    const barrier = async (): Promise<string> => (await t.prisma.$queryRawUnsafe<Array<{ s: string }>>(`
      SELECT COALESCE((SELECT pg_get_constraintdef(k.oid) FROM pg_constraint k
                        WHERE k.conname = 'ActivityDependency_install_incomplete_check'
                          AND k.conrelid = '"ActivityDependency"'::regclass), 'absent') AS s`))[0]!.s;

    expect(await barrier(), 'a finished install carries no barrier').toBe('absent');

    // A HOLLOW barrier over an unfinished install: refused, and the message says why it matters.
    await t.prisma.$executeRawUnsafe(
      `DROP TRIGGER "ActivityDependency_born_live" ON "ActivityDependency"`);
    await t.prisma.$executeRawUnsafe(
      `ALTER TABLE "ActivityDependency" ADD CONSTRAINT "ActivityDependency_install_incomplete_check" CHECK (TRUE)`);
    try {
      // It is hollow in fact, not merely in text: the table it claims to shut is wide open.
      const [a, b] = [await activity(), await activity()];
      await t.prisma.$executeRawUnsafe(
        `INSERT INTO "ActivityDependency"("id","projectId","predecessorId","successorId","createdById","createdByName","revokedAt","revokedById","revokedByName")
         VALUES ('dep-b1-hollow-${run}','${f.projectA.id}','${a}','${b}','${f.memberUser.id}','PMC',now(),'${f.memberUser.id}','PMC')`);
      expect(await edgeCount(), 'the hollow barrier stops nothing').toBe(1);
      await t.prisma.$executeRawUnsafe(
        `ALTER TABLE "ActivityDependency" DISABLE TRIGGER "ActivityDependency_no_truncate"`);
      await t.prisma.$executeRawUnsafe(`TRUNCATE TABLE "ActivityDependency"`);
      await t.prisma.$executeRawUnsafe(
      ...
      const err = applyMigration();
      expect(err, 'a hollow barrier must abort rather than be trusted').not.toBeNull();
      expect(err, 'naming it as not the barrier this migration installs')
        .toMatch(/is NOT the write barrier this migration installs: CHECK \(true\)/u);
      expect(err, 'and printing the canonical one').toMatch(/Expected: CHECK \(\(id !~ '\^'::text\)\)/u);
      expect(await barrier(), 'and the refusal changes nothing').toBe('CHECK (true)');
    } finally {
      await t.prisma.$executeRawUnsafe(
        `ALTER TABLE "ActivityDependency" DROP CONSTRAINT IF EXISTS "ActivityDependency_install_incomplete_check"`);
    }

    // AND THE CANONICAL BARRIER IS STILL THE RESUME PATH — the fix must not turn this file's own
    // partial apply into a refusal, which is the dead end the whole shape exists to avoid.
    await t.prisma.$executeRawUnsafe(
      `ALTER TABLE "ActivityDependency" ADD CONSTRAINT "ActivityDependency_install_incomplete_check" CHECK ("id" !~ '^')`);
    expect(await barrier(), 'the canonical barrier is in place').toBe(`CHECK ((id !~ '^'::text))`);
    const [c, d] = [await activity(), await activity()];
    expect(await refusal(edge(c, d)), 'and it really does shut the table')
      .toMatch(/ActivityDependency_install_incomplete_check/u);

    expect(applyMigration(), 'the file must COMPLETE its own unfinished install').toBeNull();
    expect(await barrier(), 'and lift the barrier on proof').toBe('absent');
    expect(await installed(), 'with every seal armed').toBe(COMPLETE);
    expect(await edgeCount(), 'and nothing fabricated anywhere').toBe(0);
  }, 180_000);
});
