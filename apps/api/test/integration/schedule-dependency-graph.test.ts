...
describe('Schedule B1 — the acyclic activity dependency graph (live PG)', () => {
  ...
  it('P28 the foreign keys bind to public, whatever schema the caller put in front of it', async () => {
    ...
    try {
      ...
      expect(err9, 'naming the key and what it actually references')
        .toMatch(/does not reference public\."Project"/u);
    } finally {
      await t.prisma.$executeRawUnsafe(
  ...
  it('P29 the relation has to be an ordinary permanent table, not merely a relation of that name', async () => {
    ...
    const cases: Array<{ what: string; arm: string; disarm: string; expect: RegExp }> = [
      ...
      { what: 'a rewrite rule, which rewrites the statement before any trigger fires',
        ...
        expect: /a REWRITE RULE named "b1_decoy_rule" is attached/u },
  ...
  it('P31 the install barrier is judged by its DEFINITION, and an absent one is re-armed', async () => {
    ...
    try {
      ...
      expect(err, 'naming what is there').toMatch(/carries a constraint named "ActivityDependency_install_incomplete_check" that this migration did not write: CHECK \(true\)/u);
      ...
      expect(await refusal(bornRevoked(`dep-b1-p31-fab2-${run}`, a, b)),
             'the canonical barrier refuses the same insert')
        .toMatch(/ActivityDependency_install_incomplete_check/u);
  ...
  it('P32 a foreign key that reads as valid but does not ENFORCE is refused', async () => {
    ...
    try {
      ...
      expect(err, 'naming the state').toMatch(/their enforcement machinery is incomplete or switched off/u);
      expect(err, 'and each key, each trigger, and where')
        .toMatch(/foreign key "ActivityDependency_projectId_predecessorId_fkey" is not enforced on its referencing side: expected exactly one enabled internal trigger of type 5 running pg_catalog\."RI_FKey_check_ins"\(\) on public\."ActivityDependency", found one, DISABLED/u);
  ...
  it('P33 a foreign key that has LOST one internal trigger, with its sibling intact, is refused', async () => {
    ...
    try {
      ...
      expect(err, 'and the exact trigger, and where it should be')
        .toMatch(/foreign key "ActivityDependency_revokedBy_fkey" is not enforced on its referencing side: expected exactly one enabled internal trigger of type 17 running pg_catalog\."RI_FKey_check_upd"\(\) on public\."ActivityDependency", found none/u);
  ...
  it('P34 the seals are re-verified on EVERY deploy, from the migration\'s own inventory', async () => {
    ...
    expect(sql.includes(inventory.replace(
      "SELECT string_agg(x.what, ', ' ORDER BY x.what) FROM (",
    ...
    try {
      ...
      expect(off.inventory, 'and the migration\'s own inventory names it')
        .toMatch(/armed trigger "ActivityDependency_frozen"/u);
    } finally {
      await t.prisma.$executeRawUnsafe(
...
END $hollow$`);
  ...
  // ── P35 ────────────────────────────────────────────────────────────────────────────────────
  it('P35 a CHECK of the canonical NAME that enforces nothing is refused, at install and at deploy', async () => {
    // THE FINDING (migration.sql:2163), reproduced. Section 9's constraint arm asked for a
    // constraint of each name that was `convalidated`, and nothing else. A CHECK is the one seal
    // here where that is not enough: `CHECK (true)` is a perfectly valid, validated CHECK
    // constraint. Section 1e DOES compare all ten by definition — but only on the RESUME path, and
    // a fresh install returns at the existence test without reaching it, and after the install
    // nothing re-reads section 1 at all. So the definition comparison belongs in the arm that runs
    // in both places: section 9 executes it before lifting the barrier, and the deploy-time
    // verifier extracts and re-executes that same text.
    const attribution = async (): Promise<string> => (await t.prisma.$queryRawUnsafe<Array<{ s: string }>>(`
      SELECT pg_get_constraintdef(oid) || ' / convalidated=' || convalidated::text AS s
        FROM pg_constraint
       WHERE conname = 'ActivityDependency_attribution_check'
         AND conrelid = '"ActivityDependency"'::regclass`))[0]!.s;
    const CANONICAL = `CHECK ((("createdById" !~ '^[[:space:]]*$'::text) AND ("createdByName" !~ '^[[:space:]]*$'::text)))`;

    expect(await installed(), 'starting from the finished install').toBe(COMPLETE);
    expect((await verifyB1Seals(t.prisma)).sealed, 'and sealed').toBe(true);

    const a = await activity();
    const b = await activity();
    const blank = `INSERT INTO "ActivityDependency"
        ("id","projectId","predecessorId","successorId","lagWorkingDays","createdById","createdByName")
        VALUES ('dep-b1-p35-${run}','${f.projectA.id}','${a}','${b}',0,'${f.memberUser.id}','   ')`;
    expect(await refusal(blank), 'an unanswerable attribution is refused while the CHECK is real')
    ...
    try {
      // THE PREMISE: a hollow CHECK is present, VALIDATED, and named exactly right.
      expect(await attribution(), 'the constraint is there and validated').toBe('CHECK (true) / convalidated=true');

      // THE CONSEQUENCE: an attribution answerable to nobody, which section 6 then freezes.
      await t.prisma.$executeRawUnsafe(blank);
      expect(await edgeCount(), 'a whitespace-only attribution is on the record').toBe(1);

      // THE FIX, in the one arm both readers run.
      const report = await verifyB1Seals(t.prisma);
      expect(report.sealed, 'the deploy-time verifier must not call this sealed').toBe(false);
      expect(report.inventory, 'and names the constraint, what is there, and what was expected')
        .toMatch(/constraint "ActivityDependency_attribution_check" is present as CHECK \(true\), expected CHECK/u);
      ...
      await t.prisma.$executeRawUnsafe(
        `ALTER TABLE "ActivityDependency" ADD CONSTRAINT "ActivityDependency_attribution_check" ${CANONICAL}`);
    }

    // PRECISION, not merely strictness: the restored canonical CHECK reads as sealed again.
    expect((await verifyB1Seals(t.prisma)).sealed, 'the canonical CHECK re-seals it').toBe(true);
    expect(applyMigration(), 'and the file applies over it').toBeNull();
    const c = await activity();
    const d = await activity();
    expect(await refusal(`INSERT INTO "ActivityDependency"
        ("id","projectId","predecessorId","successorId","lagWorkingDays","createdById","createdByName")
        VALUES ('dep-b1-p35b-${run}','${f.projectA.id}','${c}','${d}',0,'${f.memberUser.id}','   ')`),
      'and the rule acts again').toMatch(/attribution_check/u);
  }, 180_000);

  // ── P36 ────────────────────────────────────────────────────────────────────────────────────
  it('P36 TRUNCATE cannot erase committed evidence through a FIXED SNAPSHOT', async () => {
    // THE FINDING (migration.sql:1464), reproduced. The no-truncate seal permits TRUNCATE on an
    // EMPTY table — what must not be destroyed is the RECORD, and an empty table has none — and
    // the comment justified that as race-free because TRUNCATE takes ACCESS EXCLUSIVE before the
    // trigger fires. THE LOCK SERIALIZES THE STATEMENT, NOT THE SNAPSHOT. Under REPEATABLE READ
    // the snapshot is fixed at the transaction's first read and waiting for a lock does not
    // refresh it, so the emptiness test can answer "empty" about a table that is not.
    //
    // MEASURED at the previous head, on a live database: T1 opened REPEATABLE READ and read the
    // empty table; T2 inserted an edge and COMMITTED; T1 then truncated — accepted, no exception,
    // and the committed edge with both its attributions was gone. This is the same failure as the
    // `provolatile` finding (P30) one level up: there a STABLE clone reused the calling
    // STATEMENT's snapshot, here the TRANSACTION's isolation level fixes it, and no property of
    // the function can override that.
    //
    // No sleeps and no lock barrier are needed: the interleaving is enforced by the awaits, and
    // the second client is a genuinely separate connection whose INSERT commits on its own.
    const other = new PrismaClient({ datasources: { db: { url: process.env.DATABASE_URL } } });
    const a = await activity();
    const b = await activity();
    const insert = edge(a, b);
    try {
      // (1) THE FINDING'S OWN INTERLEAVING, under a fixed snapshot: refused, evidence intact.
      const fixed = await t.prisma.$transaction(async (tx) => {
        await tx.$queryRawUnsafe(`SELECT COUNT(*) FROM "ActivityDependency"`);   // snapshot fixed: empty
        await other.$executeRawUnsafe(insert);                                   // T2 commits an edge
        return tx.$executeRawUnsafe(`TRUNCATE TABLE "ActivityDependency"`);
      }, { isolationLevel: 'RepeatableRead' }).then(() => null, (e: unknown) => String(e));
      expect(fixed, 'a fixed-snapshot truncate must be refused').not.toBeNull();
      expect(fixed, 'naming the isolation level and why it cannot be trusted here')
        .toMatch(/cannot be truncated from a repeatable read transaction/u);
      expect(await edgeCount(), 'and the committed edge survives').toBe(1);

      // (2) THE SAME SHAPE under READ COMMITTED, where the fast path IS sound: the trigger's own
      // query takes a fresh snapshot after the lock is granted, so it SEES the committed edge and
      // the ORIGINAL rule — not the new guard — is what refuses.
      const live = await t.prisma.$transaction(async (tx) => {
        await tx.$queryRawUnsafe(`SELECT COUNT(*) FROM "ActivityDependency"`);
        return tx.$executeRawUnsafe(`TRUNCATE TABLE "ActivityDependency"`);
      }, { isolationLevel: 'ReadCommitted' }).then(() => null, (e: unknown) => String(e));
      expect(live, 'a populated table is refused under READ COMMITTED too').not.toBeNull();
      expect(live, 'by the rule about the record, not by the isolation guard')
        .toMatch(/holds the sequencing record/u);
    ...
    // reset that CASCADEs through "Activity" on a database where no edge was ever written — the
    // truncate still succeeds. Refusing unconditionally would push those callers into disabling
    // the seal routinely, which is how a seal becomes a formality.
    expect(await edgeCount(), 'the table is empty').toBe(0);
    await t.prisma.$executeRawUnsafe(`TRUNCATE TABLE "ActivityDependency"`);

    // (4) and a fixed-snapshot truncate of an EMPTY table is refused ANYWAY, because the emptiness
    // it would be relying on is exactly the thing its snapshot cannot answer for.
    const empty = await t.prisma.$transaction(
      async (tx) => tx.$executeRawUnsafe(`TRUNCATE TABLE "ActivityDependency"`),
      { isolationLevel: 'Serializable' },
    ).then(() => null, (e: unknown) => String(e));
    expect(empty, 'serializable is refused as well').not.toBeNull();
    expect(empty, 'and says which isolation level it saw')
      .toMatch(/cannot be truncated from a serializable transaction/u);
  }, 180_000);

  // ── P37 ────────────────────────────────────────────────────────────────────────────────────
  it('P37 the constraint names this file hard-codes are the names Prisma would keep', async () => {
    // THE FINDING (migration.sql:1191), reproduced. Two of the five foreign keys are named without
    // the composite `projectId` prefix Prisma's defaults use, and the relations carried no `map:`.
    // Prisma treats `schema.prisma` as the DESIRED shape, so the next generated migration would
    // have RENAMED both as drift — after which the twenty-row enforcement inventory, which names
    // each constraint literally, would reject a CORRECTLY migrated database and `migrate.sh` would
    // refuse to start. A self-inflicted outage, on a database that was never wrong.
    //
    // MEASURED with `prisma migrate diff` against a migrated database before the fix:
    //   ALTER TABLE "ActivityDependency" RENAME CONSTRAINT "ActivityDependency_createdBy_fkey"
    //     TO "ActivityDependency_projectId_createdById_fkey";
    //   ALTER TABLE "ActivityDependency" RENAME CONSTRAINT "ActivityDependency_revokedBy_fkey"
    //     TO "ActivityDependency_projectId_revokedById_fkey";
    //
    // Asked of Prisma itself rather than of the naming rule, because the rule is Prisma's to
    // change and this is a question about what Prisma actually does.
    const drift = execFileSync('npx', [
      'prisma', 'migrate', 'diff',
      '--from-url', process.env.DATABASE_URL!,
      '--to-schema-datamodel', join(__dirname, '..', '..', 'prisma', 'schema.prisma'),
      '--script',
    ], { encoding: 'utf8', cwd: join(__dirname, '..', '..'), stdio: 'pipe' });

    const mine = drift.split('\n').filter((l) => l.includes('ActivityDependency'));
    expect(mine, `no generated migration may touch this unit's objects:\n${mine.join('\n')}`).toEqual([]);

    // And the names really are the ones the inventory hard-codes — so this probe fails if either
    // side is renamed alone, which is the coupling that matters.
    const live = await t.prisma.$queryRawUnsafe<Array<{ names: string }>>(`
      SELECT string_agg(conname, ',' ORDER BY conname) AS names
        FROM pg_constraint
       WHERE conrelid = '"ActivityDependency"'::regclass AND contype = 'f'`);
    expect(live[0]!.names).toBe([
    ...
    for (const name of ['ActivityDependency_createdBy_fkey', 'ActivityDependency_revokedBy_fkey']) {
      expect(readFileSync(join(__dirname, '..', '..', 'prisma', 'schema.prisma'), 'utf8'),
             `schema.prisma must map ${name}`).toContain(`map: "${name}"`);
