...
export function InheritedContext({
  ...
  // The inherited place can VANISH under an open form — another PMC deletes the node while
  // this modal is up. The trail empties and the picker appears, but the caller would still
  // be holding the dead id, so its readiness check passes and the save carries a node the
  // server rejects after the modal has closed and taken the typed form with it. Drop the
  // value so the UI and the caller agree: nothing is chosen, and nothing can be saved.
  const vanished = inherited && value !== null && trail.length === 0;
  useEffect(() => {
    if (vanished) onChange(null);
  }, [vanished, onChange]);
