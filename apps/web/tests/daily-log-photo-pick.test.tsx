...
/**
 * Codex on `1b663396` — `readAsDataURL` is ITSELF an async boundary the store cannot see
 * across. This defect predates the capture stamp: `main` already dispatched from `onload`
 * with nothing binding the photo to the project it was chosen in. If the user switches
 * projects while the read is in flight, `useApiSync` installs the new project's gateway and
 * the callback files A's photo — and A's `photoNode` — under B.
 */
describe('a photo belongs to the project it was chosen in', () => {
  it('drops the photo when the project switched while the file was being read', async () => {
    ...
    useStore.setState({ online: true } as never);

    const { release } = stubFileReader(dataUrlOf(jpegWithCaptureTime()));
    const { DailyLogScreen } = await import('@/screens/DailyLogScreen');
    const r = render(<DailyLogScreen />);

    fireEvent.change(r.getByTestId('progress-file'), {
      target: { files: [new File([], 'site.jpg', { type: 'image/jpeg' })] },
    });

    // the user switches to project B while the read is still in flight
    useStore.setState((st) => ({ ...st, activeProjectId: 'project-b', projectScopeGeneration: 2 }));
    release();
    await new Promise((res) => setTimeout(res, 0));

    // B never receives A's photo
    expect(uploadMedia).not.toHaveBeenCalled();
  });

  it('uploads normally when the project did NOT change', async () => {
    vi.stubEnv('VITE_API_URL', 'http://api.test');
    vi.resetModules();
    const { useStore, getInitialState } = await import('@/store/store');
    const scope = await import('@/store/projectScope');
    ...
    expect(uploadMedia.mock.calls[0][0]).toMatchObject({ takenAt: '11 Aug 2026 · 9:15 AM' });
  });
});
